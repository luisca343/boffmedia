 ...
 * was actually missing is smaller: structured fragments users' release notes
 * come from, and a check that the canonical version and its build copies
 * agree. This script keeps the historical Markdown changelog useful too.
 *
 * The product version itself is now canonical in release/version.json. The
 * desktop package, Tauri config and Cargo manifest remain synchronized copies
 * because Tauri reads those files while building the installer.
 *
 * FOUR MODES:
 *
 *   --check-versions
 *       release/version.json, apps/desktop/package.json,
 *       src-tauri/tauri.conf.json and src-tauri/Cargo.toml must all carry the
 *       same version. Runs in the lint chain. A drifted copy can ship an
 *       artifact reporting a version the product release feed does not know.
 *
 *   --check <version>
 *       CHANGELOG.md must have a section for this version, and it must not be
 *       empty. Useful for the historical Markdown format and manual inspection.
 *
 *   --notes <version>
...
function desktopVersions() {
  ...
  if (!cargoMatch)
    die("apps/desktop/src-tauri/Cargo.toml: no version under [package].");
  return {
...
function canonicalVersion() {
  const file = join(ROOT, "release/version.json");
  let parsed;
  try {
    parsed = JSON.parse(readFileSync(file, "utf8"));
  } catch (error) {
    die(`release/version.json is missing or invalid: ${error.message}`);
  }
  ...
  if (
    ...
    die(
      "release/version.json must contain a valid SemVer string in `version`.",
    );
  }
  return version;
}
...
function changelogSections() {
  ...
  try {
    ...
    die(
      "CHANGELOG.md is missing. Every released version needs a section in it.",
    );
  }
  ...
  if (sections.size === 0) {
    die(
      "CHANGELOG.md has no `## [version]` headings -- its format has changed and this script reads nothing.",
    );
  }
  return sections;
}
...
if (mode === "--check-versions") {
  const versions = {
    "release/version.json": canonicalVersion(),
    ...desktopVersions(),
  };
  const distinct = new Set(Object.values(versions));
  if (distinct.size !== 1) {
    console.error(
      "FAIL -- the product version disagrees across its canonical source and build copies:\n",
    );
    for (const [file, v] of Object.entries(versions))
      console.error(`  ${v.padEnd(12)} ${file}`);
    console.error(
      "\nAll sources must match. tauri.conf.json is what desktop-release.yml checks the\n" +
  ...
  console.log(
    `OK -- product version ${version} agrees across the canonical source and all build copies.\n`,
  );
  process.exit(0);
}
...
if (mode === "--check" || mode === "--notes") {
  ...
  if (!body) {
    die(
      `CHANGELOG.md's section for ${version} is empty. An empty release note is not a release note.`,
    );
  }
  if (mode === "--notes") {
    ...
    console.log(
      `OK -- CHANGELOG.md documents ${version} (${body.split("\n").length} lines).\n`,
    );
  }
  process.exit(0);
}
...
if (mode === "--draft") {
  ...
  console.log(
    `<!-- DRAFT from ${since ? range : "the last 50 commits"}. Commit subjects are`,
  );
  console.log(
    `     written for developers; rewrite each line for a player before shipping.`,
  );
  console.log(
    `     ${skipped} commit(s) omitted as internal (chore/refactor/test/docs/ci). -->\n`,
  );
