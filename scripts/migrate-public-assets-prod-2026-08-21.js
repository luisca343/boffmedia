#!/usr/bin/env node
/**
 * One-shot, idempotent reorganization of the PRODUCTION public/ tree.
 *
 * Production is a different generation of the tree than a developer checkout —
 * fonts/ and img/ sit at the top level rather than under assets/, several
 * directories exist only here, and others sit where the code does not look.
 * That is why this map is separate from the developer one: running the wrong
 * map against the wrong tree half-migrates it, so the two never share a file.
 *
 * Environment overrides (all three should be set explicitly on a server):
 *   MIGRATE_PUBLIC_ROOT  - the asset tree            (default: <cwd>/public)
 *   MIGRATE_LABOON_ROOT  - network blob store        (default: <cwd>/laboon)
 *   MIGRATE_UPLOADS_ROOT - local uploads tier        (default: <cwd>/var/uploads)
 *
 * Invariants:
 *   - Renames where possible; cross-mount falls back to copy + verify + delete.
 *   - Pre-flight: if a source AND its destination both exist, abort before any write.
 *   - Idempotent: source absent + destination present = skipped.
 *   - Nothing is deleted outright; removals are moved into BACKUP first.
...
// Order matters twice over: img/games/tcg is lifted out BEFORE img/ moves, and
// audio/ lands INSIDE boffmedia/img, so it has to follow that move rather than
// precede it — otherwise each would find its destination parent already taken.
const MOVES = [
  { from: 'img/games/tcg', to: 'boffmedia/tools/tcg' },
  { from: 'fonts', to: 'boffmedia/fonts' },
  { from: 'img', to: 'boffmedia/img' },
  { from: 'audio', to: 'boffmedia/img/audio' },
  { from: 'battlesim', to: 'boffmedia/tools/battlesim' },
  { from: 'data/mhwilds', to: 'boffmedia/tools/mhwilds' },
  // Both of these sit where the code does not look. netfluis.service.ts reads
  // smartrotom/netfluis/series, and wingull-world.service.ts reads
  // smartrotom/data/pueblos — the towns are a live 404 until this moves.
  { from: 'netfluis', to: 'smartrotom/netfluis' },
  { from: 'smartrotom/pueblos', to: 'smartrotom/data/pueblos' },
  { from: 'uploads/profiles', to: path.join(UPLOADS, 'profiles') },
];

// Nothing here is referenced by any code path. Moved into BACKUP, never deleted
// outright, so a wrong call about any of them is reversible.
const REMOVE = [
  { from: 'nicolasa.gif', to: path.join(BACKUP, 'nicolasa.gif') },
  { from: 'teras-0.0.44b.zip', to: path.join(BACKUP, 'teras-0.0.44b.zip') },
  { from: 'gama.png', to: path.join(BACKUP, 'gama.png') },
  { from: 'profile.png', to: path.join(BACKUP, 'profile.png') },
  { from: 'minecraft', to: path.join(BACKUP, 'minecraft') },
  { from: 'smartrotom/img2', to: path.join(BACKUP, 'smartrotom-img2') },
  { from: 'data/ptgcp', to: path.join(BACKUP, 'ptgcp') },
  { from: 'data/lootboxConfig.json', to: path.join(BACKUP, 'lootboxConfig.json') },
