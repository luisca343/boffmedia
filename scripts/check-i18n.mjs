...
// Workspace tool packages (@boffmedia/tools-*) own their own catalogs and both
// hosts merge them UNCONDITIONALLY — see apps/web/src/i18n/request.ts. They are
// therefore always loaded, on every route, and a key living in one is just as
// reachable as a CORE file's. Without this the checker reports every
// package-owned key as undefined, because it only ever reads apps/web/locales.
const PACKAGE_CATALOG_GLOB = 'packages/tools';
const packageCatalogKeys = () => {
  const out = new Set();
  let domains = [];
  try {
    domains = readdirSync(PACKAGE_CATALOG_GLOB);
  } catch {
    return out; // no tool packages in this checkout
  }
  for (const domain of domains) {
    const file = join(PACKAGE_CATALOG_GLOB, domain, 'src/catalog/messages.ts');
    let src;
    try {
      src = readFileSync(file, 'utf8');
    } catch {
      continue; // a tool package without a catalog is fine (e.g. the kit)
    }
