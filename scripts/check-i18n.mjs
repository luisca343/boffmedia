...
// Workspace packages own their own catalogs and both hosts merge them
// UNCONDITIONALLY — see apps/web/src/i18n/request.ts. They are therefore always
// loaded, on every route, and a key living in one is just as reachable as a
// CORE file's. Without this the checker reports every package-owned key as
// undefined, because it only ever reads apps/web/locales.
//
// Two SHAPES of catalog root, not one, because the rule is "whichever package
// renders a string owns it" and that is not exclusive to the tool packages:
//   - `dir`   — one catalog per child directory (every @boffmedia/tools-*)
//   - `file`  — a single catalog at a fixed path (@boffmedia/ui, which owns the
//               giveaways kit's strings because it renders them)
// Adding a package that owns strings means adding a line here; nothing else in
// this script needs to know.
const PACKAGE_CATALOGS = [
  { kind: 'dir', root: 'packages/tools', catalog: 'src/catalog/messages.ts' },
  { kind: 'file', path: 'packages/ui/src/messages/index.ts' },
];

/** Keys declared by one catalog file, or none if it is absent/unparseable. */
const catalogKeys = (file, out) => {
  let src;
  try {
    src = readFileSync(file, 'utf8');
  } catch {
    return; // a package without a catalog is fine (e.g. the kit)
  }
  // The catalog is generated by JSON.stringify, so the object literal between
  // the assignment and the `as const` is valid JSON.
  const start = src.indexOf('export const messages = ');
  const end = src.lastIndexOf(' as const;');
  if (start === -1 || end === -1) return;
  const json = src.slice(start + 'export const messages = '.length, end);
  const parsed = JSON.parse(json);
  if (!parsed[BASE]) return;
  for (const key of leaves(parsed[BASE]).keys()) out.add(key);
};

const packageCatalogKeys = () => {
  const out = new Set();
  for (const entry of PACKAGE_CATALOGS) {
    if (entry.kind === 'file') {
      catalogKeys(entry.path, out);
      continue;
    }
    let children = [];
    try {
      children = readdirSync(entry.root);
    } catch {
      continue; // that root is not in this checkout
    }
    for (const child of children) catalogKeys(join(entry.root, child, entry.catalog), out);
  }
  return out;
};
const PACKAGE_KEYS = packageCatalogKeys();
let missing = 0;
let orphan = 0;
...
for (const [root, users] of rootUsage) {
  ...
  // A root can also be provided by a tool package's catalog rather than by a
  // file under locales/ — `tools` is, entirely.
  const fromPackage = [...PACKAGE_KEYS].some((k) => k === root || k.startsWith(`${root}.`));

  if (!providers.length && !fromPackage) {
    violations.push(
  ...
  // A package catalog is merged on every route, so a root it provides is CORE
  // by construction — it can never be the scoped half of a split.
  const alwaysLoaded = fromPackage || providers.some((p) => !scoped.has(p));
...
for (const file of sources) {
  ...
  const route = routeOf(file);
  const available = [...loadedFor(route).flatMap((f) => [...fileKeys[f]]), ...PACKAGE_KEYS];
...
const allKeys = new Set([...files.flatMap((f) => [...fileKeys[f]]), ...PACKAGE_KEYS]);
