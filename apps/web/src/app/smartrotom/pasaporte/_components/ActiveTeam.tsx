  ...
  return (
    <div className={`font-vinque flex flex-col justify-around h-[90%] w-full ${className}`}>
      {team?.map((pokemon, index) => {
        ...
        return (
          <div key={index} className="flex items-center min-h-[100px] w-full border-b border-edge/30 pb-2">
            {/* Pokemon Sprite and Basic Info */}
            <div className="w-[20%] flex flex-col items-center space-y-1">
              <div className="bg-stone-200/50 rounded-full p-1 border border-edge/30">
                <PokemonSprite
                  showStatus={false}
                  width={60}
                  height={60}
            ...
            {/* Ability and Item */}
            <div className="w-[20%] px-4 border-l border-dashed border-edge/50 min-h-[80px] flex flex-col justify-center">
              <div className="space-y-2 text-sm">
                <p className="font-bold">{t(`ability_${pokemon.ability.replace(" ", "")}`)}</p>
                {pokemon.item !== "item.minecraft.air" && <p className="text-ink-dim">{pokemon.item}</p>}
              </div>
            </div>
            ...
            {/* Stats Grid */}
            <div className="w-[40%] px-4 border-l border-dashed border-edge/50 min-h-[80px]">
