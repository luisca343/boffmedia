    ...
    return (
      <div>
        ...
        <div className="mb-2 text-lg">
          ...
          <div className="w-full bg-layer-2 rounded-full h-3">
        ...
        {visibleSections.map((section) => {
          ...
          return (
            <div key={section.title} className="mb-2">
              ...
              <div className="w-full bg-layer-2 rounded-full h-1 mb-2">
                <div 
                  className={`${completed === section.data.length ? 'bg-warning' : 'bg-secondary'} h-1 rounded-full`}
  ...
  return (
    <div className="flex flex-col p-4 space-y-6 font-vinque">
      {visibleSections.map((section) => {
        ...
        return (
          <div key={section.title} className="space-y-3 bg-white/80 rounded-lg p-4 shadow-sm border border-edge">
            <div className="flex justify-between items-center">
            ...
            <div className="w-full bg-layer-2 rounded-full h-2 mb-3">
              <div 
                className={`${completed === section.data.length ? 'bg-warning' : 'bg-secondary'} h-2 rounded-full`}
                style={{ width: `${sectionPercentage}%` }} 
              />
            </div>
            ...
            <div className={`w-12 h-12 ${data.completed ? "" : "filter brightness-0 opacity-60"}`}>
              ...
              {!data.completed &&  (
                <div className="absolute inset-0 flex items-center justify-center">
                  <Lock className="w-3 h-3 text-ink-muted opacity-80" />
                </div>
              )}
            </div>
          </div>
      ...
      >
        <div className="flex gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-ink-dim line-clamp-2">{data.description}</p>
            <p className="text-xs font-medium mt-1">
              {data.completed ? (
                <span className="text-warning">Obtenida: {parseDate(data.completedAt)}</span>
              ) : (
                <span className="text-ink-muted">No completado</span>
              )}
            </p>
          </div>
        </div>
            ...
            >
              ...
              {!data.completed &&  (
                <div className="absolute inset-0 flex items-center justify-center z-20">
                  <Lock className="w-5 h-5 text-ink-muted opacity-80" />
                </div>
              )}
            </div>
          </div>
      ...
      >
        <div className="flex gap-3">
          ...
          <div className="flex flex-col space-y-2">
            ...
            <p className="text-sm text-ink-dim">{data.description}</p>
