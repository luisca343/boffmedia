...
export function TemplatePicker({
  ...
  return (
    <Overlay onClose={onClose} align="center">
      <div className={`${MODAL_PANEL} w-[35rem] max-w-[92vw]`}>
        <div className="flex items-center gap-2.5 border-b border-nt-border px-[1.125rem] py-4">
          ...
          <h3 className="m-0 flex-1 text-[1rem] font-[650] text-nt-fg">{t("commands.fromTemplate")}</h3>
          <button onClick={onClose} aria-label={t("common.close")} className="text-nt-fg-subtle hover:text-nt-fg">
            <Icon name="x" size={18} />
          </button>
        </div>
        <div className="grid gap-2 p-[1.125rem] sm:grid-cols-2">
          {TEMPLATES.map((tpl) => (
            <button
              key={tpl.id}
              onClick={() => {
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block text-[0.84375rem] font-semibold text-nt-fg">{t(tpl.nameKey)}</span>
                <span className="mt-0.5 block text-[0.75rem] leading-[1.4] text-nt-fg-muted">{t(tpl.descKey)}</span>
              </span>
            </button>
          ))}
        </div>
