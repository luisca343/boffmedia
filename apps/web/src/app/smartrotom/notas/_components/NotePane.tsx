...
export function NotePane(props: NotePaneProps) {
  ...
  useEffect(() => {
    if (docRef.current && loaded?.content != null) {
      docRef.current.innerHTML = loaded.content;
  ...
  return (
    ...
    >
      <Toolbar onCmd={onInput} onInsertLink={(title) => props.onOpenTitle(title)} />
      ...
      <div className="nt-scroll flex-1 overflow-auto bg-nt-bg-1">
        <div
          ref={docRef}
          key={note.id}
