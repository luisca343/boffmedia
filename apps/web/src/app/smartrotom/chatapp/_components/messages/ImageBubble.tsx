...
export function ImageBubble({
  ...
  return (
    <div className={cn("w-[20rem] max-w-full overflow-hidden rounded-[10px] shadow-ca-bubble", out ? "bg-ca-bubble-out" : "bg-ca-bubble-in")}>
      <div className="group relative aspect-[16/10] cursor-pointer overflow-hidden bg-ca-950" onClick={onOpen}>
        ...
        {scanned && (
          <div className="absolute left-2 top-2 flex items-center gap-[0.3125rem] rounded-full border border-ca-online/40 bg-black/60 px-2 py-[3px] font-ca-mono text-[0.625rem] tracking-[.06em] text-ca-online backdrop-blur-sm">
        ...
        <button
          ...
          className="absolute right-2 top-2 grid h-[1.875rem] w-[1.875rem] place-items-center rounded-full bg-black/55 text-white opacity-0 backdrop-blur-sm transition-opacity group-hover:opacity-100"
        >
          <Icon name="download" size={15} />
        </button>
      </div>

      {meta?.caption && <div className="px-[0.6875rem] pt-[0.4375rem] text-[0.8625rem] text-ca-bubble-in-text">{meta.caption}</div>}

      {scanned && (
        <>
          <button
            onClick={() => setOpen((s) => !s)}
            className="flex w-full items-center justify-between gap-2 border-t border-ca-500/20 px-[0.6875rem] py-2 text-[0.78125rem] text-ca-300 hover:bg-ca-500/10"
          >
          ...
          {open && (
            <div className="flex flex-col gap-2 px-[0.6875rem] pb-2.5 pt-1 text-[0.75rem]">
              {meta?.location && (
              ...
              {entities.map((e, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-[0.5625rem] rounded-ca-md bg-ca-500/10 p-1.5">
                    <div className="grid h-[2.125rem] w-[2.125rem] flex-none place-items-center rounded-ca-md bg-ca-500/[.12]">
                      {isPokemon ? (
                        <img src={SPRITE(dex)} alt="" className="h-[1.875rem] w-[1.875rem] [image-rendering:pixelated]" />
                      ) : (
                        <Icon name="users" size={16} />
                      )}
                    </div>
                    <div className="min-w-0">
                      <div className="text-[0.78125rem] font-semibold text-ca-100">
                        {isPokemon ? (e as PokemonEntity).species : (e as { name: string }).name}
                        {isPokemon && (e as PokemonEntity).form === "Shiny" ? " ✦" : ""}
                      </div>
                      <div className="font-ca-mono text-[0.65625rem] text-ca-400">
                        {isPokemon ? `#${String(dex).padStart(3, "0")} · ${(e as PokemonEntity).form || "Normal"}` : "NPC"} · cob. {e.coverage}%
                      </div>
                    </div>
                    <div className="ml-auto font-ca-mono text-[0.65625rem] text-ca-accent-soft">{e.distance}m</div>
                  </div>
                );
              })}
            </div>
      ...
      <div className="px-3 pb-2 pt-0 text-right text-[0.65625rem] text-ca-500">{time}</div>
    </div>
  );
}
