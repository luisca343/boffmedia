  ...
  return (
    <aside className="flex w-[360px] flex-none flex-col border-r border-ca-800 bg-ca-panel">
      <div className="flex h-[60px] items-center gap-2.5 bg-ca-header px-4">
        ...
        <div className="text-[17px] font-bold text-ca-50">ChatApp<small className="block text-[11px] font-medium text-ca-400">SmartRotom</small></div>
      </div>
      <div className="flex flex-1 flex-col items-center justify-center gap-4 p-10 text-center">
        ...
        <h3 className="text-[17px] font-semibold text-ca-50">Aún no tienes chats</h3>
        <p className="max-w-[360px] text-[14.5px] leading-[1.6] text-ca-400">Empieza una conversación con alguien de tu servidor o crea un grupo para tu equipo.</p>
        <Button onClick={onNew}><Icon name="plus" size={16} /> Nuevo chat</Button>
      </div>
    </aside>
  );
}
  ...
  return (
    <section className="relative flex min-w-0 flex-1 flex-col bg-ca-wallpaper">
      ...
      <div className="relative z-[1] flex flex-1 flex-col items-center justify-center gap-4 p-10 text-center">
        ...
        <h3 className="text-[22px] font-semibold text-ca-50">Sin conexión con SmartRotom</h3>
        <p className="max-w-[360px] text-[14.5px] leading-[1.6] text-ca-400">No hemos podido sincronizar tus chats. Comprueba tu conexión y vuelve a intentarlo.</p>
        <Button onClick={onRetry}><Icon name="zap" size={16} /> Reintentar</Button>
      </div>
    </section>
  );
}
