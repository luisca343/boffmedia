    ...
    <Modal bare title="" onClose={onClose} scrimClassName="bg-black/85">
      <div onClick={(e) => e.stopPropagation()} className="flex w-full max-w-[720px] flex-col gap-3.5">
        <div className="relative overflow-hidden rounded-[18px] border border-ca-700 bg-ca-950">
          <img src={data.imageUrl} alt={meta?.caption || "Captura"} className="max-h-[70vh] w-full object-contain" />
        ...
        <div className="flex justify-center gap-2">
          <Button variant="ghost" onClick={save}><Icon name="download" size={16} /> Guardar en galería</Button>
          {meta?.location && <Button onClick={goLocation}><Icon name="mappin" size={16} /> Ir a la ubicación</Button>}
        </div>
      </div>
    </Modal>
  );
