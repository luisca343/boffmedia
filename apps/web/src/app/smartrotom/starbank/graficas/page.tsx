  ...
  if (isLoading || !activeAccount) {
    return (
      <>
        ...
        <Skeleton className="h-[20rem] rounded-sb-lg" />
      </>
    );
  }
    ...
    <>
      ...
      <div className="grid gap-4 md:grid-cols-12">
        ...
        <Card className="md:col-span-4">
          ...
          <CardBody>
            <div className="font-sb-display text-[2rem] font-semibold tabular-nums tracking-[-0.01em]">{formatMoney(largest)}</div>
            <div className="text-[0.8125rem] text-sb-fg-muted">{t("graficas.charts.largestExpenseDesc")}</div>
            <div className="h-px bg-sb-border" />
            <div className="mb-1.5 block text-[0.75rem] font-semibold uppercase tracking-[0.02em] text-sb-fg-muted">{t("graficas.charts.activity14d")}</div>
      ...
      <div className="grid gap-4 md:grid-cols-12">
        <Card className="md:col-span-6">
          ...
          <CardBody>
            ...
            <div className="flex justify-center gap-4 pt-2">
              <span className="flex items-center gap-1.5 text-[0.75rem]"><span className="size-2.5 rounded bg-sb-pos-2" /> {t("graficas.kpi.income")}</span>
              <span className="flex items-center gap-1.5 text-[0.75rem]"><span className="size-2.5 rounded bg-sb-neg-2" /> {t("graficas.kpi.expenses")}</span>
            </div>
          </CardBody>
        </Card>
        ...
        <Card className="md:col-span-6">
          ...
          <CardBody className="flex-row items-center justify-between gap-7">
            {byCat.length === 0 ? (
              <div className="w-full py-8 text-center text-[0.8125rem] text-sb-fg-muted">{t("graficas.charts.noExpenses")}</div>
            ) : (
              <>
