      ...
      <div className="relative flex flex-col gap-3.5">
        ...
        <div className="inline-flex items-baseline gap-1.5 font-sb-display text-[clamp(40px,6vw,64px)] font-bold leading-none tracking-[-0.03em] tabular-nums">
          ...
          <button
            type="button"
            onClick={() => setHide((v) => !v)}
            aria-label={hide ? t("heroBalance.showBalance") : t("heroBalance.hideBalance")}
        ...
        {weekDelta && (
          <div className="inline-flex items-center gap-1.5 text-[13px] text-[#b6d3ff]">
            ...
            <span className="text-[#9bb3da]">{t("heroBalance.last7days")}</span>
          </div>
        )}

        <div className="mt-1.5 flex flex-wrap gap-2.5">
          <Button variant="solid" size="md" href={`${BASE}/enviar`} className="backdrop-blur-[8px]"><Ico name="send" size={14} /> {t("common.send")}</Button>
          <Button variant="glass" size="md"><Ico name="qrcode" size={14} /> {t("heroBalance.request")}</Button>
          <Button variant="glass" size="md" onClick={() => router.push(`${BASE}/cuentas`)}><Ico name="plus" size={14} /> {t("heroBalance.moveBetween")}</Button>
          <Button variant="glass" size="md" href={`${BASE}/graficas`}><Ico name="chart" size={14} /> {t("heroBalance.insights")}</Button>
        </div>
      </div>
