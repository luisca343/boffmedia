...
export function TopBar({ currentPage, account, accounts, onSelectAccount, onOpenAccounts }: {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-4 border-b border-sb-border bg-sb-surface/85 px-4 py-3 backdrop-blur-[10px] backdrop-saturate-[1.4] md:px-7 md:py-3.5">
      <div>
        <div className="text-[0.75rem] text-sb-fg-muted">
          {crumbs.map((c, i) => (
            <span key={i}>{i > 0 ? " / " : ""}<strong className="font-semibold text-sb-fg-2">{t(c)}</strong></span>
          ))}
        </div>
        <div className="font-sb-display text-[1.125rem] font-semibold tracking-[-0.01em]">{t(titleKey)}</div>
      </div>

      <div className="relative hidden max-w-[26.25rem] flex-1 md:block">
        <Ico name="search" size={16} className="absolute left-[0.6875rem] top-1/2 -translate-y-1/2 text-sb-fg-subtle" />
        <input
          placeholder={t("topBar.searchPlaceholder")}
          aria-label={t("common.search")}
          className="h-[2.375rem] w-full rounded-sb-md border border-sb-border bg-sb-surface-2 pl-9 pr-3 text-sb-fg outline-none transition-colors focus:border-sb-400 focus:bg-sb-surface"
        />
        <span className="absolute right-2.5 top-1/2 -translate-y-1/2 rounded-md border border-sb-border bg-sb-surface px-1.5 py-0.5 text-[0.625rem] text-sb-fg-subtle">⌘K</span>
      </div>
      ...
      <div className="ml-auto flex items-center gap-2">
        <button
          ...
          className="relative grid size-[2.375rem] place-items-center rounded-sb-md border border-sb-border bg-sb-surface text-sb-fg-2 transition-colors hover:border-sb-border-strong hover:bg-sb-surface-2 hover:text-sb-fg"
        >
          <Ico name="bell" size={16} />
          <span className="absolute right-[0.5625rem] top-2 size-2 rounded-full bg-sb-neg-2 shadow-[0_0_0_2px_rgb(var(--sb-surface))]" aria-hidden />
        </button>
        ...
        <div className="relative" ref={ref}>
          ...
          >
            <span className="size-[1.875rem] overflow-hidden rounded-full">
              <AccountAvatar account={account ?? undefined} size={30} />
            </span>
            <div className="hidden leading-[1.15] sm:block">
              <div className="text-[0.78125rem] font-semibold">{account ? displayName(account.name) : "—"}</div>
              <div className="text-[0.6875rem] tabular-nums text-sb-fg-muted">{formatMoney(account?.balance ?? 0)}</div>
            </div>
          ...
          {open && (
            <div className="absolute right-0 top-full z-50 mt-2 w-72 overflow-hidden rounded-sb-md border border-sb-border bg-sb-surface shadow-sb-3">
              <div className="border-b border-sb-border px-4 py-2.5">
                <div className="text-[0.6875rem] uppercase tracking-[0.1em] text-sb-fg-subtle">{t("sidebar.switchAccount")}</div>
              </div>
              <div className="max-h-72 overflow-y-auto py-1">
                {list.length === 0 ? (
                  <div className="px-4 py-3 text-[0.8125rem] text-sb-fg-muted">{t("topBar.noAccounts")}</div>
                ) : (
                  list.map((acc) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-[0.8125rem] font-semibold">{displayName(acc.name)}</div>
                          <div className="text-[0.71875rem] tabular-nums text-sb-fg-muted">{formatMoney(acc.balance)}</div>
                        </div>
              ...
              <div className="border-t border-sb-border">
                <button type="button" onClick={() => { setOpen(false); onOpenAccounts(); }} className="w-full px-4 py-2.5 text-left text-[0.8125rem] font-medium text-sb-700 transition-colors hover:bg-sb-50">
