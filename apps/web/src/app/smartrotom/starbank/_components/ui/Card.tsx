...
export function SectionHead({ title, eyebrow, action, className }: { title: React.ReactNode; eyebrow?: React.ReactNode; action?: React.ReactNode; className?: string }) {
  return (
    <div className={cn("flex items-center justify-between gap-3 px-5 pb-1.5 pt-[18px]", className)}>
      <div className="min-w-0">
        {eyebrow ? <div className="mb-0.5 text-[11px] uppercase tracking-[0.1em] text-sb-fg-subtle">{eyebrow}</div> : null}
        <h3 className="m-0 truncate font-sb-display text-[15px] font-semibold tracking-[-0.005em] text-sb-fg">{title}</h3>
      </div>
      {action ?? null}
    </div>
  );
