...
export function EmptyState({
  ...
  return (
    ...
    >
      ...
      <p className="font-ft-deck mx-auto mt-1 max-w-[30rem] text-xl italic text-ft-ink/85">
        {message}
      </p>
...
export function Stat({
  ...
  return (
    <div
      className={`border-ft rounded-ft min-w-[4.5rem] border-ft-ink px-3.5 py-2 text-center shadow-ft-pop-sm ${TONE[tone]}`}
    >
      <div className="font-ft-display text-3xl leading-none">{value}</div>
      <div className="font-ft-ui mt-0.5 text-[0.625rem] font-extrabold uppercase tracking-[0.18em]">
        {label}
      </div>
    </div>
  );
