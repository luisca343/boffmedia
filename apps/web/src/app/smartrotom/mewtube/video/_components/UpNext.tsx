...
function Reco({ v }: { v: VideoCardData }) {
  ...
  return (
    ...
    >
      <div className="relative aspect-video w-[168px] flex-none overflow-hidden rounded-mw-lg bg-mw-800">
        {v.thumb && (
           
          <img src={v.thumb} alt="" loading="lazy" className="h-full w-full object-cover" />
        )}
        {v.duration && (
          <span className="absolute bottom-1.5 right-1.5 rounded-[3px] bg-black/85 px-1.5 py-0.5 font-mono text-[10px] font-bold text-white">
            {v.duration}
          </span>
        )}
      </div>
      <div className="min-w-0 flex-1">
        <h4 className="m-0 mb-1 line-clamp-2 text-[13px] font-semibold leading-[1.3] group-hover:text-mw-accent">
          {v.title}
        </h4>
        <div className="inline-flex items-center gap-1 text-[11px] text-mw-fg-mute">
          {v.creator}
          {v.verified && <Check />}
        </div>
      ...
      {loading
        ? Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="flex gap-2.5 p-1.5">
              <Skeleton className="aspect-video w-[168px] flex-none rounded-mw-lg" />
