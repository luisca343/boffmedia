...
export const ChannelStats = ({
  ...
  return (
    <div className="bg-layer-3 p-5 rounded-lg">
      <h3 className="text-xl font-medium mb-4">{t("channel.stats")}</h3>
      <div className="space-y-4">
        <div>
          <p className="text-ink-muted text-sm">{t("channel.joined")}</p>
          <p className="font-medium">{formatDate(joinDate)}</p>
        </div>
        <div>
          <p className="text-ink-muted text-sm">{t("channel.totalViews")}</p>
          <p className="font-medium">{parseInt(viewCount).toLocaleString()}</p>
        </div>
        <div>
          <p className="text-ink-muted text-sm">{t("channel.totalVideos")}</p>
          <p className="font-medium">{parseInt(videoCount).toLocaleString()}</p>
        </div>
        <div>
          <p className="text-ink-muted text-sm">{t("channel.subscribers_stats")}</p>
