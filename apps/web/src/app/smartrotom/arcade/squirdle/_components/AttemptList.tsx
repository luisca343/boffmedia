import { useTranslations } from "next-intl"
import type { WordlePokemon } from "../_hooks/useGetWordlePokemon"
import { AttemptRow } from "./AttemptRow"

export interface AttemptListProps {
  guesses: WordlePokemon[]
  target: WordlePokemon | null
  gameOver: boolean
  nameOf: (raw: string) => string
}

/** The comparison grid — every guess so far, newest last. */
export function AttemptList({ guesses, target, gameOver, nameOf }: AttemptListProps) {
  const t = useTranslations("arcade")

  if (guesses.length === 0) {
    return (
      <div className="grid place-items-center rounded-xl border border-dashed border-white/[.1] px-4 py-10 text-center">
        <p className="m-0 font-ar-mono text-[0.6875rem] leading-relaxed text-ar-ink-muted">
          {t("squirdle.noAttemptsYet")}
          <br />
          {t("squirdle.guessAPokemon")}
        </p>
      </div>
    )
  }

  return (
    <div className="flex flex-col gap-2">
      <div className="hidden grid-cols-[4.75rem_1fr_1.2fr_1.2fr_.9fr_.9fr] gap-2 px-0.5 md:grid">
        {["", t("squirdle.columns.type1"), t("squirdle.columns.type1"), t("squirdle.columns.type2"), t("squirdle.columns.height"), t("squirdle.columns.weight")].map((head, i) => (
          <span
            key={head || i}
            className="self-end text-center font-ar-display text-[0.5rem] uppercase tracking-[0.1em] text-ar-ink-muted"
          >
            {head}
          </span>
        ))}
      </div>
      {guesses.map((guess, i) => (
        <AttemptRow
          key={`${guess.name}-${i}`}
          guess={guess}
          target={target}
          name={nameOf(guess.name)}
          revealed={i === guesses.length - 1 && gameOver && guess.name === target?.name}
        />
      ))}
    </div>
  )
}
