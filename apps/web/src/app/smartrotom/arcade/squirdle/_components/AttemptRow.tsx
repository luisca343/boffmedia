...
export function AttemptRow({ guess, target, name, revealed }: AttemptRowProps) {
  ...
  return (
    ...
    >
      ...
      <FeedbackCell label="Gen" fb={numberFeedback(guess.gen, target?.gen ?? guess.gen)}>
        {guess.gen}
      </FeedbackCell>
      <FeedbackCell label="Tipo 1" fb={typeFeedback(guess.type1, 1, target)}>
        {guess.type1 ? t(`type_${guess.type1}`) : "—"}
      </FeedbackCell>
      <FeedbackCell label="Tipo 2" fb={typeFeedback(guess.type2, 2, target)}>
        {guess.type2 ? t(`type_${guess.type2}`) : "—"}
      </FeedbackCell>
      <FeedbackCell label="Altura" fb={numberFeedback(guess.height, target?.height ?? guess.height)}>
        {guess.height}m
      </FeedbackCell>
      <FeedbackCell label="Peso" fb={numberFeedback(guess.weight, target?.weight ?? guess.weight)}>
        {guess.weight}
      </FeedbackCell>
    </div>
  )
