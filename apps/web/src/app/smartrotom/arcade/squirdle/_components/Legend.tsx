...
export function Legend() {
  ...
  return (
    <Panel tone="void" tight>
      <div className="mb-2.5 font-ar-display text-[0.5625rem] uppercase text-ar-cyan">{t("squirdle.legend.title")}</div>
      <ul className="m-0 flex list-none flex-col gap-2 p-0">
        {ROWS.map((row) => (
          <li key={row.state} className="flex items-center gap-2.5">
            <span
              aria-hidden
              className={cn("h-[1.375rem] w-[1.375rem] shrink-0 rounded-md border", FEED_SKIN[row.state])}
            />
            <span className="font-ar-mono text-[0.6875rem] leading-tight text-ar-ink-dim">{row.text}</span>
          </li>
        ))}
        <li className="flex items-center gap-2.5">
          ...
          <span className="font-ar-mono text-[0.6875rem] leading-tight text-ar-ink-dim">
