...
export function ClaimRewardsModal({ items, onClose, onClaimSuccess, uuid }: ClaimRewardsModalProps) {
  ...
  if (claimableItems.length === 0) {
    return (
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm p-4">
        <div className="bg-layer-1 border-2 border-edge-strong rounded-lg p-6 max-w-md w-full">
          ...
          <div className="bg-layer-2/60 rounded-lg p-4 flex flex-col items-center justify-center min-h-[200px]">
            <AlertCircle size={32} className="text-ink-muted mb-3" />
            <p className="text-ink text-center">No tienes objetos para reclamar en este momento</p>
          </div>

          <button
            onClick={onClose}
            className="w-full mt-4 bg-layer-2 hover:bg-layer-3 text-white py-3 rounded-lg border border-edge font-bold"
          >
            Cerrar
          </button>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm p-4">
      <div className="bg-layer-1 border-2 border-edge-strong rounded-lg p-6 max-w-2xl w-full">
        <div className="flex justify-between items-start mb-4">
          <h3 className="text-xl font-bold text-white">Reclamar Recompensas</h3>
          <button 
            onClick={onClose}
        ...
        {selectedNonPokemon.length > 0 && (
          <div className="bg-secondary-soft/30 border border-secondary-active/50 rounded-lg p-3 mb-4 flex items-center">
            <Box size={20} className="text-secondary-hover mr-2" />
            <div>
              ...
              <p className="text-secondary-hover text-sm">
        ...
        {selectedPokemon.length > 0 && (
          <div className="bg-warning-soft/30 border border-warning-border/50 rounded-lg p-3 mb-4">
            <p className="text-warning-hover">
        ...
        <div className="bg-layer-2/60 rounded-lg p-4 max-h-[400px] overflow-y-auto">
          {pokemonItems.length > 0 && (
            <div className="mb-4">
              <h4 className="text-cyan-300 font-medium mb-2 border-b border-edge pb-1">Pokémon</h4>
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                {pokemonItems.map(item => {
                  const isSelected = selectedItems.some(i => i.id === item.id);
                  return (
                    <div key={item.id} className="flex flex-col items-center text-ink">
                      <ItemDisplay
                        type={item.itemType!}
          ...
          {nonPokemonItems.length > 0 && (
            <div>
              <h4 className="text-amber-300 font-medium mb-2 border-b border-edge pb-1">Objetos de Minecraft</h4>
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <div className="flex space-x-3">
            <button
              onClick={onClose}
              className="bg-layer-2 hover:bg-layer-3 text-white px-4 py-2 rounded-lg border border-edge"
            >
              Cancelar
            </button>
            ...
            <button
              ...
              className={`flex items-center px-4 py-2 rounded-lg font-bold ${
                selectedItems.length === 0 || isClaiming
                  ? 'bg-layer-3 text-ink-muted cursor-not-allowed border border-edge'
