      ...
      {isLoading ? (
        ...
        notifications.map((n) => {
          ...
          const body = (
            ...
            >
              <span className={cn("grid h-[1.875rem] w-[1.875rem] flex-none place-items-center rounded-full", g.wash)}>
                <Icon name={g.icon} size={17} className={g.tone} />
              </span>
              <div className="min-w-0 flex-1">
                <div className="text-[0.875rem] leading-snug text-rk-fg">
                  <span className={unread ? "font-bold" : "font-semibold"}>{n.title}</span>
                  <span className="text-rk-fg-subtle"> · {relTime(n.createdAt)}</span>
                </div>
                {n.body && <p className="mt-0.5 truncate text-[0.84375rem] text-rk-fg-subtle">{n.body}</p>}
              </div>
