...
import { statColor, totalStatColor, getContrastingTextColor } from "../../../_components/dexUtils"
...
export function StatsTable({ pokemon, formIndex }: { pokemon: Pokemon; formIndex: number }) {
  ...
  const stats = pokemon.forms[formIndex].battleStats
    ? pokemon.forms[formIndex].battleStats
    : pokemon.forms[0].battleStats as BattleStats
    ...
    <div className="flex flex-col w-full">
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="flex items-baseline gap-2">
          <b
            className="font-orbitron font-bold text-[26px] tabular-nums"
            style={{ color: totalStatColor(statTotal) }}
          >
            {statTotal}
          </b>
          <span className="font-jetbrains text-[10px] tracking-[0.08em] uppercase text-surface-500">Total</span>
        </div>
      </div>

      {/* Header row */}
      <div
        className="grid items-center pb-2 border-b border-white/[0.06]"
        style={{ gridTemplateColumns: "100px 1fr 72px 72px", gap: "14px" }}
      >
        <span className="font-jetbrains text-[10px] tracking-[0.08em] uppercase text-surface-500">Estadística</span>
        <span className="font-jetbrains text-[10px] tracking-[0.08em] uppercase text-surface-500">Base</span>
        <span className="font-jetbrains text-[10px] tracking-[0.08em] uppercase text-surface-500 text-center">Mín N.50</span>
        <span className="font-jetbrains text-[10px] tracking-[0.08em] uppercase text-surface-500 text-center">Máx N.100</span>
      </div>
      ...
      {Object.entries(stats).map(([stat, statValue]) => {
        ...
        return (
          <div
            ...
            style={{ gridTemplateColumns: "100px 1fr 72px 72px", gap: "14px" }}
          >
            {/* Label with color mark */}
            <div className="flex items-center gap-1.5 text-[12px] text-surface-300 font-medium">
              <span
            ...
            <div className="relative h-[26px] bg-white/[0.03] rounded overflow-hidden">
              ...
              />
              <span
                className="absolute right-2 top-1/2 -translate-y-1/2 font-jetbrains text-xs font-semibold tabular-nums drop-shadow-sm"
            ...
            {/* Min */}
            <div className="font-jetbrains text-xs text-center tabular-nums text-surface-300">
              {min50}
            </div>

            {/* Max */}
            <div className="font-jetbrains text-xs text-center tabular-nums font-semibold text-primary-300">
              {max100}
            </div>
          </div>
        )
      ...
      <div className="mt-5 bg-white/[0.02] border border-white/[0.05] rounded-[10px] overflow-hidden">
        <button
          className="w-full flex items-center justify-between px-3.5 py-3 text-[12.5px] font-medium text-surface-200 hover:text-surface-50 transition-colors cursor-pointer"
        ...
        {calcOpen && (
          <div className="px-3.5 pb-3.5 grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="flex flex-col gap-1">
              <label className="font-jetbrains text-[10px] tracking-[0.08em] uppercase text-surface-500">
                Nivel
              </label>
              <input
                type="number"
                ...
                className="bg-white/[0.03] border border-white/[0.07] rounded-[7px] text-surface-100 font-jetbrains text-[13px] px-2.5 py-1.5 outline-none focus:border-primary-400/50"
              />
            </div>
            <div className="flex flex-col gap-1">
              <label className="font-jetbrains text-[10px] tracking-[0.08em] uppercase text-surface-500">
