...
export function TypeEffectivenessSection({ type1, type2 }: TypeEffectivenessSectionProps) {
  const defenseRaw = getPokemonDefense(type1!, type2)
  const coverageRaw = getPokemonCoverage(type1!, type2)
  const defense = groupByMultiplier(defenseRaw)
  const coverage = groupByMultiplier(coverageRaw)
  ...
  return (
    <div className="bg-white/[0.015] border border-white/[0.05] rounded-xl p-[16px_18px]">
      <h4 className="font-orbitron font-semibold text-sm text-surface-50 mb-3.5 pb-3 border-b border-white/[0.05] flex items-center gap-2">
        {title}
        <span className="font-inter font-normal text-[11.5px] text-surface-500 ml-auto">{sub}</span>
      </h4>
          ...
          return (
            <div
              ...
              style={{
                ...
                background: cfg.bg,
                borderColor: `color-mix(in oklab, ${cfg.fg} 22%, transparent)`,
              }}
            >
              <div
                className="flex flex-col border-r pr-2.5"
                style={{ borderColor: `color-mix(in oklab, ${cfg.fg} 18%, transparent)` }}
              >
                <span
                  className="font-orbitron font-bold text-lg leading-none tabular-nums"
                  style={{ color: cfg.fg }}
                >
                  {cfg.label}
                </span>
                <span className="font-jetbrains text-[10px] tracking-[0.08em] uppercase text-surface-400 mt-0.5">
                  {cfg.es}
                </span>
              </div>
              <div className="flex flex-wrap gap-1 items-center">
                {types.length === 0 ? (
                  <span className="text-[11.5px] text-surface-500 italic">— ningún tipo —</span>
                ) : (
                  types.map((type) => (
