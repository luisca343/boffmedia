  ...
  return (
    <div>
        ...
        {TABS.map((tb) => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-2.5 text-[0.8125rem] font-medium cursor-pointer relative transition-colors whitespace-nowrap inline-flex items-center gap-1.5 ${
                isActive ? "text-pk-primary-300" : "text-pk-surface-400 hover:text-pk-surface-100"
              }`}
            >
              {tb.label}
              <span className="font-pk-mono text-[0.625rem] text-pk-surface-500 bg-white/[0.05] px-1.5 py-px rounded-[3px]">{tb.count}</span>
      ...
      <div className="flex items-center gap-2.5 mb-3">
        <div className="relative flex-1 max-w-[18.75rem]">
          ...
          <input
            ...
            className="w-full bg-white/[0.03] border border-white/[0.07] rounded-[7px] text-pk-surface-100 text-[0.78125rem] px-2.5 py-1.5 pl-8 outline-none focus:border-pk-primary-400/50"
          />
        </div>
        <select
          value={typeFilter}
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[42.5rem]">
          <div className="grid gap-2.5 px-3 py-2 font-pk-mono text-[0.625rem] tracking-[0.08em] uppercase text-pk-surface-500 border-b border-white/[0.05]" style={{ gridTemplateColumns: GRID }}>
          ...
          {filteredMoves.length > 0 ? (
            filteredMoves.map((move, index) => (
              <Link
                ...
                className="grid gap-2.5 px-3 py-2.5 rounded-lg items-center text-[0.8125rem] transition-colors odd:bg-white/[0.012] hover:bg-white/[0.03] odd:hover:bg-white/[0.04]"
                style={{ gridTemplateColumns: GRID }}
              >
                <span className="justify-self-start inline-flex items-center justify-center bg-white/[0.05] rounded-[5px] px-2 py-[3px] font-pk-mono text-[0.6875rem] text-pk-surface-200 font-semibold">
                ...
                <span className="text-[0.6875rem] font-semibold uppercase tracking-[0.04em]" style={{ color: CATEGORY_COLORS[move.category] || "#cdd7e3" }}>
                ...
                <span className="text-center">
                  <span className="w-[1.625rem] h-[1.625rem] inline-grid place-items-center rounded text-pk-surface-400">
            ...
            <div className="py-[1.875rem] text-center text-pk-surface-500">{t("moves_no_results")}</div>
          )}
        </div>
      </div>
    </div>
