...
export function MoveDetailPane({ moveKey }: { moveKey: string }) {
  ...
  const stats = [
    { label: "Poder", value: move.basePower || "—" },
    { label: "Precisión", value: move.accuracy > 0 ? move.accuracy : "—" },
    { label: "PP", value: move.ppBase ?? "—" },
    { label: "Pokémon", value: learners.length },
  ]
    ...
    <>
      <div className="bg-white/[0.025] border border-white/[0.06] rounded-xl p-[18px_20px]">
        <div className="flex items-center gap-3 flex-wrap mb-3.5 pb-3 border-b border-white/[0.05]">
          ...
          <span className="ml-auto font-pk-mono text-[11px] text-pk-surface-500 tracking-[0.06em] uppercase">{CATEGORY_LABELS[catKey] || move.attackCategory}</span>
        </div>

        <div className="grid grid-cols-4 gap-2.5 mb-3.5">
          {stats.map((s) => (
            <div key={s.label} className="bg-white/[0.02] border border-white/[0.04] rounded-[9px] p-[10px_12px]">
              <div className="font-pk-mono text-[9.5px] tracking-[0.1em] uppercase text-pk-surface-500 mb-1">{s.label}</div>
      ...
      {learners.length > 0 && (
        <div className="bg-white/[0.025] border border-white/[0.06] rounded-xl p-[18px_20px]">
          <div className="flex items-center justify-between mb-2.5">
            <span className="font-pk-mono text-[10.5px] tracking-[0.1em] uppercase text-pk-surface-500">Pokémon que lo aprenden</span>
            <span className="text-[11px] text-pk-surface-400 font-pk-mono">· {learners.length}</span>
          </div>
