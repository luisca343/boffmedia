  ...
  return (
    <div className="bg-white/[0.025] border border-white/[0.06] rounded-[14px] p-4 flex flex-col gap-3">
      ...
      {isLoading ? (
        ...
        <div className="flex justify-center items-center h-20 text-surface-400 text-sm">
          {t("hub_recent_empty")}
        </div>
      ) : (
        <div className="flex flex-col gap-1.5">
          {enriched.map((reg, i) => {
            ...
            const spriteUrl =
              reg.spriteUrl ||
              getSpriteUrl({ id: reg.pokemonId, form: reg.formId || "base", palette: reg.paletteId || "none" })
              ...
              >
                <div
                  className={`w-12 h-12 rounded-lg grid place-items-center shrink-0 border ${
                    isShiny
                      ? "bg-[radial-gradient(40px_30px_at_50%_50%,rgba(240,171,252,0.25),transparent_70%)] border-accent-300/30"
                ...
                <div className="flex flex-col gap-0.5 min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[13.5px] font-semibold text-surface-50">{reg.name || `#${reg.pokemonId}`}</span>
                    <StatusPill status={statusKey} size="sm" showLabel={false} />
                    {reg.types?.map((type) => (
                      <TypeChip key={type} type={type} size="sm" />
                    ))}
                  </div>
                  <div className="flex items-center gap-2 text-[11.5px] text-surface-400">
                    <span className="w-[3px] h-[3px] rounded-full bg-surface-600" />
                    <span>{reg.formId || "base"}</span>
                  </div>
                </div>
                <span className="font-jetbrains text-[10.5px] text-surface-500 shrink-0 text-right">
