...
export function Compass() {
  return (
    <div className={`${CHIP} left-4 top-4 grid h-11 w-11 place-items-center rounded-full text-tx-blue-400`} aria-hidden="true">
      <span className="absolute top-[3px] text-[9px] font-extrabold text-tx-accent">N</span>
  ...
  return (
    <div className={`${CHIP} right-4 top-4 flex flex-col overflow-hidden rounded-tx-md`}>
      <button
        ...
        onClick={() => onZoom(1.25)}
        aria-label="Acercar"
        className="grid h-10 w-[42px] place-items-center text-tx-txt transition-colors duration-150 hover:bg-tx-surface-2"
      >
...
export function ScaleChip({ scale, bottom }: { scale: number; bottom: number }) {
  return (
    <div
      className={`${CHIP} left-4 flex flex-col gap-1 rounded-tx-sm px-2.5 py-1.5 text-[11px] font-bold`}
      style={{ bottom }}
    >
      <span
        className="h-1 min-w-[24px] rounded-sm bg-tx-txt-2"
        style={{ width: GRID_CELL_BLOCKS * scale }}
      />
      <span>{GRID_CELL_BLOCKS} bloques</span>
    </div>
  )
}
