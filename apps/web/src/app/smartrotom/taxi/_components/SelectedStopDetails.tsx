...
export default function SelectedStopDetails({
  ...
  return (
    <div className="absolute left-4 right-4 bottom-4 bg-white/10 backdrop-blur-lg rounded-xl shadow-2xl overflow-hidden border-2 border-yellow-400 z-30 animate-appear">
      <div className="bg-gradient-to-r from-secondary-active to-secondary-soft p-3 text-white">
      ...
      <div className="p-4">
        <p className="text-secondary-hover mb-4">{selectedStop.description || "Sin descripción disponible"}</p>
        
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="bg-white/10 p-3 rounded-lg text-center backdrop-blur-sm border border-white/20">
            <div className="flex items-center justify-center mb-1 text-secondary-hover">
          ...
          <div className="bg-white/10 p-3 rounded-lg text-center backdrop-blur-sm border border-white/20">
            ...
            <div className={`font-bold ${canAfford ? 'text-warning-hover' : 'text-red-400'}`}>
              {formatMoney(price)}
            </div>
          </div>
        </div>
