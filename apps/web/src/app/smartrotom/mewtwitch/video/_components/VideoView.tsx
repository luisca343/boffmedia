...
export function VideoView({ id }: { id: string }) {
  ...
  if (video.isLoading) return <div className="p-6 text-sm text-mw-fg-faint">Cargando vídeo…</div>
  if (!v) return <div className="p-6 text-sm text-mw-fg-faint">No se encontró el vídeo.</div>
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-4 pb-20 pt-6 md:px-10">
      ...
      <div className="flex flex-wrap items-center gap-2 text-[13px] text-mw-fg-mute">
        ...
        <span>·</span>
        <span>{compactCount(v.view_count)} visitas</span>
        <span>·</span>
