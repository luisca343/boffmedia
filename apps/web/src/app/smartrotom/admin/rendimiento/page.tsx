...
export default function ServerPerformanceMonitor() {
  ...
  if (!performance) {
    return (
      <div className="w-full min-h-screen bg-black text-warning-hover font-mono p-4 flex items-center justify-center">
        <div className="animate-pulse flex flex-col items-center">
          <div className="text-warning text-xl mb-2">Inicializando sistema de monitoreo...</div>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {performanceApps.map((app) => (
            <Card key={app.nombre} className="bg-black border border-warning-border/30 hover:border-warning-border rounded-md overflow-hidden transition-all duration-300 hover:shadow-neon">
              <CardHeader className="flex flex-row items-center justify-between pb-2 border-b border-warning-border/50">
                <div className="flex items-center space-x-3">
                  <div className="bg-warning-soft p-2 rounded-md text-warning-hover">
                    {app.icono}
                  </div>
                  <CardTitle className="text-warning-hover font-bold text-lg">{app.nombre}</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="pt-4">
                <CardDescription className="text-warning/80 mb-2">
                  <span className="text-warning/60">{"//"} </span>
                  {app.descripcion}
                </CardDescription>
                <div className="flex items-center justify-between">
                  <p className="text-3xl font-mono font-bold text-warning">{app.getValue(performance)}</p>
                  {app.nombre === "TPS" && (
                    <div className="h-2 w-16 bg-black rounded-full overflow-hidden border border-warning-border/50">
                      <div 
        ...
        <div className="mt-6 p-4 border border-warning-border/40 rounded-md bg-black/60">
          <div className="flex flex-col sm:flex-row justify-between items-center">
            <p className="text-warning text-sm mb-2 sm:mb-0">
        ...
        <div className="mt-4 border-t border-warning-border/30 pt-2">
          <div className="flex justify-between text-xs text-warning">
            <span>INTERVALO:</span>
            <span className="text-warning-hover flex items-center">
              5s
          ...
          <div className="flex justify-between text-xs text-warning">
            <span>ESTADO:</span>
            <span className="text-warning-hover">ACTIVO</span>
          </div>
        </div>
      </TerminalCard>
...
function getStatusColor(tps: number) {
  ...
  if (tps < 18) return "bg-warning";
  if (tps < 19) return "bg-warning";
...
function getStatusTextColor(tps: number) {
  ...
  if (tps < 18) return "text-warning";
  if (tps < 19) return "text-warning";
