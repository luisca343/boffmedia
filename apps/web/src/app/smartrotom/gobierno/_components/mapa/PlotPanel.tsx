    ...
    <div className="animate-gt-pop-scale motion-reduce:animate-none">
      <div className="mb-3 flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={onClose}
          aria-label="Cerrar ficha de la parcela"
      ...
      <h2 className="font-gt-display text-[26px] leading-none text-gt-ink-900">Parcela #{plot.number}</h2>
      ...
      {plot.owner ? (
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="font-gt-mono text-[9px] uppercase tracking-[.12em] text-gt-ink-400">Propietario</div>
        ...
        <Sunken className="p-3.5 text-center">
          <div className="mb-1 font-gt-display text-[15px] text-gt-ink-600">Parcela vacante</div>
          <div className="font-gt-mono text-[11px] text-gt-ink-400">Sin subasta abierta</div>
        </Sunken>
      )}
      ...
      <div className="mt-4">
        ...
        {isLoading ? (
          ...
          <div className="text-[12px] italic text-gt-ink-400">Sin cambios de titularidad registrados.</div>
        ) : (
          hist.items.map((h) => (
            <div
              key={h.id}
            ...
            >
              ...
              <span className="min-w-0 flex-1 truncate text-gt-ink-700">
                {h.previousOwner?.username ?? "Municipio"} → <strong>{h.newOwner?.username ?? "—"}</strong>
              </span>
              <span className="font-gt-mono text-[10px] text-gt-ink-400">{fmtDate(h.changedAt)}</span>
            </div>
          ))
        )}
      </div>
