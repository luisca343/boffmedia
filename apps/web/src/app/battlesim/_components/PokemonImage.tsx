...
export function PokemonImage(
    ...
    if (url === "https://play.pokemonshowdown.com/sprites/gen5/0.png") {
        ...
        url = `${env.NEXT_PUBLIC_API}/smartrotom/img/sprites/Front/${pokemon?.speciesForme?.toUpperCase()}.png`;
        pixelated = true;
    }
    return (
        <div className={`w-full h-full flex items-end justify-center relative z-50 ${className}`} id={id}>
            {pokemon && Object.keys(pokemon.volatiles).includes("protect") &&
                <div className={`h-[75px] w-[100px] bg-secondary-hover opacity-30 self-center bottom-8 absolute ${side === 'p1' ? 'z-0' : 'z-50'}`} />
            }
