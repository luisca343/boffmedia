...
export function useLiveBattleManager() {
  ...
  const connect = useCallback(() => {
    ...
    registerListenersOnce('battle', [
      ...
      ['protocol', (data: { roomId: string; line: string }) => { const session = sessionsRef.current.get(data.roomId); if (session) { session.addLine(data.line); } }],
      ...
      ['battleEnd', (data: { roomId: string; winner: string; replay: string; replayId?: number }) => { const session = sessionsRef.current.get(data.roomId); if (session) { session.winner = data.winner; session.replay = data.replay; session.replayId = data.replayId ?? null; session.status = 'finished'; session.isWaitingForChoice = false; session.currentRequest = null; (session.battle as any).winner = data.winner; triggerUpdate(); } }],
