...
export function useBSXLayout(state: BattleState | null): BSXLayout {
  return useMemo(() => {
    if (!state) {
      return {
        bsxAlly: null, bsxAlly2: null, bsxFoe: null, bsxFoe2: null,
        bsxMoves: [], bsxMoves2: [], bsxBench: [],
        bsxTicks: [], bsxTimerP1: 0, bsxTimerP2: 0,
        bsxScoreP1: null, bsxScoreP2: null,
        requestType: null, isDoubles: false,
    ...
    const isDoubles = (request?.active?.length ?? 0) > 1;
