...
import { ChangelogAdmin } from "../_sections/changelog-admin";
import UnauthorizedPage from "@/components/boffmedia/ui/layout/Unauthorized";

const NAV_META: {
  labelKey: string;
  items: { id: string; labelKey: string; icon: IconName }[];
}[] = [
  {
    labelKey: "portal",
    items: [
      ...
      { id: "changelog", labelKey: "changelog", icon: "list" },
    ],
  },
  {
    labelKey: "app",
...
function AdminContent() {
  ...
  const isSuperAdmin = roles.includes(USER_ROLES.BOFF_ADMIN);
  const canManageChangelog =
    isSuperAdmin || roles.includes(USER_ROLES.BOFF_ADMIN_CONTENT);
  const rawSection = searchParams.get("section");
  const allowedSections = isSuperAdmin ? VALID_SECTIONS : ["changelog"];
  ...
  return (
    ...
    >
      ...
      {section === "changelog" && <ChangelogAdmin />}
    </AvShell>
  );
}
