...
export function MgBar({ pct, done }: { pct: number; done?: boolean }) {
  return (
    <div className="h-2 overflow-hidden bg-base-2">
      <div
        className={cn("h-full transition-[width] duration-300 ease-out", done ? "bg-ok" : "bg-accent")}
        style={{ width: `${pct}%` }}
      />
    </div>
  )
...
export function MgFormatTag({ kind }: { kind: "cbz" | "epub" }) {
  return (
    <AvPill tone={kind === "epub" ? "accent" : "default"}>{kind}</AvPill>
  )
}
