          ...
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-[var(--surface-2)] border border-[var(--border)] flex items-center justify-center overflow-hidden shrink-0">
              {g.icon ? (
                ...
                <Gamepad2 className="w-4 h-4 text-[var(--text-dim)]" />
              )}
            </div>
            <span className="font-medium text-[var(--text)]">{g.title}</span>
          </div>
        )},
        { key: "description", label: "Descripción", render: (g) => (
          <p className="text-sm text-[var(--text-muted)] truncate max-w-md">{g.description}</p>
        )},
        { key: "createdAt", label: "Fecha", render: (g) => (
          <span className="text-sm text-[var(--text-muted)]">{new Date(g.createdAt).toLocaleDateString()}</span>
        )},
      ]}
    />
  )
