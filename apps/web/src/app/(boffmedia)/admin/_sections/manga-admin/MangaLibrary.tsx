...
import { AvSectionHead, AvPanel, AvPill, AvStickyBar } from "../../_components/ui/av-kit";
import { MgCard } from "./ui/mg-kit";
    ...
    <div className={cn(
      ...
      <div className="flex items-center gap-2 px-3 py-2.5">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          {hasEpub && <AvPill tone="accent">{t("epub")}</AvPill>}
...
function MangaLibraryInner() {
  ...
  useEffect(() => {
    ...
    if (wasInLibrary && !isInLibrary) {
      try {
        sessionStorage.setItem("admin.manga.libraryScrollY", String(window.scrollY));
      } catch {}
    } else if (!wasInLibrary && isInLibrary) {
      try {
        const saved = sessionStorage.getItem("admin.manga.libraryScrollY");
        if (saved) {
  ...
  return (
    <div className="pb-32">
      ...
      {seriesSlug && (
        <>
          <div className="mb-5">
            <Button size="sm" variant="ghost" icon="back" onClick={goToLibrary}>{t("backToLibrary")}</Button>
            <AvSectionHead
              title={seriesSlug}
              actions={
                <Button
                  size="sm"
                  variant="ghost"
                  icon={refreshingLibrary ? undefined : "refresh"}
                  onClick={() => doRefresh(seriesSlug)}
