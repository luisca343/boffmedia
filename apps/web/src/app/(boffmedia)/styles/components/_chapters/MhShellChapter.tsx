    ...
    <div style={MH_VARS}>
      ...
      <Section id="mhshelldmg" kicker="Monster Hunter" title="Laboratorio de daño" lead={<>El Laboratorio cruza un arma con la tabla de hitzones de un monstruo. Cada parte muestra el índice de daño físico + elemental con la mejor resaltada.</>}>
        <Sample title="Selector arma × monstruo" code=".mh-dmgsel" col>
          <div className="flex w-full flex-wrap items-stretch gap-3">
            <div style={{ "--sh": 15 } as React.CSSProperties} className="flex flex-1 basis-[18.75rem] cursor-pointer items-center gap-3 border border-solid border-line bg-panel px-[0.875rem] py-3 text-left transition-[border-color,background] duration-[140ms] cut-corner cut-corner-edge hover:[--cut-line:var(--line-2)] [--cut-line:var(--line)] [--cut-lg:10px] hover:border-line-2 hover:bg-panel-2">
              ...
              <span className="flex min-w-0 flex-1 flex-col gap-0.5">
                <span className="font-mono text-[0.5625rem]/none font-semibold uppercase tracking-[0.1em] text-txt-dim">Arma</span>
                <span className="truncate font-display text-[1.0625rem]/[1.1] font-extrabold italic uppercase">{weapon.name}</span>
                <span className="truncate font-mono text-[0.625rem]/[1.3] font-medium text-txt-muted">
                  {weapon.typeLabel} · Atk {weapon.attack}
                </span>
              </span>
              <Icon name="edit" size={16} className="flex-none text-txt-dim" />
            </div>
            <span className="flex-none self-center font-mono text-[0.6875rem]/none font-bold uppercase tracking-[0.1em] text-txt-dim">contra</span>
            <div className="flex flex-1 basis-[18.75rem] cursor-pointer items-center gap-3 border border-solid border-line bg-panel px-[0.875rem] py-3 text-left transition-[border-color,background] duration-[140ms] cut-corner cut-corner-edge hover:[--cut-line:var(--line-2)] [--cut-line:var(--line)] [--cut-lg:10px] hover:border-line-2 hover:bg-panel-2">
              ...
              <span className="flex min-w-0 flex-1 flex-col gap-0.5">
                <span className="font-mono text-[0.5625rem]/none font-semibold uppercase tracking-[0.1em] text-txt-dim">Monstruo</span>
                <span className="truncate font-display text-[1.0625rem]/[1.1] font-extrabold italic uppercase">{MONSTER.name}</span>
                <span className="truncate font-mono text-[0.625rem]/[1.3] font-medium text-txt-muted">{MONSTER.title}</span>
              </span>
              <Icon name="edit" size={16} className="flex-none text-txt-dim" />
            </div>
          </div>
        ...
        <Sample title="Fila de daño por parte" code=".mh-dmg__row" col>
          <div className="flex w-full max-w-[28.75rem] flex-col gap-[0.4375rem]">
            <div className="border border-solid border-line border-l-2 border-l-[color:var(--mh)] bg-[var(--mh-soft)] px-3 py-2.5">
              <div className="mb-[0.4375rem] flex items-center justify-between gap-2.5">
                <span className="inline-flex items-center gap-1.5 font-body text-[0.8125rem]/[1.2] font-semibold">
                  <Icon name="star" size={12} className="text-[color:var(--mh-bright)]" />
                  Cabeza
                </span>
                <span className="font-display text-[1.0625rem]/none font-extrabold italic">20.7</span>
              </div>
              <div className="flex h-[0.5625rem] overflow-hidden border border-solid border-line bg-base-deep">
                <span className="h-full bg-[#ff7a5c]" style={{ width: "72%" }} />
                <span className="h-full bg-[color:var(--info)]" style={{ width: "18%" }} />
              </div>
              <div className="mt-[0.4375rem] flex flex-wrap gap-[0.875rem] font-mono text-[0.6875rem]/none font-semibold text-txt-muted [&_i]:not-italic [&_i]:text-txt-dim">
                <span>
                  Fís 16.2 <i>(65%)</i>
                </span>
            ...
            <div className="border border-solid border-line border-l-2 border-l-line-2 bg-base-2 px-3 py-2.5">
              <div className="mb-[0.4375rem] flex items-center justify-between gap-2.5">
                <span className="inline-flex items-center gap-1.5 font-body text-[0.8125rem]/[1.2] font-semibold">Cuello</span>
                <span className="font-display text-[1.0625rem]/none font-extrabold italic">13.1</span>
              </div>
              <div className="flex h-[0.5625rem] overflow-hidden border border-solid border-line bg-base-deep">
      ...
      <Section id="mhshellhunt" kicker="Monster Hunter" title="Lista de caza" lead={<>La Caza agrupa por monstruo los materiales que faltan para tus objetivos (builds guardadas + favoritos), con casillas de progreso sincronizadas con el Planner.</>}>
        <Sample title="Tarjeta de caza" code=".mh-huntcard" col>
          <div className="w-full max-w-[28.75rem] border border-solid border-line bg-panel cut-corner cut-corner-edge">
            <div className="flex items-center gap-[0.6875rem] border-b border-solid border-line px-[0.875rem] py-3">
              <span className="grid h-[2.125rem] w-[2.125rem] flex-none place-items-center border border-solid border-[hsl(8_60%_55%/0.4)] bg-[hsl(8_60%_55%/0.13)] text-[hsl(8_70%_62%)]">
                <Icon name="skull" size={18} />
              </span>
              <div className="min-w-0 flex-1">
                <div className="font-display text-[1rem]/none font-extrabold italic uppercase text-txt">Rathalos</div>
                <div className="mt-[3px] flex items-center gap-1.5 font-mono text-[0.6875rem]/[1.2] font-medium text-txt-muted">3 pendientes de 4</div>
              </div>
              <span className="inline-flex items-center gap-1.5 border border-solid border-line bg-panel-2 px-2 py-1 font-mono text-[0.625rem]/none font-semibold uppercase text-txt-dim">
                <Icon name="crosshair" size={11} />
                Daño
              </span>
            </div>
            <div className="flex flex-col gap-1 px-[0.875rem] py-3">
              {[
                ...
                <div key={name} className="flex items-center gap-2.5 py-1">
                  ...
                  <span className="flex-1 font-body text-[0.8125rem] text-txt">
                    {name} <span className="ml-1 font-mono text-[0.625rem] text-txt-dim">{rar}</span>
                  </span>
                  <span className="font-mono text-[0.75rem] font-bold text-txt-muted">{qty}</span>
                </div>
              ))}
            </div>
          </div>
