  ...
  return (
    <section className="pt-20 pb-48 bg-gradient-to-b from-base via-secondary-soft/30 to-layer-2 relative overflow-hidden">
      <div className="relative z-10 max-w-7xl mx-auto px-6 space-y-8">
        <div className="h-10 w-64 bg-layer-3 rounded-lg animate-pulse mx-auto" />
        <div className="h-5 w-96 bg-layer-3 rounded animate-pulse mx-auto" />
        <div className="h-64 w-full bg-layer-2/80 rounded-3xl animate-pulse" />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="h-48 bg-layer-2/60 rounded-2xl animate-pulse" />
          ))}
        </div>
      </div>
    </section>
