...
import { DesktopDownloadsService } from "@/services/api/boffmedia/desktopDownloadsService"
...
export function AppDownloadView() {
  ...
  const [rows, setRows] = React.useState<DesktopDownloadEntity[]>([])
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await DesktopDownloadsService.list()
        if (!alive) return
        if (!response.success) {
          setFailed(true)
          return
