...
export function BuildHeader({
  ...
  const renderBuildSummary = () => (
    <div className="space-y-4 mt-4">
      <div>
        <h3 className="text-sm font-black uppercase tracking-widest mb-2 text-ink"
          style={{ fontFamily: "Orbitron, sans-serif" }}>
          {t("equipment")}
        </h3>
        <div className="space-y-1.5">
          {completeData.weapon  && <SummaryRow label={t("weapon")}  value={completeData.weapon.name}  labelColor="text-red-400" />}
          {completeData.head    && <SummaryRow label={t("head")}    value={completeData.head.name}    labelColor="text-secondary-hover" />}
          {completeData.chest   && <SummaryRow label={t("chest")}   value={completeData.chest.name}   labelColor="text-warning-hover" />}
          {completeData.arms    && <SummaryRow label={t("arms")}    value={completeData.arms.name}    labelColor="text-yellow-400" />}
          {completeData.waist   && <SummaryRow label={t("waist")}   value={completeData.waist.name}   labelColor="text-secondary-hover" />}
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {skills.sort((a, b) => b.level - a.level).map((skill) => (
            ...
            >
              ...
              <span className={skill.level > skill.maxLevel ? "text-yellow-400 text-sm" : "text-warning-hover text-sm"}>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <SummaryRow label={t("defense")}  value={String(stats.defenseMin)} labelColor="text-secondary-hover" />
          <SummaryRow label={t("attack")}   value={String(stats.attack)}     labelColor="text-red-400" />
          <SummaryRow
            label={t("affinity")}
            value={`${stats.affinity >= 0 ? '+' : ''}${stats.affinity}%`}
            labelColor={stats.affinity >= 0 ? "text-warning-hover" : "text-red-400"}
          />
          {stats.element && (
            <SummaryRow label={t("element")} value={`${stats.element.type} ${stats.element.damage}`} labelColor="text-secondary-hover" />
          )}
          {stats.status && (
        ...
        <div>
          ...
          <div className="flex gap-2">
            <Input
              value={shareUrl}
              readOnly
              className="bg-layer-1/60 border-edge/60 text-ink"
            />
            <Button
              onClick={() => {
    ...
    <div className="mb-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center">
          <LuShield className="mr-2 h-6 w-6 text-warning-hover hidden sm:block" />
          <div>
            <h1
            ...
            <p className="text-ink-muted text-sm mt-1">{t("build_planner.subtitle")}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
          <Input
            ...
            className="max-w-[250px] bg-layer-1/60 border-edge/60 placeholder:text-ink-muted"
          ...
          <div className="flex space-x-2">
            <Button
              variant="outline"
              size="sm"
              className="text-warning-hover border-warning-border/50"
              onClick={handleSave}
            >
            ...
            <Button
              variant="outline"
              size="sm"
              className="text-secondary-hover border-secondary/50"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="text-ink">
                  <Share2 className="mr-1 h-4 w-4" /> {t("build_planner.share")}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent
                className="border-0 text-ink"
                style={{
        ...
        >
          <DialogHeader>
            <DialogTitle className="text-xl font-black uppercase tracking-widest"
              ...
              <span className="ml-2 text-primary-hover normal-case">{buildData.name}</span>
            </DialogTitle>
          </DialogHeader>
          {renderBuildSummary()}
          <DialogFooter>
