...
| 2026-05-02 | Shared-type migration | Added Swagger response DTO coverage for VGC meta, Champions speed, and VGC tracker sync endpoints so `pnpm run --filter api generate:shared` emits the client-facing models. `apps/web/src/services/api/boffmedia/vgcService.ts` and `apps/web/src/services/api/boffmedia/vgcTrackerService.ts` now anchor on generated shared types instead of handwritten API contracts; Limitless nullability, tracker sync DTOs, and the admin regulation upsert payload were aligned with the generated contracts. |
