    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-surface-900 border border-surface-700 rounded-xl w-full max-w-sm mx-4 shadow-2xl">
        <div className="flex items-center justify-between p-4 border-b border-surface-700">
          <h2 className="font-semibold text-surface-50">{t('exportImport.title')}</h2>
        ...
        <div className="p-4 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            {sessionId && (
              <button
                ...
                disabled={busy}
                className="flex items-center gap-2 w-full px-3 py-2.5 rounded-lg bg-surface-800 border border-surface-700 hover:border-surface-600 text-surface-200 text-sm transition-colors disabled:opacity-50"
              >
                <Download size={15} className="text-primary-400 shrink-0" />
                <span>{t('exportImport.exportSession')}</span>
                {sessionLabel && <span className="text-surface-500 truncate">— {sessionLabel}</span>}
              </button>
            )}
            <button
          ...
          >
            ...
            <div className="text-left">
              ...
              <span className="text-surface-500 text-xs">{t('exportImport.importHint')}</span>
            </div>
          </button>
