...
function HPMini({ poke, apiEntry, align = 'left', useChampions = false }: { poke: CalcPokemon; apiEntry?: VgcPokemon; align?: 'left' | 'right'; useChampions?: boolean }) {
  ...
  const textColor = pct > 50 ? 'text-success-hover' : pct > 25 ? 'text-warning-hover' : 'text-danger-hover'
  ...
  return (
    <div className={`flex flex-col gap-0.5 min-w-0 ${align === 'right' ? 'items-end' : ''}`}>
      <div className={`flex items-center gap-1.5 w-full ${align === 'right' ? 'justify-end' : ''}`}>
        <span className="font-mono text-[11px] text-ink-muted">{cur}/{maxHP}</span>
        <div className="h-2 bg-layer-1/80 rounded-full overflow-hidden border border-edge/40 w-full max-w-[110px] min-w-[84px]">
...
export function MoveStripPokemonCard({ poke, apiEntry, align = 'left', useChampions = false }: MoveStripPokemonCardProps) {
  ...
  return (
    <div className={`rounded-md border border-edge/50 bg-layer-2/55 px-2 py-1.5 ${reverse ? 'text-right' : ''}`}>
      <div className={`flex items-center gap-2 ${reverse ? 'flex-row-reverse' : ''}`}>
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[13px] font-black text-ink truncate">{poke.name}</div>
          <div className="text-[10px] text-ink-muted">{t('level', { level: poke.level })}</div>
        </div>
      </div>
      <div className="mt-1">
