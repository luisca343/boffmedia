...
export class DesktopAuthService {
  static describe(userCode: string) {
    return apiAuthedAutoGET<DeviceRequest>(
      `/desktop/auth/device?userCode=${encodeURIComponent(userCode)}`,
    );
  }

  static approve(userCode: string) {
    return apiAuthedAutoPOST<{ success: true }>('/desktop/auth/device/approve', {
      userCode,
    });
  }

  static deny(userCode: string) {
    return apiAuthedAutoPOST<{ success: true }>('/desktop/auth/device/deny', {
      userCode,
    });
  }
}
