  ...
  return (
    ...
    >
      <div className="min-w-0">
        {eyebrow != null && (
          <span className="mb-2.5 flex w-max items-center gap-1.5 whitespace-nowrap rounded-mw-pill border border-mw-accent/30 bg-mw-accent/[.14] px-2.5 py-1 font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-mw-accent">
            {eyebrow}
          </span>
        )}
        <h2 className="m-0 font-mw-display text-[26px] font-extrabold leading-[1.2] tracking-[-0.01em] text-mw-fg">
          {title}
        </h2>
        {subtitle != null && <p className="mt-1.5 text-sm text-mw-fg-mute">{subtitle}</p>}
        {rule && <div className="mt-3 h-[3px] w-[120px] rounded-full bg-mw-accent" />}
      </div>
