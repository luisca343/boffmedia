...
export function IconButton({ icon, label, dot = false, bordered = false, active = false, href, onClick, size = 18, className = "", children, ...rest }: IconButtonProps) {
  const cls = [
    ...
    bordered ? "border border-edge-strong" : "border border-transparent",
    active ? "text-[var(--orange-500)] bg-[color-mix(in_srgb,var(--orange-500)_10%,transparent)]" : "text-ink-muted bg-transparent hover:text-ink hover:bg-[color-mix(in_srgb,var(--text)_8%,transparent)]",
