...
const TONE_CLASSES: Record<string, string> = {
  ...
  neutral: "text-ink border-edge-strong bg-layer-3",
}
      ...
      {options.map((o) => {
        ...
        return onChange ? (
          <button
            ...
            type="button"
            className={cn(
              "text-xs font-semibold px-[0.6rem] py-[0.32rem] rounded-[var(--radius)] [border-width:var(--hairline)] border-solid [border-color:var(--border)] text-ink-muted bg-transparent cursor-pointer",
          ...
          <span
            key={o.value}
            className={cn(
              "text-xs font-semibold px-[0.6rem] py-[0.32rem] rounded-[var(--radius)] [border-width:var(--hairline)] border-solid [border-color:var(--border)] text-ink-muted bg-transparent",
              size === "sm" && "text-[11px] px-[0.5rem] py-[0.22rem]",
              on && TONE_CLASSES[tone],
            )}
          >
