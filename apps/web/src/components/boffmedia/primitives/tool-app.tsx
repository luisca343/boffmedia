...
export function ToolApp({ toolbar, subbar, children, className, bare }: ToolAppProps) {
  return (
    <div className={cn("flex flex-col h-[calc(100dvh-68px)] min-h-[520px] bg-[var(--bg)]", bare && "", className)}>
      {toolbar && <div className="relative z-[20] shrink-0 border-b-[var(--hairline)] border-b-solid border-b-[var(--border)] bg-[color-mix(in_srgb,var(--surface)_86%,transparent)] backdrop-blur-[8px]">{toolbar}</div>}
