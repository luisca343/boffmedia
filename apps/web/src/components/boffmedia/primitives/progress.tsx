...
export function BoffProgress({ value = 0, tone = "accent", label, className }: BoffProgressProps) {
  return (
    <div className={cn("flex flex-col gap-[7px]", className)}>
      ...
      <div className={cn("h-2 rounded-[999px] bg-layer-3 overflow-hidden", "data-[direction=hud]:rounded-sm")}>
        <div
...
export function BoffRing({ value = 0, size = 64, tone = "orange", className }: BoffRingProps) {
  const r = (size - 8) / 2
  const c = 2 * Math.PI * r
  const color = tone === "accent" ? "var(--secondary-hover)" : tone === "emerald" ? "var(--emerald-400)" : "var(--orange-500)"
  return (
    <span className={cn("relative inline-grid place-items-center", className)} style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="var(--layer-3)" strokeWidth="6" />
