...
export function BSXScorePlate({ name, rating, team, right, av }: BSXScorePlateProps) {
  return (
    <div
      className={`flex items-center gap-[.6rem] p-[.5rem_.7rem] min-w-0 rounded-[var(--radius-lg)]`}
      style={{
    ...
    >
      <span
        className="w-[36px] h-[36px] rounded-[10px] shrink-0 grid place-items-center font-display font-extrabold text-[.8rem] text-white"
      ...
      <div className="flex flex-col min-w-0">
        ...
        <span className="font-mono text-[.58rem] tracking-[.08em] whitespace-nowrap" style={{ color: "var(--text-dim)" }}>{rating}</span>
      </div>
      <div className={`flex gap-[.3rem] ${right ? "" : "mx-auto mr-[.1rem]"}`} style={right ? { marginInline: ".1rem auto" } : undefined}>
        {team.map((m, i) => {
          const isKo = m.fnt
          const isLow = !isKo && m.hp < 35
          return (
            <span
              key={i}
