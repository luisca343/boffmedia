...
export function DkBracket<M>({ rounds, renderMatch }: { rounds: DkBracketRound<M>[]; renderMatch: (m: M, ri: number, mi: number) => React.ReactNode }) {
  ...
  return (
    <div className="flex items-stretch gap-[30px] overflow-x-auto pb-2.5">
      {rounds.map((rd, ri) => (
        <div key={ri} className="flex w-[250px] flex-none flex-col">
          <div className="pb-2.5 pl-0.5 font-mono text-[10px]/none font-bold uppercase tracking-[0.16em] text-txt-dim">{rd.phase}</div>
