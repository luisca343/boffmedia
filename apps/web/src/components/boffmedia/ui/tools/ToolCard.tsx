...
export function ToolCard({ tool, variant = "senal" }: { tool: ToolCardData; variant?: "senal" | "fila" }) {
  ...
  if (variant === "fila") {
    return (
      <Link
        ...
        className={cn(
          "group relative flex w-full items-center gap-[15px] border border-solid border-line bg-panel px-[18px] py-[15px] text-left no-underline",
  ...
  return (
    <Link
      ...
      style={hueStyle(tool.hueColor)}
      className={cn(
        "group relative flex h-full flex-col border border-solid border-line bg-panel px-5 pb-[18px] pt-5 text-left no-underline",
        "transition-[border-color,background,transform,box-shadow] duration-[140ms]",
        SENAL_CLIP,
        soon
          ? "cursor-default"
          : CARD_HOVER_LINE + " hover:-translate-y-[3px] hover:bg-panel-2 hover:border-[color-mix(in_srgb,var(--ghue)_45%,var(--line))] hover:shadow-[0_16px_34px_rgba(0,0,0,0.28)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[color-mix(in_srgb,var(--ghue)_75%,var(--text))]",
      )}
    >
      ...
      <div className="mb-4 flex items-center gap-[11px]">
        <span className={cn(ICON_BOX, "h-[38px] w-[38px]", soon && "border-line-2 bg-transparent text-txt-dim [--cut-line:var(--line-2)]")}>
          <Icon name={tool.icon} size={20} />
        </span>
