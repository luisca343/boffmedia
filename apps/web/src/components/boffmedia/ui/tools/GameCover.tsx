...
export function GameCover({ game, mini = false }: { game: GameCoverData; mini?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 top-0 z-[2] flex items-center gap-3 px-[1.125rem] py-4">
        <GameLogo label={game.logoLabel} hueColor={game.hueColor} size="sm" />
        <span className="ml-auto border border-solid border-line bg-[color-mix(in_srgb,var(--bg)_72%,transparent)] px-[0.5625rem] py-1.5 font-mono text-[0.625rem]/none font-semibold uppercase tracking-[0.12em] text-txt backdrop-blur-[4px]">
          {game.toolCount} {t("toolCount", { count: game.toolCount })}
        </span>
      </div>

      <div className="relative z-[2] px-5 pb-5">
        <h3 className={cn("leading-[0.95]", mini ? "text-[1.75rem]" : "text-[clamp(1.75rem,2.6vw,2.375rem)]")}>{game.name}</h3>
        {!mini && <p className="mt-[0.4375rem] max-w-[34ch] text-pretty text-[0.84375rem] text-txt-muted">{game.tagline}</p>}
        <span className={cn("flex items-center gap-2 font-mono text-[0.6875rem]/none font-semibold uppercase tracking-[0.1em] text-txt-muted", mini ? "mt-2.5" : "mt-3.5")}>
          {t("enter")}
