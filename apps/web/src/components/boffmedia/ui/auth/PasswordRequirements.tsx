...
export function PasswordRequirements({ value }: { value: string }) {
  ...
  return (
    <ul className="flex flex-col gap-[5px] font-body text-[12.5px]/[1.3] not-italic normal-case">
      {PASSWORD_RULES.map((rule) => {
        ...
        return (
          <li key={rule.id} className={`flex items-center gap-2 ${ok ? "text-success" : "text-txt-dim"}`}>
            <span
              aria-hidden
              className={`grid h-[15px] w-[15px] flex-none place-items-center rounded-full text-[9px] font-bold ${
      ...
      {PASSWORD_WARNINGS.map((rule) => {
        ...
        return (
          <li key={rule.id} className="flex items-center gap-2 text-danger">
            <span
              aria-hidden
              className="grid h-[15px] w-[15px] flex-none place-items-center rounded-full bg-danger/15 text-[9px] font-bold"
            >
              ✕
            </span>
            {t(`pw.${rule.id}`)}
