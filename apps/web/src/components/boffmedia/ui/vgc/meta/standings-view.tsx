              ...
              <>
                <tr
                  ...
                  className={cn(
                    ...
                    expanded
                      ? "bg-secondary-soft"
                      : "hover:bg-[color-mix(in_srgb,var(--layer-3)_45%,transparent)]",
                  )}
                  onClick={() => handleExpand(p.slug)}
                >
                  <td className="font-mono text-ink-dim text-right py-3 px-4 border-b border-edge text-sm">#{p.placing}</td>
                  <td className="py-3 px-4 border-b border-edge">
                    <span className="inline-flex items-center gap-1.5 font-semibold text-ink text-sm">
                      <Icon name="chevron" size={13} className="text-ink-dim" style={{ transform: expanded ? "none" : "rotate(-90deg)" }} />
                      {p.name}
                    </span>
                  </td>
                  <td className="py-3 px-4 border-b border-edge">
                    <span className="inline-flex font-mono text-xs px-2 py-[0.15rem] rounded-[var(--radius)] border border-edge bg-layer-2 text-ink-muted">
                      {p.record}
                    </span>
                  </td>
                  <td className="py-3 px-4 border-b border-edge">
                ...
                {expanded && (
                  <tr key={`${p.slug}-detail`}>
                    <td colSpan={4} className="bg-[color-mix(in_srgb,var(--layer-2)_45%,transparent)] p-4">
                      {isLoading ? (
                        <div className="flex items-center justify-center py-6 text-xs text-ink-dim">
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-[repeat(auto-fit,minmax(140px,1fr))] gap-3">
                            {player.team.map((s, i) => (
                              <div key={i} className="flex flex-col items-center text-center p-3 border border-edge rounded-[var(--radius)] bg-[color-mix(in_srgb,var(--layer-2)_50%,transparent)]">
                                <img src={spriteUrl(s.name)} alt={s.name} width={48} height={48} className="object-contain" onError={handleSpriteError} />
                                <p className="text-xs font-bold text-ink leading-tight mt-1">{s.name}</p>
                                <p className="text-[11px] text-ink-dim">{s.item}</p>
                                <span
                                ...
                                <ul className="list-none m-1 p-0 flex flex-col gap-px">
                                  {s.moves.map((m) => (
                                    <li key={m} className="text-[11px] text-ink-muted">{m}</li>
                                  ))}
                                </ul>
                              </div>
                            ))}
                        ...
                        <p className="text-xs text-ink-dim text-center py-4">Equipo no disponible para este jugador.</p>
                      )}
                    </td>
                  </tr>
                )}
