...
export function TvHero({ lvl, density }: { lvl: number; density: number }) {
  ...
  return (
    <section className="relative z-[1] grid min-h-screen items-center overflow-hidden pb-24 pt-[7.375rem]" id="tv-hero">
      {lvl >= 2 && <FxParticles density={density} />}
      <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden" aria-hidden="true">
        <i
          data-pglow
          className="absolute left-[-12.5rem] top-[-10rem] h-[47.5rem] w-[47.5rem] rounded-full blur-[110px] will-change-transform [background:radial-gradient(circle,rgba(255,92,10,0.16),transparent_66%)] [[data-theme=light]_&]:opacity-55"
        />
        <i
          data-pglow
      ...
      <div className="wrap relative z-[2] grid grid-cols-[1.08fr_0.92fr] items-center gap-[3.125rem] px-5 max-[980px]:grid-cols-1 max-[980px]:gap-[1.875rem] min-[640px]:px-10">
        <div>
          ...
          <p
            ...
            className="mb-0 mt-6 max-w-[31.25rem] font-body text-[1.0625rem] font-normal leading-[1.65] text-txt-muted"
          >
            {t("lead")}
          </p>
          ...
          <div data-reveal style={{ ["--i"]: 3 } as React.CSSProperties} className="mt-10 flex gap-3.5 max-[820px]:flex-wrap">
            {hud.map((h) => (
              <div
                key={h.k}
                className="relative min-w-[9.375rem] border border-solid border-line px-4 pb-[0.8125rem] pt-3.5 backdrop-blur-[6px] [background:rgba(10,12,16,0.6)] cut-tag cut-tag-edge [--cut-tag:9px] before:absolute before:bottom-0 before:left-0 before:top-0 before:w-[3px] before:bg-accent before:content-[''] [[data-theme=light]_&]:[background:rgba(255,255,255,0.6)]"
              >
                <span className="inline-flex items-center gap-1.5 font-mono text-[0.625rem] font-semibold uppercase leading-none tracking-[0.12em] text-txt-dim">
                ...
                <span className="mb-[3px] mt-2 block font-display text-[1.875rem] font-extrabold leading-none text-txt tabular-nums">
                  <CountUp value={h.big} />
                  {h.suf && <b>{h.suf}</b>}
                </span>
                <span className="block font-body text-[0.75rem] font-normal leading-[1.3] text-txt-muted">{h.sub}</span>
              </div>
            ))}
          </div>
          )}
        ...
        <div
          className="relative grid min-h-[31.25rem] place-items-center max-[980px]:order-first max-[980px]:min-h-[22.5rem]"
          ref={artRef}
          data-reveal="scale"
          style={{ ["--i"]: 2 } as React.CSSProperties}
        >
          <div
            className="absolute z-0 h-[28.75rem] w-[28.75rem] rounded-full blur-[60px] [background:radial-gradient(circle,rgba(255,92,10,0.32),transparent_66%)]"
            aria-hidden="true"
          />
          <i className={cn(BEAMS, "left-[calc(50%_-_360px)] top-[calc(50%_-_360px)] h-[45rem] w-[45rem]")} aria-hidden="true" />
          <i
      ...
      <button
        ...
        className="absolute bottom-[-2px] left-1/2 z-[4] inline-flex -translate-x-1/2 cursor-pointer flex-col items-center gap-3.5 font-mono text-[0.625rem] font-semibold uppercase leading-none tracking-[0.2em] text-txt-muted transition-colors duration-[140ms] hover:text-txt max-[820px]:left-[1.125rem] max-[820px]:translate-x-0 max-[820px]:items-start"
      >
        <span>{t("scrollCta")}</span>
        <i
          aria-hidden="true"
          className="relative block h-[7.5rem] w-[3px] shadow-[0_0_18px_rgba(255,92,10,0.65)] [background:linear-gradient(rgba(255,92,10,0),var(--accent)_55%,var(--accent))] before:absolute before:bottom-[-3px] before:left-1/2 before:h-[1.875rem] before:w-[1.875rem] before:-translate-x-1/2 before:rounded-full before:blur-[2px] before:content-[''] before:[background:radial-gradient(circle,rgba(255,92,10,0.6),transparent_70%)] after:absolute after:left-[-1.5px] after:top-0 after:h-4 after:w-1.5 after:rounded-[3px] after:bg-white after:shadow-[0_0_14px_var(--accent)] after:content-[''] after:animate-[tv-drop_1.9s_ease-in-out_infinite] [.no-motion_&]:after:animate-none"
        />
      </button>
    </section>
  )
