          ...
          >
            <div className={cn("relative overflow-hidden bg-base-2", i === 0 ? "aspect-[16/7] max-[520px]:aspect-[16/9]" : "aspect-[4/3]")}>
              {g.img ? (
                ...
                <div className="relative grid h-full w-full min-h-[7.5rem] place-items-center outline-1 outline-dashed outline-line-2 [outline-offset:-6px] [background:repeating-linear-gradient(-45deg,var(--stripe)_0_10px,transparent_10px_20px)]">
                  <span className="px-3.5 text-center font-mono text-[0.75rem]/[1.5] font-medium text-txt-muted">{t(`${gameKeys[i]}.n`)}</span>
                </div>
              )}
              <span
              ...
              />
              <span
                className="absolute right-2.5 top-2 z-[2] font-display text-[1.5rem] font-extrabold italic leading-[0.8] text-transparent [-webkit-text-stroke:1.4px_rgba(255,255,255,0.5)]"
                aria-hidden="true"
              >
                {String(i + 1).padStart(2, "0")}
              </span>
              <div className="absolute bottom-[0.8125rem] left-[0.9375rem] right-[0.9375rem] z-[2]">
                <span className="mb-[0.5625rem] inline-block bg-[rgba(var(--zr),var(--zg),var(--zb),0.92)] px-[0.5625rem] py-[0.3125rem] font-mono text-[0.59375rem] font-semibold uppercase leading-none tracking-[0.1em] text-white transition-[background] duration-[260ms] ease-linear">
                  {t(`${gameKeys[i]}.tag`)}
                </span>
            ...
            <div className="flex flex-1 flex-col gap-2.5 px-4 pb-4 pt-3.5">
              <p className="font-body text-[0.8125rem] font-normal leading-[1.5] text-txt-muted">{t(`${gameKeys[i]}.d`)}</p>
              <span className="mt-auto inline-flex items-center gap-1.5 font-mono text-[0.6875rem] font-semibold uppercase leading-none tracking-[0.06em] text-txt-muted transition-[color,gap] duration-[140ms] group-hover/game:gap-2.5 group-hover/game:text-[rgba(var(--zr),var(--zg),var(--zb),1)]">
