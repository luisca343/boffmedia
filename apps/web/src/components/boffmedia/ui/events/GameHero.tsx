...
export function GameHero({
  ...
  return (
    ...
    >
      ...
      <Icon name="gamepad" size={340} className="pointer-events-none absolute right-[-1.875rem] top-[-1.25rem] z-[1] text-[var(--ghue)] opacity-[0.13]" />

      <div className="relative z-[2] max-w-[62.5rem] px-[clamp(1.375rem,3.2vw,2.5rem)] pb-[1.625rem] pt-10">
        <div className="mb-4 flex flex-wrap items-center gap-3.5">
          <span className="grid h-[3.375rem] w-[3.375rem] flex-none place-items-center border-[1.5px] border-solid border-[color-mix(in_srgb,var(--ghue)_55%,var(--line-2))] bg-[color-mix(in_srgb,var(--ghue)_14%,var(--bg))] text-[var(--ghue)] cut-seal cut-seal-edge [--cut-w:1.5px] [--cut-line:color-mix(in_srgb,var(--ghue)_55%,var(--line-2))] [--cut:7px]">
            <Icon name="gamepad" size={27} />
          </span>
          {/* [deferred] short code — omitted until the API has it */}
          {game.short && (
            <span className="font-mono text-[0.6875rem] font-semibold uppercase leading-none tracking-[0.16em] text-txt-muted">{game.short}</span>
          )}
          <Badge tone={active ? "ok" : "default"}>{active ? t("active") : t("inactive")}</Badge>
        </div>
        <h1 className="text-[clamp(2.75rem,5.4vw,5.125rem)]/[0.9]">{game.title}</h1>
        {game.description && (
          <p className="mt-3.5 max-w-[66ch] font-body text-[1.0625rem]/[1.55] text-txt-muted text-pretty">{game.description}</p>
        )}
        {children && <div className="mt-[1.375rem] flex flex-wrap gap-3">{children}</div>}
      </div>

      <div className="relative z-[3] flex flex-wrap items-center gap-x-6 gap-y-3 border-t border-solid border-[color-mix(in_srgb,var(--ghue)_30%,var(--line))] bg-[color-mix(in_srgb,var(--bg)_80%,transparent)] px-[clamp(1.375rem,3.2vw,2.5rem)] py-[0.8125rem] backdrop-blur-[6px]">
        <span className="inline-flex items-center gap-2 whitespace-nowrap font-mono text-[0.6875rem]/none font-semibold uppercase tracking-[0.09em] text-txt-muted [&_b]:font-bold [&_b]:text-txt">
        ...
        {game.createdAt && (
          <span className="ml-auto inline-flex items-center gap-2 whitespace-nowrap font-mono text-[0.6875rem]/none font-medium uppercase tracking-[0.09em] text-txt-dim">
