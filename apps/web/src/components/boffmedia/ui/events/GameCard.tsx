  ...
  return (
    ...
    >
      <div className="relative aspect-[16/8] overflow-hidden border-b-2 border-[color-mix(in_srgb,var(--ghue)_70%,transparent)] bg-base-2">
        ...
        />
        <Icon name="gamepad" size={128} className="pointer-events-none absolute right-[-1.125rem] top-1/2 z-[1] -translate-y-1/2 text-[var(--ghue)] opacity-[0.14]" />
        ...
        <div className="absolute inset-x-0 top-0 z-[3] flex items-center gap-2.5 p-3.5">
          ...
          {game.short && (
            <span className="font-mono text-[0.625rem] font-semibold uppercase leading-none tracking-[0.14em] text-txt">{game.short}</span>
          )}
      ...
      <div className="flex flex-1 flex-col p-5">
        <h3 className="text-[clamp(1.5rem,2.2vw,1.875rem)]/[0.98] text-txt">{game.title}</h3>
        {game.description && (
          <p className="mt-2 line-clamp-3 font-body text-[0.84375rem]/[1.5] text-txt-muted text-pretty">{game.description}</p>
        )}
        <div className="mt-auto flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-dashed border-line pt-3.5">
          ...
          {game.events != null && (
            <span className="inline-flex items-center gap-1.5 font-mono text-[0.6875rem]/none uppercase tracking-[0.05em] text-txt-muted [&_b]:font-bold [&_b]:text-txt">
          ...
          {game.createdAt && (
            <span className="inline-flex items-center gap-1.5 font-mono text-[0.6875rem]/none uppercase tracking-[0.05em] text-txt-muted">
