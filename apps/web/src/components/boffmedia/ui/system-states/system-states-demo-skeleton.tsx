...
function SkelCard() {
  return (
    <div className="border border-[var(--border)] rounded-[var(--radius-lg)] bg-[var(--surface-2)] p-4 flex flex-col gap-3">
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(190px,1fr))] gap-4">
        {loading
          ...
          : Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="border border-[var(--border)] rounded-[var(--radius-lg)] bg-[var(--card-bg)] p-4 flex flex-col gap-3">
                <div className="h-[84px] rounded-[var(--radius)] bg-[var(--accent-soft)] grid place-items-center text-[var(--accent-bright)]">
                  <Icon name={["sword", "calc", "tree", "cards"][i]} size={26} />
                </div>
                <div className="flex items-center gap-[0.7rem]">
                  <span className="w-[34px] h-[34px] rounded-full bg-[var(--surface-3)] grid place-items-center shrink-0">
                    <Icon name="gamepad" size={16} />
                  </span>
                  <div className="min-w-0">
                    <div className="font-bold text-[length:var(--t-sm)]">{["Battlesim", "Calc VGC", "Árbol MH", "Wonder Mail"][i]}</div>
                    <div className="text-[var(--text-dim)] text-[length:var(--t-xs)]">Herramienta</div>
                  </div>
                </div>
                <p className="text-[var(--text-muted)] text-[length:var(--t-xs)] m-0">Lista para usar — datos cargados.</p>
              </div>
            ))}
      </div>
    </div>
