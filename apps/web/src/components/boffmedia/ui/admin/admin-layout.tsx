...
export function AdminLayout({ nav, section, onNavigate, children, loading }: AdminLayoutProps) {
  ...
  return (
    <div className="flex min-h-screen bg-base">
      {/* Desktop sidebar */}
      <aside className="hidden md:flex md:w-56 lg:w-60 shrink-0 flex-col border-r border-edge bg-layer-1 py-5 px-3 gap-0.5">
        <div className="px-3 mb-5 flex items-center gap-2.5">
          ...
          <span className="text-sm font-bold text-ink tracking-tight">Admin</span>
        </div>

        {nav.map((group) => (
          <div key={group.label} className="mb-3">
            <p className="px-3 mb-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-ink-dim">
              {group.label}
            </p>
              ...
              <button
                ...
                className={cn(
                  ...
                  section === id
                    ...
                    : "text-ink-muted hover:bg-[color-mix(in_srgb,var(--text)_6%,transparent)] hover:text-ink"
                )}
              >
                <Icon className={cn("w-4 h-4 shrink-0", section === id ? "text-[var(--orange-500)]" : "text-ink-dim")} />
                {label}
              </button>
            ))}
          </div>
      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        {/* Top bar */}
        <div className="sticky top-0 z-10 bg-[color-mix(in_srgb,var(--layer-1)_86%,transparent)] backdrop-blur-[8px] border-b border-edge px-4 py-3">
          {/* Mobile nav */}
          <div className="md:hidden overflow-x-auto scrollbar-thin scrollbar-thumb-[var(--layer-3)] scrollbar-track-transparent">
            <div className="inline-flex min-w-max items-center gap-1 rounded-xl border border-edge-strong bg-layer-2 p-1">
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-ink">{activeItem?.label ?? "Admin"}</span>
          </div>
        </div>
