...
export function LzReleaseCard({ game, wished, onWish, onOpen, dense = false, showDate = false }: { game: LzRelease; wished?: boolean; onWish?: (id: LzRelease["id"]) => void; onOpen?: (g: LzRelease) => void; dense?: boolean; showDate?: boolean }) {
  ...
  return (
    ...
    >
      <LzCover genre={game.genre} size={dense ? 18 : 24} className={cn(dense ? "w-[44px]" : "w-[52px]", "group-hover:border-line-2")} />
      <div className="flex min-w-0 flex-1 flex-col justify-center gap-1.5">
        <h4 className={cn("line-clamp-2", dense ? "text-[13.5px]/[1.2]" : "text-[15px]/[1.2]")}>{game.title}</h4>
        <LzVersList platforms={game.platforms} max={max} />
        {showDate && (
          <div className="inline-flex items-center gap-[5px] font-mono text-[10px]/none font-semibold uppercase tracking-[0.06em] text-txt-dim">
...
export function LzBannerCard({ game, popular, wished, onWish, onOpen }: { game: LzRelease; popular?: boolean; wished?: boolean; onWish?: (id: LzRelease["id"]) => void; onOpen?: (g: LzRelease) => void }) {
  ...
  return (
    <article
      ...
      role="button"
      onKeyDown={(e) => onCardKey(e, open)}
      className={cn("group relative block min-h-[300px] cursor-pointer overflow-hidden border border-solid border-line bg-panel outline-none transition-[border-color] duration-[140ms] hover:border-line-2 focus-visible:[box-shadow:inset_0_0_0_1px_var(--accent-line)]", isPop && "border-l-4 border-l-accent")}
    >
      <div aria-hidden className="absolute inset-0 grid place-items-center overflow-hidden [background:repeating-linear-gradient(135deg,var(--bg-2)_0_10px,var(--panel-2)_10px_20px)]">
        ...
        <span className="absolute right-3 top-[11px] z-[2] font-mono text-[8px]/none font-semibold uppercase tracking-[0.14em] text-[color:color-mix(in_oklab,var(--text)_52%,transparent)]">key art · {game.title}</span>
      </div>
      <div className="relative z-[2] flex h-full w-[min(66%,300px)] flex-col gap-[11px] p-[15px]">
        ...
        <span className="mt-auto flex min-w-0 flex-col gap-2.5">
          <h3 className="line-clamp-2 text-[22px]/[0.98] tracking-[-0.01em] text-txt [text-shadow:0_2px_16px_rgba(0,0,0,0.55)]">{game.title}</h3>
          ...
          <span className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={cn("group/f inline-flex h-[30px] items-center gap-1.5 border border-solid px-2.5 font-mono text-[11px]/none font-bold tracking-[0.02em] transition-[color,border-color,background] duration-[140ms]", wished ? "border-[color-mix(in_oklab,var(--warn)_55%,transparent)] bg-warn-soft text-warn" : "border-line-2 bg-panel-2 text-txt-muted hover:border-[color-mix(in_oklab,var(--warn)_55%,transparent)] hover:text-txt")}
            >
              <Icon name="bell" size={13} className={cn(wished ? "fill-current text-warn" : "text-txt-dim group-hover/f:text-warn")} />
              {followers}
            </button>
            <span className="inline-flex items-center gap-1.5 font-mono text-[11px]/none font-bold uppercase tracking-[0.04em] text-[color:color-mix(in_oklab,var(--text)_80%,transparent)]">
...
export function LzPosterCard({ game, onOpen }: { game: LzRelease; onOpen?: (g: LzRelease) => void }) {
  const open = () => onOpen && onOpen(game)
  return (
    <article onClick={open} tabIndex={0} role="button" onKeyDown={(e) => onCardKey(e, open)} className="group flex cursor-pointer flex-col gap-[9px] outline-none">
      ...
      <div className="flex min-w-0 flex-col gap-1.5">
        <h4 className="line-clamp-2 text-[14.5px]/[1.24] text-txt group-hover:text-accent" title={game.title}>
          {game.title}
        </h4>
        <LzVersList platforms={game.platforms} max={3} icon />
      </div>
