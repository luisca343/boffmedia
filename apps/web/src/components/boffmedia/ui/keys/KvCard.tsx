    ...
    >
      <div className="relative aspect-[460/200] border-b border-solid border-line">
        ...
        <div className="absolute inset-x-2.5 top-2.5 z-[2] flex items-start justify-between gap-2">
          ...
          <span className="inline-flex items-center gap-[0.3125rem] border border-solid border-line-2 bg-[color-mix(in_srgb,var(--bg)_82%,transparent)] px-2 py-[0.3125rem] font-display text-[0.8125rem]/none font-extrabold italic backdrop-blur-[4px]" style={{ color: kvReviewColor(review) }}>
            <Icon name="star" size={12} />
            {review}%
          </span>
        </div>
        {item.price.discount > 0 && <span className="absolute bottom-2.5 left-2.5 z-[2] border border-solid border-[color-mix(in_srgb,var(--ok)_45%,transparent)] bg-ok-soft px-2 py-[0.3125rem] font-display text-[0.8125rem]/none font-extrabold italic text-ok backdrop-blur-[4px]">-{item.price.discount}%</span>}
        {!item.given && item.stock > 1 && (
          <span className="absolute bottom-2.5 right-2.5 z-[2] inline-flex items-center gap-1.5 border border-solid border-line-2 bg-[color-mix(in_srgb,var(--bg)_82%,transparent)] px-2 py-[0.3125rem] font-mono text-[0.625rem]/none font-bold uppercase tracking-[0.06em] text-txt backdrop-blur-[4px]">
      ...
      <div className="flex flex-1 flex-col gap-2.5 px-4 pb-4 pt-3.5">
        <div className="flex items-baseline justify-between gap-3">
          <span className="min-w-0 truncate font-display text-[1.1875rem]/[1.1] font-bold">{item.name}</span>
          <span className="flex-none font-display text-[1.125rem]/none font-extrabold italic text-accent">{item.price.final}</span>
        </div>
