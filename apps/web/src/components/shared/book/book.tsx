...
export function BookLink({children, book, page, className}: {children: React.ReactNode, book: any, page: number, className?: string}){
    return (
        <button className={`hover:cursor-pointer hover:underline hover:text-ink-dim transition-colors duration-200 font-vinque ${className}`} onClick={(e) => turnPage(book, page, e)}>
            {children}
        </button>
    )
}
