...
import { errorCode, errorDetail } from "@/lib/schematic/errors";
import { parseBlockState } from "@/lib/schematic/normalizer";
import type { EnvRole } from "@/lib/schematic/state";
import type { ExportFormat, RegistryHandle, RuleSetMeta } from "@/lib/schematic/types";
  ...
  const exportSchematic = useCallback(
    async (format: ExportFormat) => {
      ...
      try {
        ...
        triggerDownload(blob, blob.type === "application/zip" ? partsArchiveName(name) : name);
      } catch (err) {
        store.getState().setError(errorDetail(err), errorCode(err));
      } finally {
