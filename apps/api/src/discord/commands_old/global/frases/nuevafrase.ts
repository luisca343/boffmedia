...
const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option.setName('frase').setDescription('Frase a añadir').setRequired(true),
  )
  .addStringOption((option) =>
    option.setName('comentario').setDescription('Comentario adicional'),
  );
...
async function execute(interaction, service: CommandsService) {
  ...
  const response = await service.addQuote(
    interaction.guildId,
    user,
    quote,
    comment,
