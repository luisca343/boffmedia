...
const data = new SlashCommandBuilder()
  ...
  .addIntegerOption((option) =>
    option.setName('page').setDescription('Página a mostrar'),
  );
...
async function createEmbed(service: CommandsService, guildId, userId, page) {
  const { totalPages, frases } = await service.getFrases(
    guildId,
    userId,
    page,
    MAX_QUOTES,
  ...
  const embed = new EmbedBuilder()
    ...
    .setAuthor({
      name: `${userId ? params?.discordName : 'Ficus Quotes'}`,
...
async function handleButton(
  ...
  const { embed, row } = await createEmbed(
    service,
    interaction.guildId,
    userId,
    page,
