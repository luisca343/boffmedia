...
function fmtList(
  arr: Array<{ name: string; percent: number }>,
  limit = 5,
): string {
  if (!arr?.length) return '—';
  return arr
    .slice(0, limit)
    .map((e) => `**${e.name}** ${pct(e.percent)}`)
    .join('\n');
}

function fmtSpreads(
  arr: Array<{ nature: string; spread: string; percent: number }>,
  limit = 5,
): string {
  if (!arr?.length) return '—';
  return arr
    .slice(0, limit)
    .map((e) => `**${e.nature}** \`${e.spread}\` ${pct(e.percent)}`)
    .join('\n');
}

function fmtTeammates(
  arr: Array<{ name: string; percent: number }>,
  limit = 3,
): string {
  if (!arr?.length) return '—';
...
export function buildDetailPages(
  ...
  const p1 = base()
    ...
    .addFields(
      ...
      ...(d.topItem
        ? [{ name: 'Top Item', value: d.topItem, inline: true }]
        : []),
      ...(d.topMove
        ? [{ name: 'Top Move', value: d.topMove, inline: true }]
        : []),
      ...(d.topTeraType
        ? [{ name: 'Top Tera Type', value: d.topTeraType, inline: true }]
        : []),
      ...(d.teammates?.length
        ? [
            {
              name: 'Top Teammates',
              value: fmtTeammates(d.teammates, 3),
              inline: false,
            },
          ]
        : []),
  ...
  const p3 = base()
    ...
    .addFields({ name: 'Moves', value: fmtList(d.moves, 8), inline: false });
