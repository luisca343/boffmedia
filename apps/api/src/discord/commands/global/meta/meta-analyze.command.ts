...
import { detectArchetype, analyzeWeaknesses, analyzeRoles } from './meta-team-utils';
...
function buildAnalysisPages(
  ...
  const firstTypes = slots.map((s) => entryMap.get(s.speciesId)?.types ?? []).find((t) => t.length) ?? ['normal'];
  ...
  const p1 = base()
    ...
    .addFields({ name: 'Archetype', value: archetypes.join(', '), inline: false });
  ...
  const p2 = base()
    ...
    .setDescription(weakLines.length ? weakLines.join('\n') : 'No notable type weaknesses.')
    .addFields({
      name:   'Note',
  ...
  const p3 = base()
    ...
    .addFields(
      { name: 'Present',  value: roles.present.join(', ') || '—', inline: false },
      { name: 'Missing',  value: roles.missing.join(', ') || '—', inline: false },
    );
...
export class MetaAnalyzeCommand {
  ...
  @Subcommand({ name: 'analyze', description: 'Analyze a team paste for meta ranks, archetypes, and type weaknesses' })
