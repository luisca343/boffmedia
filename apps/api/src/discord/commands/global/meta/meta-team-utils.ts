...
const DEFENSE_CHART: Record<string, Record<string, number>> = {
  ...
  steel:    {
    ...
    normal: 0.5, grass: 0.5, ice: 0.5, flying: 0.5, rock: 0.5, bug: 0.5,
    steel: 0.5, dragon: 0.5, fairy: 0.5, psychic: 0.5, water: 0.5, electric: 0.5, poison: 0,
  },
...
export function analyzeWeaknesses(
  ...
  for (const atkType of ALL_TYPES) {
    ...
    results.push({ atkType, doubleHits, quadHits, total: doubleHits + quadHits * 2 });
  }
...
const HO_ITEMS = new Set([
  'choice scarf', 'choice band', 'choice specs', 'life orb', 'booster energy',
]);
...
export function detectArchetype(slots: VgcMetaSlot[]): string[] {
  ...
  const psyCount = slots.filter((s) => s.moves.some((m) => psyMoves.has(m.toLowerCase()))).length;
...
const ROLE_MOVES: Record<string, string[]> = {
  'Speed Control': ['trick room', 'tailwind', 'electroweb', 'icy wind', 'scary face', 'bulldoze'],
  'Fake Out':      ['fake out'],
