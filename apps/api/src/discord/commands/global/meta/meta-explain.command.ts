...
const SUPPORT_MOVES  = new Set(['fake out', 'follow me', 'rage powder', 'helping hand', 'tailwind', 'trick room', 'wide guard', 'quick guard', 'spore', 'sleep powder', 'thunder wave', 'encore', 'taunt', 'disable']);
const PIVOT_MOVES    = new Set(['volt switch', 'u-turn', 'flip turn', 'parting shot', 'teleport']);
const PROTECT_MOVES  = new Set(['protect', 'detect', 'wide guard', 'quick guard', 'kings shield', "king's shield", 'mat block']);
...
function buildRoleExplanation(d: PokemonUsageDetail): string[] {
  ...
  // Usage tier
  if (d.usagePercent >= 20)      lines.push('**Meta staple** — seen on over 1 in 5 teams.');
  else if (d.usagePercent >= 10) lines.push('**Common pick** — solid meta presence.');
  else if (d.usagePercent >= 5)  lines.push('**Niche pick** — appears in specific team styles.');
  else                            lines.push('**Fringe option** — rarely seen; likely a counter to specific threats.');
  ...
  if (moveNames.some((m) => PROTECT_MOVES.has(m) && m !== 'wide guard' && m !== 'quick guard'))
                                                              roles.push('scout/stall');
  if (moveNames.some((m) => m === 'spore' || m === 'sleep powder'))
                                                              roles.push('sleep setter');
  if (moveNames.some((m) => m === 'helping hand'))            roles.push('Helping Hand support');
  ...
  if (topItem) {
    ...
    lines.push(note ? `**Most common item:** ${topItem} — ${note}` : `**Most common item:** ${topItem}`);
  }

  // Top teammate
  const topTm = d.teammates[0];
  if (topTm) {
    lines.push(`**Best partner:** ${topTm.name} (${topTm.percent.toFixed(1)}%)`);
  }
...
export class MetaExplainCommand {
  ...
  @Subcommand({ name: 'explain', description: "Explain a Pokémon's meta role and competitive context" })
  public async onExplain(
    ...
    if (!entry) {
      await interaction.editReply(`**${pokemon}** was not found in **${reg.name}**.`);
      return;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Usage',      value: `${entry.usagePercent.toFixed(1)}%`, inline: true },
      )
