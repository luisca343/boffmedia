...
@Injectable()
export class LauncherUpdatesService {
  private readonly logger = new Logger(LauncherUpdatesService.name);

  constructor(private readonly releases: LauncherReleasesRepository) {}
    ...
    return {
      ...
      platforms: {
        [platform]: {
          ...
          url: `${baseUrl}/launcher/updates/download/${newest.version}/${platform}`,
        },
      },
    };
  }
  ...
  async artifact(version: string, target: string): Promise<ArtifactStream> {
    ...
    if (!row || !row.published) {
      throw new NotFoundException({
        message: `launcher release ${version}/${target} is not published`,
...
function toEntity(row: LauncherRelease): LauncherReleaseEntity {
  return {
    id: row.id,
    version: row.version,
    target: row.target,
