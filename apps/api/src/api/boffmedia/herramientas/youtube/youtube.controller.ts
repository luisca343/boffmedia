...
export class YoutubeController {
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Transcription retrieved successfully.' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Video not found or transcription not available.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid video ID or URL.' })
  @ApiResponse({ status: HttpStatus.INTERNAL_SERVER_ERROR, description: 'Failed to retrieve transcription.' })
  @ApiParam({ 
    name: 'videoId', 
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Video information retrieved successfully.' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Video not found.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid video ID or URL.' })
  @ApiResponse({ status: HttpStatus.INTERNAL_SERVER_ERROR, description: 'Failed to retrieve video information.' })
  @ApiParam({ 
    name: 'videoId', 
    description: 'YouTube video ID or full URL',
    example: 'dQw4w9WgXcQ'
