 ...
 */
const NATURE_MULTIPLIERS: Record<string, Partial<Record<keyof StatSpread, number>>> = {
...
export class StatCalcService {
   ...
   */
  computeChampionsStats(base: StatSpread, sps: StatSpread, nature: string): StatSpread {
  ...
  isValidChampionsSpread(sps: StatSpread): boolean {
    ...
    return values.every((v) => v >= 0 && v <= 32) && values.reduce((a, b) => a + b, 0) <= 66;
  }
}
