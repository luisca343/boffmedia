...
export const Scripts: ModdedBattleScriptsData = {
  ...
  pokemon: {
    ...
    formeChange(speciesId, source, isPermanent, abilitySlot = '0', message) {
      ...
      if (
        isPermanent &&
        (!source || !['disguise', 'iceface'].includes(source.id))
      ) {
        ...
        const ability =
          species.abilities[abilitySlot] || species.abilities['0'];
    ...
    setStatus(status, source, sourceEffect, ignoreImmunities) {
      ...
      if (status.id) {
        const result: boolean = this.battle.runEvent(
          'SetStatus',
          this,
          source,
          sourceEffect,
  ...
  actions: {
    ...
    canMegaEvo(pokemon: Pokemon) {
      ...
      if (
        ...
        pokemon.baseMoves.includes(this.dex.toID(altForme.requiredMove)) &&
        !item.zMove
      ) {
    ...
    modifyDamage(baseDamage, pokemon, target, move, suppressMessages) {
      ...
      if (typeMod > 0) {
        if (!suppressMessages)
          this.battle.add('-supereffective', target, Math.min(typeMod, 2));
      ...
      if (typeMod < 0) {
        if (!suppressMessages)
          this.battle.add('-resisted', target, Math.min(-typeMod, 2));
      ...
      const bypassProtect = (target.getMoveHitData(move) as any).bypassProtect;
      if (bypassProtect) {
...
} as any as ModdedBattleScriptsData;
