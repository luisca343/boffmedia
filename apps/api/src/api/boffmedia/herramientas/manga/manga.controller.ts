...
export class MangaController {
  ...
  @Get('search')
  @ApiOperation({ summary: 'Search for manga titles across configured sources' })
  @ApiQuery({ name: 'q', type: String, description: 'Search query, e.g. "raeliana"' })
  ...
  @Get('detail')
  @ApiOperation({ summary: 'Get manga detail and full chapter list from a source URL' })
  @ApiQuery({ name: 'url', type: String, description: 'Full URL of the manga detail page' })
  ...
  @Get('local')
  @ApiOperation({ summary: 'List locally downloaded .cbz files for a manga series' })
  @ApiQuery({ name: 'series', type: String, description: 'Series name (used as folder name)' })
  @ApiResponse({ status: HttpStatus.OK, type: LocalChaptersResult })
  async getLocalChapters(@Query('series') seriesName: string): Promise<LocalChaptersResult> {
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'SSE stream of download events.' })
