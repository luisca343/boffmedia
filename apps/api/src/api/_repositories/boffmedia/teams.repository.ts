...
export class TeamsRepository {
  ...
  async findAll(): Promise<EventTeam[]> {
    return this.db
      ...
      .leftJoin(
        boffMediaEvents,
        eq(boffMediaEvents.id, boffMediaEventTeams.eventId),
      )
      .where(
        and(
  ...
  async calculateTeamScore(teamId: number): Promise<number> {
    const result = await this.db
      ...
      .innerJoin(
        boffMediaEventTeamMembers,
        eq(
          boffMediaEventTeamMembers.participantId,
  ...
  async removeMember(teamId: number, participantId: number): Promise<void> {
    await this.db
      .delete(boffMediaEventTeamMembers)
      .where(
        and(
          eq(boffMediaEventTeamMembers.teamId, teamId),
  ...
  async findParticipantTeamInEvent(
    participantId: number,
    eventId: number,
  ): Promise<EventTeamMember[]> {
    return this.db
