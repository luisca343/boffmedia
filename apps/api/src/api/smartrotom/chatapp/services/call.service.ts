...
import { CHAT_REPOSITORY_TOKEN, CHAT_MEMBER_REPOSITORY_TOKEN, CHAT_MESSAGE_REPOSITORY_TOKEN } from '@api/_utils/repositories/interfaces/chatapp.repository.token';
...
export class CallService {
  ...
  async initializeCall(chatId: number, callerUuid: string): Promise<CallSession> {
    ...
    const callerInChat = await this.chatMemberRepository.findUserInChat(chatId, callerUuid);
    if (!callerInChat) {
  ...
  async updateCallUserStatus(
    ...
    const userInChat = await this.chatMemberRepository.findUserInChat(chatId, uuid);
    if (!userInChat) {
