...
export class FicusAIFacadeService {
  ...
  private async generateAIResponse(
    ...
    try {
      ...
      this.currentContextMessages =
        await this.messageService.getMessagesForContext(uuid, 5);

      // Generate AI response
      const geminiResponse = await this.aiService.generateResponse(
        userMessage,
        this.currentContextMessages,
      );
          ...
          parts: [
            {
              ...
              content:
                geminiResponse.text || 'No se pudo generar una respuesta.',
            },
          ],
        };
      }
  ...
  private async processFunctionCalls(
    ...
    const sharedRandomPokemon =
      await this.getSharedRandomPokemonIfNeeded(uniqueCalls);
    ...
    for (const call of uniqueCalls) {
      ...
      try {
        const functionParts = await this.executeFunctionCall(
          call,
          sharedRandomPokemon,
        );
        ...
        if (error instanceof PokemonNotFoundError) {
          ...
          const fallbackResponse =
            await this.aiService.generateFallbackResponse(
              error.pokemonName,
  ...
  private async getSharedRandomPokemonIfNeeded(
    uniqueCalls: any[],
  ): Promise<any> {
    const randomPokemonCalls = uniqueCalls.filter(
      (call) => call.name === 'getRandomPokemon',
    );
  ...
  private async executeFunctionCall(
    ...
    switch (call.name) {
      ...
      case 'getRandomPokemon':
        const dataTypes =
          Array.isArray(call.args.dataTypes) && call.args.dataTypes.length > 0
            ? call.args.dataTypes
            : ['basic'];
      ...
      case 'getPokemonData':
        if (typeof call.args.pokemon === 'string') {
          ...
          const moveTypes = Array.isArray(call.args.moveTypes)
            ? call.args.moveTypes
            : [];

          // Try to get Pokemon data
          const pokemonData = this.pokemonDataService.getPokemonDataParts(
            call.args.pokemon,
            dataTypes,
            moveTypes,
          );
          ...
          if (pokemonData === null) {
            ...
            const similarPokemon =
              this.pokemonDataService.getSimilarPokemonNames(
                call.args.pokemon,
                3,
              );
  ...
  private buildFinalResponse(
        ...
        parts: [
          {
            type: MessagePartType.TEXT,
            content:
              responseText || 'No se pudo obtener información relevante.',
          },
        ],
      };
    }
    ...
    const showIntro = ['type', 'stats', 'moves', 'habitat'].some((type) =>
