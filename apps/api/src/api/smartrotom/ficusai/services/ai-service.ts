...
export class AIService {
  ...
  async generateFallbackResponse(
    ...
    try {
      ...
      const responseText =
        response.text ||
        `Lo siento, no encontré información sobre "${pokemonName}" en la región de Teras. ${similarPokemon.length > 0 ? `¿Te refieres a alguno de estos?: ${similarPokemon.join(', ')}` : '¿Podrías revisar el nombre del Pokémon?'}`;
    ...
    return [
      {
        name: 'countPokemon',
        description:
          'Counts the total number of Pokémon in the Teras region. This function should be called when the user asks for "how many Pokémon", "number of Pokémon", or "total Pokémon".',
        parameters: {
          type: Type.OBJECT,
          properties: {},
        },
      },
      {
        ...
        description:
          'Returns a random Pokémon from the Teras region. Call this when the user asks for a "random Pokémon", "surprise me with a Pokémon", "pick a Pokémon for me", or when they want information about a random Pokémon (like "stats of a random Pokémon", "type of a random Pokémon", etc.). IMPORTANT: Only return information that is provided by this function - never supplement with invented data.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            dataTypes: {
              ...
              description:
                'What information to include about the random Pokémon. Can include: "basic" (just show the Pokémon), "type" (types), "stats" (statistics), "moves" (moves), "habitat" (habitat info). If not specified, defaults to ["basic"].',
            },
          },
          required: [],
        },
        ...
        parameters: {
          ...
          properties: {
            ...
            dataTypes: {
              ...
              items: { type: Type.STRING },
              description:
                'What information to retrieve. Can include: "type" (types), "stats" (statistics), "moves" (moves), "habitat" (habitat info, location, where it can be found, spawn locations). For location/habitat questions, ALWAYS include "habitat" in this array. If not specified, returns all available data.',
            },
            moveTypes: {
              type: Type.ARRAY,
              items: { type: Type.STRING },
              description:
                'Types of moves to filter when moves are requested (level, tutor, egg, tm, tr, hm). If not specified, returns all moves.',
            },
          },
          required: ['pokemon'],
        },
