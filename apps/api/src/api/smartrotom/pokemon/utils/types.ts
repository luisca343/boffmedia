...
export const wolfeyTypeRanking: {
  ...
  type2: string;
  tier: string;
}[] = JSON.parse(fs.readFileSync(defaultDirDef, 'utf8'));
...
function factorFrequency(score: number, type: string, deffensive: boolean) {
  ...
  if (deffensive)
    return realScore >= 25 || realScore < 0
      ? realScore * offTypeFrequency[type]
      : realScore / offTypeFrequency[type];
  //return score > 25 ? realScore * deffTypeFrequency[type] : realScore / deffTypeFrequency[type]
  return realScore >= 25
    ? realScore * deffTypeFrequency[type]
    : realScore / deffTypeFrequency[type];
}
...
export function getOffensiveScoreRanking() {
  ...
  for (const key in sortedRanking) {
  ...
  const _result = Array.isArray(wolfeyTypeRanking)
    ? wolfeyTypeRanking.find((item: any) => {
        return (
          ...
          (item.type1 === type2 && item.type2 === type1)
        );
      })
    : undefined;
}
...
export function getOverallScoreRanking() {
  ...
  for (const key in offensiveRanking) {
    result[key] = offensiveRanking[key] + defensiveRanking[key];
  }

  // Sort the ranking
  const sortedRanking = Object.entries(result)
    .sort((a, b) => b[1] - a[1])
    .reduce(
      (acc, [key, value]) => {
        acc[key] = value;
