...
export class PokemonImageService {
  ...
  async getImage({
    ...
    const form =
      pokemon.forms.find((f) => f.name === formName) || pokemon.forms[0];
    let status = 0;
    ...
    if (pokemonId > 0) {
      ...
      const pokemonStatusFiltered =
        hide == 1
          ? pokemonStatus.filter(
              (p) =>
                p.pokemonId == pokemonId &&
                p.formId === formName &&
                p.paletteId === paletteName,
            )
          : pokemonStatus.filter(
              (p) => p.pokemonId == pokemonId && p.formId === formName,
            );

      if (pokemonStatusFiltered.length > 0) {
        status = pokemonStatusFiltered[0].caughtAt
          ? 2
          : pokemonStatusFiltered[0].seenAt
            ? 1
            : 0;
      }
    }
    ...
    if (fs.existsSync(publicDir))
      return {
        url: path.join('/smartrotom/packs/resourcepack', url),
        type: 'sprite',
        status,
        showImg,
      };
  ...
  getSimpleSprite(
    ...
    try {
      ...
      if (
        ...
        form.genderProperties[0].palettes
      ) {
        palette = form.genderProperties[0].palettes[0];
      }
