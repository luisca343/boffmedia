...
export class InventoryService {
  ...
  async getUserInventory(
    ...
    const aggregatedItems = rawItems.reduce(
      (acc, item) => {
  ...
  async addItemToInventory(
    itemData: ClaimItemData,
  ): Promise<{ success: boolean; item: ArcadeInventoryItem }> {
    ...
    const newItem = await this.arcadeInventoryRepository.findById(
      result.insertId,
    );
  ...
  async removeItem(
    ...
    const existingItem = await this.arcadeInventoryRepository.findUserItem(
      uuid,
      itemId,
    );
    ...
    const success = await this.arcadeInventoryRepository.removeItem(
      uuid,
      itemId,
    );
  ...
  async getInventoryStats(uuid: string): Promise<{
    ...
    inventory.forEach((item) => {
      ...
      itemsByType[item.itemType] =
        (itemsByType[item.itemType] || 0) + item.amount;

      // Count by rarity
      itemsByRarity[item.rarity] =
        (itemsByRarity[item.rarity] || 0) + item.amount;
    });
  ...
  async markItemAsUsed(
    ...
    const item = await this.arcadeInventoryRepository.findUserItem(
      uuid,
      itemId,
    );
