...
export class DesktopDeviceService {
  private readonly logger = new Logger(DesktopDeviceService.name);

  constructor(
    private readonly repo: DesktopDeviceRepository,
  ...
  async poll(deviceCode: string): Promise<DevicePollResult> {
    ...
    const tokenVersion =
      (await this.packsRepo.getDesktopTokenVersion(user.id)) ?? 0;
    ...
    try {
      ...
      this.logger.error(
        `No se pudo auditar la autorización de la app para el usuario ${user.id}: ${
