...
const ctx = (headers: Record<string, string>) =>
  ({
    switchToHttp: () => ({ getRequest: () => ({ headers }) as unknown as LauncherRequest }),
...
describe('LauncherAuthGuard', () => {
  ...
  beforeEach(() => {
    auth = { verifySession: jest.fn().mockReturnValue(principal()) } as unknown as jest.Mocked<PacksAuthService>;
    repo = {
  ...
  it('accepts a matching token version and populates req.launcher', async () => {
    const req = { headers: { authorization: 'Bearer tok' } } as unknown as LauncherRequest;
    const context = {
  ...
  it('treats a versionless principal as 0', async () => {
    auth.verifySession.mockReturnValue(principal({ tokenVersion: undefined }) as never);
