...
const row = (over: Record<string, unknown> = {}) => ({
  ...
  deviceCode: DEVICE,
  userCode: 'BCDF-GHJK',
  clientLabel: 'Boff Launcher 1.0 (win)',
  status: 'pending',
  userId: null,
  consumedAt: null,
...
describe('LauncherDeviceService', () => {
  let service: LauncherDeviceService;
  let repo: jest.Mocked<LauncherDeviceRepository>;
  ...
  beforeEach(() => {
    ...
    } as unknown as jest.Mocked<LauncherDeviceRepository>;
    ...
    packsRepo = {
      ...
      getLauncherTokenVersion: jest.fn().mockResolvedValue(4),
    } as unknown as jest.Mocked<PacksRepository>;

    service = new LauncherDeviceService(repo, auth, users, packsRepo);
  });
  ...
  describe('poll', () => {
    ...
    it('writes the durable launcher.auth audit row', async () => {
      repo.findByDeviceCode.mockResolvedValue(
        row({ status: 'approved', userId: 7 }) as never,
      );
      ...
      expect(packsRepo.audit).toHaveBeenCalledWith(
        ...
        null,
        UUID,
        { userId: 7, clientLabel: 'Boff Launcher 1.0 (win)' },
        7,
      );
    });
    ...
    it('embeds 0 when the account has no revocation counter yet', async () => {
      ...
      (packsRepo.getLauncherTokenVersion as jest.Mock).mockResolvedValue(null);
      await service.poll(DEVICE);
