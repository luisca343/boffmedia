...
export class DesktopUpdatesController {
  ...
  @SkipEnvelope()
  @ApiOperation({
    summary: 'Feed de actualización de la app',
    description:
      'Configúralo en tauri.conf.json como `.../desktop/updates/{{target}}-{{arch}}/{{current_version}}`. Devuelve 204 sin cuerpo cuando ya está actualizado, y el JSON de Tauri v2 (version, notes, pub_date, platforms) cuando hay algo más nuevo. Sin sobre `{success,data}`: Tauri deserializa el cuerpo tal cual.',
  })
 ...
 *  absoluta. DESKTOP_UPDATE_BASE_URL manda; si no está, se deriva de las
