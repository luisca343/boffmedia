...
// DESKTOP_RELEASE_DIR; this table is the index, mirroring how packs keeps
// override blobs on disk and their metadata in MySQL.

/** Tauri's platform key: `{os}-{arch}` — windows-x86_64, darwin-aarch64, … */
export type DesktopTarget = string;

export const desktopReleases = mysqlTable(
  'desktop_releases',
  {
  ...
  (table) => ({
    ...
    versionTargetIdx: uniqueIndex('desktop_releases_version_target_uq').on(
      table.version,
      table.target,
    ),
...
export type NewDesktopRelease = typeof desktopReleases.$inferInsert;
