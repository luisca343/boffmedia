...
export const desktopDeviceCodes = mysqlTable(
  ...
  (table) => ({
    userIdx: index('ddc_user_idx').on(table.userId),
    expiresIdx: index('ddc_expires_idx').on(table.expiresAt),
    userFk: foreignKey({
...
export type DesktopDeviceCode = typeof desktopDeviceCodes.$inferSelect;
