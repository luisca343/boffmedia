...
--
-- Marks the 4 migrations in the journal as ALREADY APPLIED, so that
...
-- migration and skip the other 3.
...
SELECT * FROM (
  ...
  UNION ALL SELECT 'd195228aaf057cf8dfcd723b31169153936adb15d5aea06283f6ef3d990ef7f5', 1789308313164  -- 0003_wise_korvac
) AS applied
WHERE @ledger_rows = 0;

-- What you should see: 4 rows, and a newest `created_at` of
-- 1789308313164 (0003_wise_korvac).
