...
Decisions and reference implementations: [`docs/DESKTOP_HANDOFF.md`](docs/DESKTOP_HANDOFF.md).
Read §4.2's superseded note first — the shell is Tauri, not Electron.

## Working on the UI (no Rust needed)

```bash
pnpm --filter desktop dev:renderer     # http://localhost:5273
```
...
```bash
pnpm --filter desktop dev              # tauri dev: builds Rust, opens the window
pnpm --filter desktop build            # tauri build: produces installers
```
...
```powershell
$env:BOFF_API_URL="https://api.ficuslab.es"; pnpm --filter desktop dev
```
