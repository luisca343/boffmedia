// Where the app keeps everything, and the one-shot layout migration into it.
//
...
/// The app's data root. Every other module asks this one — a second
...
/// Bring an install made by an earlier build up to the current instance layout:
...
/// is an instance the player reinstalls, not an app that will not open.
pub fn migrate(app: &tauri::AppHandle) {
    let Ok(root) = data_root(app) else {
        return;
    };

    // Skipped once the marker says this tree has already been through it.
        ...
        match std::fs::read_dir(&legacy_game) {
            Ok(children) => {
                for child in children.flatten() {
                    ...
                    // the app's own state, and clobbering one is worse
...
mod tests {
    ...
    fn flatten_moves_the_game_tree_up_and_renames_bin() {
        ...
        // The app's own state is untouched by the move.
