...
/// `.boff-install.json` is. `is_app_owned` therefore guards the delete
/// path too, and both only apply at the root: a `.boff-`-named file the player
/// puts inside `config/` is theirs.
fn is_app_owned(rel_dir: &str, name: &str) -> bool {
...
pub async fn instance_delete_path(
    ...
    if !trimmed.contains('/') && is_app_owned("", trimmed) {
        return Err(InstallFailure::message(
            "Ese archivo pertenece a la app y no se puede borrar aquí.",
        ));
    }
    let root = instance_root(&slug, &app)?;
...
mod tests {
    ...
    #[test]
    fn app_state_is_hidden_at_the_root_but_not_inside_the_players_folders() {
        assert!(is_app_owned("", ".boff-install.json"));
        assert!(is_app_owned("", ".boff-bin"));
        // The game's own tree is never filtered, dot-prefixed or not.
        assert!(!is_app_owned("", "mods"));
        assert!(!is_app_owned("", ".fabric"));
        // Same name one level down belongs to the player.
        assert!(!is_app_owned("config", ".boff-install.json"));
    }
