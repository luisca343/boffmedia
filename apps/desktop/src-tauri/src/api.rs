...
async fn activate_boff(api: &ApiState, id: i64) -> Result<bool, ApiError> {
    let Some(token) =
        store::load_app_session_for(id).map_err(|e| ApiError::Store(e.to_string()))?
    else {
        return Ok(false);
    };
    store::save_app_session(&token).map_err(|e| ApiError::Store(e.to_string()))?;
...
pub async fn boff_device_poll(
    ...
    if body.data.status == "approved" {
        ...
        if let Err(e) = store::save_app_session_for(account.id, &token) {
            let mut roster = load_boff_roster(&app);
...
pub async fn boff_session_restore(
    ...
    if roster.accounts.is_empty() {
        match store::load_app_session() {
            Ok(Some(token)) => {
                *api.token.lock().await = Some(token.clone());
                return match boff_me(&api).await {
                    Ok(account) => {
                        store::save_app_session_for(account.id, &token)
            ...
            Ok(true) => match boff_me(&api).await {
                Ok(account) => {
                    ...
                    let dead: Vec<i64> = roster
                        ...
                        .filter(|other| {
                            *other != account.id
                                && matches!(store::load_app_session_for(*other), Ok(None))
                        })
                        .collect();
                    for id in dead {
                        remove_boff(&mut roster, id);
                ...
                Err(ApiError::NeedsSignin(_)) => {
                    let _ = store::clear_app_session_for(id);
                    remove_boff(&mut roster, id);
                }
                Err(e) => return Err(e),
            },
...
async fn switch_inner(
    ...
    match boff_me_with(api, &candidate).await {
        Ok(account) => {
            store::save_app_session(&candidate)
                .map_err(|e| ApiError::Store(e.to_string()))?;
...
pub async fn boff_sign_out(
    ...
    let _ = store::clear_app_session_for(active);
...
pub async fn boff_offline(
    ...
    let Some(token) =
        store::load_app_session_for(active).map_err(|e| ApiError::Store(e.to_string()))?
    else {
