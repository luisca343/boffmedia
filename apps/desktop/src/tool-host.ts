 ...
 * so there it falls back to the website's origin — the assets are public, and a
 * dev renderer that could not show an image would be a poor way to build one.
 *
...
function desktopAssetUrl(path: string): string {
  ...
  if (!isDesktop()) return `${webBaseUrl().replace(/\/$/, "")}${rooted}`
