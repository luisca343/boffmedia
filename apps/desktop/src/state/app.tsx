...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const boffSignOutFn = React.useCallback(async () => {
    ...
    try {
      ...
      if (next) {
        ...
        log({ level: "info", source: "app", text: `Cuenta activa: ${next.username}` })
      } else {
  ...
  const switchBoffAccount = React.useCallback(
    async (id: number) => {
      ...
      try {
        ...
        log({ level: "info", source: "app", text: `Cuenta activa: ${account.username}` })
      } catch (err) {
  ...
  const signIn = React.useCallback(async (): Promise<boolean> => {
    ...
    if (!isDesktop()) {
      ...
      log({ level: "info", source: "app", text: `Sesión simulada como ${MOCK_ACCOUNT.username}` })
      return true
    }
    ...
    try {
      ...
      log({ level: "info", source: "app", text: `Sesión iniciada como ${account.username}` })
      return true
    } catch (err) {
      const failure = err as { message?: string }
  ...
  const repair = React.useCallback(
    async (packId: string) => {
      ...
      log({ level: "info", source: "app", text: `Reparando ${entry.pack.name}…` })
      try {
  ...
  const stop = React.useCallback(() => {
    ...
    log({ level: "warn", source: "app", text: "Juego detenido por el usuario" })
    if (!packId) {
      dispatch({ type: "game/state", game: { kind: "idle" } })
      return
    }
  ...
  const revalidate = React.useCallback(async () => {
    ...
    try {
      ...
      if (account) {
        ...
        dispatch({ type: "boff/signout" })
        log({ level: "warn", source: "app", text: "Tu sesión ya no es válida. Vuelve a autorizar el launcher." })
      }
    } catch (err) {
