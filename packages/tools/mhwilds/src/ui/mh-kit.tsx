...
export function MhWrap({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`px-[clamp(1rem,2.4vw,2.25rem)] pt-5 pb-16 ${className}`}>{children}</div>
}
      ...
      {options.map((o) => {
        ...
        return (
          <button
            ...
            className={`appearance-none border-0 cursor-pointer font-mono text-[0.75rem] leading-none tracking-[0.04em] uppercase font-semibold py-2 px-[0.8125rem] inline-flex items-center gap-1.5 transition-colors ${on ? "bg-[var(--mh-soft)] text-[var(--mh-bright)]" : "bg-transparent text-txt-muted hover:text-txt"}`}
          >
            {o.label}
          </button>
        )
...
export function MhPanel({
  ...
  return (
    <section className={`bg-panel border border-line cut-corner cut-corner-edge ${className}`}>
      {(title || aside) && (
        <div className="flex items-center gap-2.5 px-[0.9375rem] pt-3 pb-[0.6875rem] border-b border-line">
          {icon && <Icon name={icon} size={15} className="text-[var(--mh-bright)]" />}
          {title && <h3 className="font-mono text-[0.8125rem] leading-none font-bold uppercase tracking-[0.08em] text-txt-muted m-0 not-italic">{title}</h3>}
          <span className="flex-1" />
          {count != null && <span className="font-mono text-[0.6875rem] leading-none text-txt-dim tracking-[0.04em]">{count}</span>}
          {aside}
        </div>
      )}
      <div className="px-[0.9375rem] py-3.5">{children}</div>
    </section>
  )
}
...
export function MhStat3({ items }: { items: { value: React.ReactNode; label: string; mod?: "attack" | "def"; color?: string }[] }) {
  return (
    <div className="grid grid-cols-3 gap-2">
      {items.map((it, i) => (
        <div key={i} className="py-[0.6875rem] px-2.5 bg-base-2 border border-line text-center">
          <b
            className="block font-display text-[1.5rem] leading-none italic font-extrabold"
            style={{ color: it.color || (it.mod === "attack" ? "#ff7a5c" : it.mod === "def" ? "var(--info)" : "var(--text)") }}
          >
            {it.value}
          </b>
          <span className="font-mono text-[0.625rem] leading-none uppercase tracking-[0.05em] text-txt-dim mt-1 block">{it.label}</span>
        </div>
      ))}
    </div>
  )
...
export function MhSharpness({ sharpness, legend }: { sharpness: Record<string, number>; legend?: string }) {
  ...
  return (
    <div>
      <div className="flex h-[0.9375rem] bg-base-deep border border-line overflow-hidden">
        {SHARP_ORDER.map((s) => {
      ...
      {legend && <div className="mt-1.5 font-mono text-[0.625rem] leading-none text-txt-dim">{legend}</div>}
    </div>
  )
}
...
export function MhSkillRow({
  ...
  return (
    <div
      className={`grid grid-cols-[1fr_auto] items-center gap-2.5 py-2 px-[0.6875rem] bg-base-2 border border-line transition-colors ${over ? "border-l-2 border-l-warn" : "border-l-2"}`}
      style={over ? undefined : ({ borderLeftColor: sk } as React.CSSProperties)}
      title={desc}
    >
      <div className="min-w-0">
        <div className="font-body text-[0.8125rem] leading-tight font-semibold truncate">{name}</div>
        {desc && <div className="font-mono text-[0.6875rem] leading-tight text-txt-dim mt-0.5 truncate">{desc}</div>}
      </div>
      <div className="flex items-center gap-[3px]">
        ...
        {over > 0 && <span className="font-mono text-[0.6875rem] leading-none font-bold text-warn ml-1">+{over}</span>}
      </div>
    </div>
  )
}
...
export function MhCatLegend({ labels }: { labels: Record<string, string> }) {
  ...
  return (
    <div className="flex flex-wrap gap-x-3 gap-y-1">
      {cats.map((c) => (
        <span key={c} className="inline-flex items-center gap-1.5 font-mono text-[0.625rem] leading-none uppercase tracking-[0.04em] text-txt-muted">
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex flex-col gap-0.5">
        ...
        <span className={`font-body text-[0.875rem] leading-tight truncate ${filled ? "text-txt font-semibold" : "text-txt-dim italic font-medium"}`}>{name}</span>
      </span>
  ...
  return (
    <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2.5 py-2 px-[0.6875rem] bg-base-2 border border-line">
      ...
      <span className="flex items-center gap-1.5">
        <span className="font-mono text-[0.8125rem] leading-none font-bold text-txt">×{quantity}</span>
        {onToggle && (
...
export function MhEquipItem({
  ...
  return (
    <button
      type="button"
      onClick={onPick}
      className={`grid grid-cols-[auto_1fr_auto] items-center gap-[0.6875rem] w-full text-left py-[0.6875rem] px-3 bg-base-2 border cursor-pointer transition-colors hover:bg-panel-2 hover:border-line-2 ${active ? "border-[var(--mh)] shadow-[inset_0_0_0_1px_var(--mh)]" : "border-line"}`}
    >
      <MhRarity rarity={item.rarity} />
      <span className="min-w-0">
        <span className="block font-body text-[0.875rem] leading-[1.2] font-semibold truncate">{item.name}</span>
        {skills.length > 0 && (
          <span className="mt-[0.3125rem] flex flex-wrap gap-1">
  ...
  return (
    <div className={`py-[0.5625rem] px-[0.6875rem] bg-base-2 border border-line ${active ? "" : "opacity-50"}`}>
      <div className="flex items-center gap-2">
        <span className="font-display text-[0.75rem] leading-none font-bold uppercase tracking-[0.03em]">{bonus.bonusName}</span>
        <span className="ml-auto inline-flex gap-[3px]">
          {[1, 2, 3, 4, 5].map((n) => (
            <i key={n} className="w-[0.4375rem] h-[0.4375rem] rounded-full" style={{ background: n <= bonus.pieces ? "var(--mh)" : "var(--line-2)" }} />
          ))}
        </span>
      </div>
      <div className={`mt-[0.3125rem] font-mono text-[0.6875rem] leading-[1.3] ${active ? "text-[var(--mh-bright)]" : "text-txt-dim"}`}>
        {active ? (
  ...
  return (
    <div className="flex flex-col gap-3">
      {Array.from({ length: n }).map((_, i) => (
        <div
          key={i}
          className="h-[4.125rem] border border-line bg-[linear-gradient(100deg,var(--panel)_30%,var(--panel-2)_50%,var(--panel)_70%)] bg-[length:220%_100%] animate-[mh-shimmer_1.3s_linear_infinite] motion-reduce:animate-none"
        />
      ))}
    </div>
  )
...
export function MhRing({ pct, label }: { pct: number; label: React.ReactNode }) {
  return (
    <div
      className="w-[2.625rem] h-[2.625rem] rounded-full flex-none grid place-items-center relative"
      style={{ background: `conic-gradient(var(--mh) ${pct}%, var(--line) 0)` }}
    >
      <span className="w-8 h-8 rounded-full bg-panel absolute" />
      <span className="relative font-mono text-[0.6875rem] leading-none font-bold text-txt z-[1]">{label}</span>
    </div>
  )
}
...
export function MhItem({
  ...
  return (
    <button
      type="button"
      onClick={onPick}
      className={`grid grid-cols-[auto_1fr_auto] items-center gap-[0.6875rem] w-full text-left py-[0.6875rem] px-3 bg-base-2 border cursor-pointer transition-colors hover:bg-panel-2 ${active ? "border-[var(--mh)] shadow-[inset_0_0_0_1px_var(--mh)]" : "border-line hover:border-line-2"}`}
    >
      {children}
    </button>
  )
...
export function MhTag({ children, sk }: { children: React.ReactNode; sk?: boolean }) {
  return (
    <span className={`font-mono text-[0.625rem] leading-none px-1.5 py-[3px] tracking-[0.02em] border ${sk ? "text-[var(--mh-bright)] border-[var(--mh-line)] bg-panel" : "text-txt-muted border-line bg-panel"}`}>
      {children}
    </span>
  )
}
    ...
    <>
      ...
      >
        <div className="flex items-center gap-[0.6875rem] flex-none py-[0.8125rem] px-4 border-b border-line">
          ...
          <div className="min-w-0">
            <div className="font-display text-[1rem] leading-[1.1] font-bold uppercase tracking-[0.03em] not-italic">{title}</div>
            {sub && <div className="font-mono text-[0.6875rem] leading-none text-txt-muted mt-0.5">{sub}</div>}
          </div>
