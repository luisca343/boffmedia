          ...
          <div className="grid grid-cols-1 items-start lg:grid-cols-[minmax(18.75rem,22.5rem)_minmax(0,1fr)]">
            ...
            <div className="flex flex-col min-h-0 lg:sticky lg:top-[calc(var(--tool-sticky-top,0px)_+_var(--tool-bar-h,3.625rem))] lg:h-[calc(var(--tool-vh,100dvh)_-_var(--tool-bar-h,3.625rem))] lg:border-r border-solid border-line bg-base-2">
                  ...
                  <div className="flex border border-solid border-line bg-panel">
                    {(["grid", "list"] as View[]).map((v) => (
                      <button
                        ...
                        onClick={() => setView(v)}
                        aria-label={v}
                        className={cn("w-[2.125rem] h-[1.875rem] grid place-items-center transition-colors", view === v ? "bg-[var(--mh-soft)] text-[var(--mh-bright)]" : "text-txt-dim hover:text-txt")}
                      >
              ...
              <div className="flex-1 min-h-0 overflow-y-auto p-[12px_13px_40px] bm-scroll">
                ...
                {filtered.length === 0 ? (
                  <p className="font-mono text-[0.75rem] text-txt-dim py-6 text-center">{t("noResults")}</p>
                ) : view === "grid" ? (
      ...
      <div className="flex flex-wrap items-start gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2.5 flex-wrap">
            <h2 className="font-display text-[clamp(1.625rem,4vw,2.375rem)] font-extrabold italic leading-[0.95] uppercase">{m.name}</h2>
            <Tag2 dot={`hsl(${(m.species.length * 47) % 360} 45% 60%)`}>{cap(m.species)}</Tag2>
            <Tag2>{m.kind === "large" ? t("kindLarge") : t("kindSmall")}</Tag2>
          </div>
          {m.description && <p className="mt-3 text-txt-muted text-[0.90625rem] leading-[1.55] max-w-[70ch] text-pretty">{m.description}</p>}
        </div>
      </div>
        ...
        <div className="flex flex-col">
          ...
          {m.elements && m.elements.length > 0 && (
            <div className="grid grid-cols-[auto_1fr] items-baseline gap-3 py-2">
              <span className="font-mono text-[0.6875rem] uppercase tracking-[0.04em] text-txt-dim">{t("elements")}</span>
        ...
        {drops.length > 0 ? (
          <div className="flex flex-col gap-[0.3125rem]">
            {drops.map((d) => {
              ...
              return (
                <div key={d.key} className="grid grid-cols-[minmax(0,1.5fr)_auto_auto_minmax(5.25rem,0.8fr)] items-center gap-3 p-[8px_11px] bg-base-2 border border-solid border-line">
                  <span className="flex items-center gap-2 min-w-0">
                    <span className="w-[0.6875rem] h-[0.6875rem] rotate-45 border border-solid" style={{ borderColor: `var(--rar${Math.min(8, d.rarity)})`, background: `var(--rar${Math.min(8, d.rarity)})` }} />
                    <span className="font-body text-[0.8125rem] font-semibold truncate">{d.name}</span>
                  </span>
                  <span className="font-mono text-[0.625rem] uppercase tracking-[0.04em] text-txt-muted whitespace-nowrap">{d.rank ? `${d.rank} · ` : ""}{d.kind}</span>
                  <span className="font-mono text-[0.75rem] font-bold text-txt">×{d.qty}</span>
                  <span className="inline-flex items-center gap-1.5">
                    <span className="flex-1 h-[0.4375rem] bg-base-deep border border-solid border-line overflow-hidden">
                      <span className="block h-full" style={{ width: `${d.chance}%`, background: tone.color }} />
                    </span>
                    <span className="font-mono text-[0.6875rem] font-bold min-w-[1.875rem] text-right" style={{ color: tone.color }}>{d.chance}%</span>
                  </span>
                </div>
              )
            })}
...
function StatRow({ k, v }: { k: React.ReactNode; v: React.ReactNode }) {
  return (
    <div className="grid grid-cols-[auto_1fr] items-baseline gap-3 py-2 border-b border-dashed border-line last:border-b-0">
      <span className="font-mono text-[0.6875rem] uppercase tracking-[0.04em] text-txt-dim">{k}</span>
      <span className="font-body text-[0.8125rem] font-semibold text-txt text-right">{v}</span>
    </div>
  )
}
