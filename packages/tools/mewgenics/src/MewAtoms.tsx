...
export function MewTile({ cat, rec, size = 44, glyph, frame = "blob", art: artProp }: { cat: string; rec: MewRec; size?: number; glyph?: IconName; frame?: "blob" | "slot"; art?: string | null }) {
  ...
  // Reset when the art changes: this component is reused across records as the
  // grid re-renders, and a sticky `err` would blank a perfectly good image.
  React.useEffect(() => setErr(false), [artProp, rec.id])

  // Resolve art: use prop if provided, otherwise auto-resolve via category-specific logic
  const art = React.useMemo(() => {
    // `err` first, and the order is the bug this used to have: written as
    // `if (err || artProp) return artProp || null`, a tile GIVEN its art kept
    // handing back the same broken url after `onError` fired, so the fallback
    // glyph only ever worked for tiles that resolved their own art. The fiche
    // hero passes `art`, which is why a missing image left a browser's broken
    // -file icon inside the frame while the grid behind it degraded cleanly.
    if (err) return null
    if (artProp) return artProp
