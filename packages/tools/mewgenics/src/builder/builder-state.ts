...
import { MEW_DEFAULT_CAT_PARTS, MEW_DEFAULT_PALETTE } from "../cat"
import type { CatParts } from "../cat"
 ...
 *  Extension-agnostic on purpose: the asset build rasterises icons to
 *  `.webp` (icon_map is rewritten to the new extension), but older trees
 *  and any future format still end in `<frame>.<ext>`. */
export function itemFrame(item: { icon?: unknown }): number | null {
  const m = /(\d+)\.\w+$/.exec(String(item.icon || ""))
