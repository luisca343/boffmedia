...
export function AxisSlider({ axis = "Y", value, max, onChange, className }: AxisSliderProps) {
  return (
    <div className={cn("shrink-0 flex items-center gap-2.5 py-[0.5625rem] px-3 border-t border-line", className)}>
      <span className="font-mono text-[0.6875rem] font-bold text-accent-bright w-3.5 text-center shrink-0">{axis}</span>
      <input
        type="range"
        min={0}
        max={max}
      ...
      />
      <span className="font-mono text-[0.6875rem] text-txt-muted w-11 text-right tabular-nums shrink-0">
        {value}/{max}
      </span>
    </div>
  )
