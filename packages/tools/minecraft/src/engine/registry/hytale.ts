...
import { HYTALE_FLUID_BLOCKS } from "../fluids";
...
export interface HytaleRegistryOptions {
  /**
   * Block ids to guarantee in the catalog even when this install never defines
   * them. Injected by the caller — the conversion layer passes its cross-game
   * mapping targets so a mapped block always resolves. Building the catalog must
   * not read a rule table itself: what blocks *exist* is registry construction,
   * what blocks *map to* is conversion policy, and the second may not drive the
   * first.
   */
...
export async function buildHytaleRegistry(
  ...
  for (const id of options?.requiredBlockIds ?? []) {
    // Force-injected ids aren't in this install, so their real variant is
