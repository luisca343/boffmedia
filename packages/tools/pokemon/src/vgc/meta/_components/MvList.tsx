...
import { type PokeData, type UsageEntry } from "../_lib/meta-types"
          ...
          filtered.map((e) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex w-full min-w-0 items-center gap-[0.75rem] border border-solid border-l-[3px] px-[0.6875rem] py-[0.5625rem] text-left transition-[background,border-color]",
                  on ? "border-line border-l-accent bg-panel" : "border-transparent border-l-transparent hover:bg-panel",
                )}
              >
                <span className={cn("w-[1.5rem] flex-none text-right font-mono text-[0.625rem] font-bold leading-none", on ? "text-accent-bright" : "text-txt-dim")}>
                  {String(rank).padStart(2, "0")}
                </span>
                <DkSprite src={spriteUrl(p?.name ?? e.id)} alt={p?.name ?? e.id} size={40} onError={handleSpriteError} />
                <span className="grid min-w-0 flex-1">
                  <b className="truncate font-display text-[0.8125rem] font-bold uppercase leading-[1.15] tracking-[0.03em]">{p?.name ?? e.id}</b>
                </span>
                <span className="w-[3.875rem] flex-none text-right">
                  <b className="font-mono text-[0.8125rem] font-bold leading-none">{e.usage.toFixed(1)}%</b>
                </span>
              </button>
            )
          })
