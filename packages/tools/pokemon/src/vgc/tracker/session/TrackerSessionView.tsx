...
import { useMatches, useBuilderPresets, useSeries, useSessions, usePreset } from "../../tracker-core/hooks/useVgcDb"
...
import type { Match, Series, SeriesGame } from "../../tracker-core/types"
import { TrStats } from "./TrStats"
import { TrMatchRow, TrSeriesRow } from "./TrRows"
...
export function TrackerSessionView({ sessionId }: Props) {
  const t = useVgcT("tracker")
  const tStats = useVgcT("tracker.sessionStats")
  ...
  // The teambuilder is the source of truth for team format and legality.
  const sessionPresets = selectablePresets
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      if (newMatches.length > 0) {
        // Queue what was STORED: `putMany` stamps each row with the version
        // the server's conflict check compares against.
        const stored = await vgcDb.matches.putMany(newMatches)
        for (const m of stored) pushChange("matches", m.id, m)
