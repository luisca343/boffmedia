...
import { DkApp, DkBar, DkBody, DkTitle, DkDivider, DkSeg, DkSelect, DkSpacer, DkChip } from "@boffmedia/ui/datakit";
import { VgcService, ChampionsRegulation, SpeedTierEntry } from "../../service";
...
function SpeedPageContent() {
  ...
  const tTiers = useVgcT("speedTiers");
  const { query: searchParams } = useVgcNav();
  const router = useVgcNav();
  const { path: pathname } = useVgcNav();
...
export function SpeedTiersView() {
  return (
    <VgcRoot initialHref="/pokemon/vgc/speed">
      <SpeedScreen />
    </VgcRoot>
  )
}
