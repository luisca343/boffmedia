...
export interface TeamPreset {
  ...
  updatedAt: number;
  /**
   * The version the SERVER compares, set on every local write.
   *
   * Distinct from `updatedAt` because a row adopted from the server carries the
   * writing device's stamp, and this device's next write has to advance past it
   * — not sit equal to it, which is what happens when only `updatedAt` moves.
   */
