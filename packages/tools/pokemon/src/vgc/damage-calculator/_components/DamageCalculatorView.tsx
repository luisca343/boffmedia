...
function DamageCalcShell() {
  ...
  return (
    <div className="flex min-w-0 flex-col" style={{ minHeight: "var(--tool-vh, 100dvh)" }}>
      {/* App bar. An App surface: it deliberately carries NO title — the tool is
          ...
          `--tool-sticky-top`, so this no longer hardcodes `--nav-h`. The box above
          reads `--tool-vh` for the same reason: `--nav-h` exists only in
          apps/web, so in the desktop app the whole declaration is dropped. */}
      <ToolStrip>
        <Tabs
          value={tab}
...
export function DamageCalculatorView() {
  return (
    <VgcRoot initialHref="/pokemon/vgc/damage-calculator">
      <CalcScreen />
    </VgcRoot>
  )
}
