 ...
 *
 * Every url this package builds — sprites, effects, backgrounds, audio — is a
 * root-relative path under the shared asset tree, handed to the host so it can
 * say where those bytes actually live: the site's own origin on the web, the
 * `boffasset://` cache in the launcher.
 *
...
export function battlesimAssetUrl(path: string): string {
  const rooted = joinAssetPath(ASSET.boffmedia.tools.battlesim, path);
