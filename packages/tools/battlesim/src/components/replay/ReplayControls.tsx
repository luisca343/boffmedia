...
import { REPLAY_SPEEDS } from '../../engine/replaySpeed';
import { BSIM_SEG_FOCUS } from '../bsim-kit';
        ...
        <div className="flex w-full min-w-0 flex-col gap-2">
            ...
            <div className="relative px-1" aria-label={t('replays.ctl.timeline')}>
                <input
                    ...
                    className="w-full cursor-pointer accent-accent outline-none focus-visible:outline-2 focus-visible:outline-accent-line focus-visible:outline-offset-[3px]"
            ...
            <div
                ...
                className="cut-tag cut-tag-edge [--cut-tag:10px] [--cut-line:var(--line)] flex flex-wrap items-center gap-2 border border-solid border-line bg-[color-mix(in_srgb,var(--panel)_88%,transparent)] px-3 py-2 backdrop-blur-[4px]"
            >
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    <Input
                        ...
                        value={turnInput}
                        onChange={(e) => {
                            // An empty field parses to NaN, which used to be
                            // written straight into state and then handed to
                            // the seek as a turn number.
                            const n = parseInt(e.target.value, 10);
                            if (!Number.isNaN(n)) setTurnInput(Math.min(Math.max(1, n), Math.max(1, lastTurn)));
                        }}
                        min={1}
                        max={lastTurn}
