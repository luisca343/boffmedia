...
export function SelectCard({ selected, onChange, title, description, icon, className }: SelectCardProps) {
  return (
    <button
      ...
      className={cn(
        "flex items-start gap-3 border border-solid bg-panel p-[0.9375rem] text-left",
    ...
    >
      <span
        className={cn(
          "grid size-[1.375rem] shrink-0 place-items-center border border-solid transition-[border-color,background] duration-[140ms]",
