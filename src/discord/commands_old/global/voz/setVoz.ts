...
async function execute(interaction, service: CommandsService) {
    let voz = interaction.options.getInteger('voz')
...
async function autocomplete(interaction){
    ...
    let filtered = choices.filter(choice => choice.name.toLowerCase().includes(focusedValue.toLowerCase()));
