...
export function CallStatus() {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      const activeUser = activeCall.users.find(
        (user) => user.uuid === getSmartRotomUser(session).uuid
      );
      if (
        callStartTime &&
        Date.now() - callStartTime > 6000 &&
        activeUser?.status === UserStatus.RINGING
      ) {
  ...
  useEffect(() => {
    ...
    if (
      activeCall.users.length > 1 &&
      (users.RINGING?.length || 0) + (users.IN_CALL?.length || 0) <= 1
    ) {
      exitCall();
    }
  ...
  return (
    <nav
      ...
      } bg-surface-800 text-surface-50 font-bold z-20 ${
        activeCall || isExpanded ? "" : "hidden"
      }`}
    >
      <audio
        ref={audioRef}
        src="/smartrotom/audio/chatapp/denden.mp3"
        preload="auto"
      ></audio>
      ...
      <div className="flex justify-around">
        {isExpanded && (
          <div>
            ...
            <div>
              {activeCall.users.filter(
                (user) => user.status === UserStatus.IN_CALL
              ).length === 1 && "Esperando respuesta..."}
              {activeCall.users.filter(
                (user) => user.status === UserStatus.IN_CALL
              ).length > 1 && "En llamada"}
            </div>
