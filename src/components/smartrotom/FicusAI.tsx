...
export default function FicusAI() {
  ...
  async function enviarMensaje() {
    ...
    setMensajes((prevMensajes) => [
      ...prevMensajes,
      { sender: "user", parts: [{ type: "text", content: text }] },
    ]);
    ...
    <div className="max-w-xl mx-auto flex flex-col p-4 h-full text-lg">
      ...
      <div className="flex items-center mt-4">
        ...
        <button
          ...
          className="bg-blue-800 text-text-primary px-4 py-2 rounded"
        >
          Enviar
        </button>
      </div>
...
function MensajeChat({
  ...
  const messageClass =
    sender === "user"
      ? "bg-blue-800 text-text-primary self-end"
      : "bg-green-800 text-text-primary self-start";

  return (
    <li
      className={`border border-border-dark rounded p-3 m-2 max-w-full ${messageClass} whitespace-pre-wrap`}
          ...
          return (
            <div key="1234124">
              ...
              <div className="flex items-center">
                <div className="w-2/5 mx-2">PS: {stats.hp}</div>
                <Progress className="w-3/5 mx-2" value={stats.hp / 2.55} />
              </div>
              <div className="flex items-center">
                <div className="w-2/5 mx-2">Ataque: {stats.attack}</div>
                <Progress className="w-3/5 mx-2" value={stats.attack / 2.55} />
              </div>
              <div className="flex items-center">
                <div className="w-2/5 mx-2">Defensa: {stats.defense}</div>
                <Progress className="w-3/5 mx-2" value={stats.defense / 2.55} />
              </div>
              <div className="flex items-center">
                <div className="w-2/5 mx-2">
                  At. Especial: {stats.specialAttack}
                </div>
                <Progress
                  className="w-3/5 mx-2"
                  value={stats.specialAttack / 2.55}
                />
              </div>
              <div className="flex items-center">
                <div className="w-2/5 mx-2">
                ...
                <Progress
                  className="w-3/5 mx-2"
                  value={stats.specialDefense / 2.55}
                />
              </div>
              <div className="flex items-center">
                <div className="w-2/5 mx-2">Velocidad: {stats.speed}</div>
                <Progress className="w-3/5 mx-2" value={stats.speed / 2.55} />
              </div>
            </div>
          );
                  ...
                  <section key={index} className="mt-2">
                    ...
                    <p>
                      {movimientos[type]
                        .map(
                          (
                            move: { level: string; attacks: string[] },
                            index: number
                          ) => {
