          ...
          <PopoverContent className="w-80 bg-gray-800 border-gray-700 text-orange-100">
            ...
            <ScrollArea className="h-64">
              {notifications.length === 0 ? (
                <p className="text-center text-gray-400 py-4">No hay notificaciones</p>
              ) : (
                notifications.map((notification) => (
                  <div key={notification.id} className="mb-2 p-2 bg-gray-700 rounded">
                    {/* @ts-ignore */}
                    <p className="text-sm">{notification.content}</p>
                    <div className="flex justify-end mt-1">
                      <Button variant="ghost" size="sm" onClick={() => remove(notification.id)}>
                        Eliminar
                      </Button>
                      {!notification.read && (
