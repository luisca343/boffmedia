      ...
      <NavigationMenuList>
        <NavigationMenuItem>
          ...
          <NavigationMenuContent>
            <ul className="grid w-[400px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[600px] bg-main-900 rounded-md border border-main-800 shadow-xl">
              {Object.entries(HERRAMIENTAS).map(([category, tools]) => (
                <li key={category} className="row-span-3 space-y-3">
                  <h3 className="font-bold text-lg text-orange-300 mb-2 pl-3">{category.charAt(0).toUpperCase() + category.slice(1)}</h3>
      ...
      <NavigationMenuLink asChild>
        <a
          ...
          className={cn(
            ...
            "text-orange-300 hover:text-orange-100 hover:bg-main-800",
            className
          )}
          {...props}
        >
          ...
          {children && (
            <p className="line-clamp-2 text-sm leading-snug text-main-400 mt-2">
              {children}
            </p>
          )}
        </a>
