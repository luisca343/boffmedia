...
export const starBankTransactions = mysqlTable("rotom_bank_transactions", {
    id: int("id").primaryKey().autoincrement(),
    from: int("from").notNull().references(() => starBankAccounts.id, {onDelete: "cascade", onUpdate: "cascade"}),
    to: int("to").notNull().references(() => starBankAccounts.id, {onDelete: "cascade", onUpdate: "cascade"}),
...
export const  starBankUsersAccounts = mysqlTable("rotom_bank_users_accounts", {
    ...
    accountId: int("account_id").notNull().references(() => starBankAccounts.id, {onDelete: "cascade", onUpdate: "cascade"}),
});
