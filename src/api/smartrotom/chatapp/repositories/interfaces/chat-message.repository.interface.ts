...
export interface IMessageRepository {
  ...
  createMessage(messageData: { chatId: number; content: string; senderUUID: string; type: string }): Promise<{ insertId: number }>;
  ...
  markMessageAsRead(messageId: number, uuid: string): Promise<{ insertId: number }>;
