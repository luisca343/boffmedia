...
export class Pokemon {
  ...
  @ApiProperty({ example: ['Fake Out', 'Nuzzle', 'Thunderbolt', 'Spiky Shield'], description: 'Moveset (up to 4 moves)' })
  moves: (string | null)[];

  @ApiProperty({ example: [17, 10, 19, 30, 9, 23], description: 'IVs (HP, Atk, Def, SpA, SpD, Spe)' })
  ivs: number[];

  @ApiProperty({ example: [252, 0, 3, 252, 3, 0], description: 'EVs (HP, Atk, Def, SpA, SpD, Spe)' })
  evs: number[];

  @ApiProperty({ example: [320, 211, 150, 178, 160, 220], description: 'Stats (HP, Atk, Def, SpA, SpD, Spe)' })
  stats: number[];
}
