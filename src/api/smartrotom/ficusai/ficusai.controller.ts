import { Controller, Post, Get, Delete, Body, Query, HttpStatus, UseInterceptors } from '@nestjs/common';
...
export class FicusAIController {
  constructor(
    private readonly ficusAIFacadeService: FicusAIFacadeService,
  ) {}

  @Get('messages')
  @ApiOperation({ 
    summary: 'Get chat messages for a user',
    description: 'Retrieves the chat history for a specific user UUID with optional limit'
  })
  @ApiResponse({
  ...
  async getMessages(@Query() getMessagesDto: GetMessagesDto): Promise<FicusMessageContentDto[]> {
  ...
  @ApiOperation({ 
    ...
    description: 'Sends a user message to Professor Ficus and returns the AI response'
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async sendMessage(@Body() sendMessageDto: SendMessageDto): Promise<FicusMessageContentDto> {
  ...
  async initializeChat(@Body('uuid') uuid: string): Promise<FicusMessageContentDto> {
  ...
  async deleteUserMessages(@Query('uuid') uuid: string): Promise<SuccessResponse> {
  ...
  async getUserStats(@Query('uuid') uuid: string) {
    const messageCount = await this.ficusAIFacadeService.getUserMessageCount(uuid);
