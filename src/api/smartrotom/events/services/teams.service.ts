...
export class TeamsService {
  ...
  async createTeam(eventId: number, createTeamDto: CreateTeamDto): Promise<EventTeam> {
    ...
    const leaderParticipant = await this.participantsService.getOrCreateParticipantByUserId(createTeamDto.leaderId);
  ...
  async joinTeam(eventId: number, teamId: number, userId: number): Promise<EventTeamMember> {
    ...
    const participant = await this.participantsService.getOrCreateParticipantByUserId(userId);
    
    // 2. Check if participant is already in a team for this event
    const existingTeam = await this.teamsRepository.findParticipantTeamInEvent(participant.id, eventId);
  ...
  async leaveTeam(teamId: number, userId: number): Promise<{ success: boolean; message?: string }> {
    // 1. Get participant
    const participant = await this.participantsService.getOrCreateParticipantByUserId(userId);
    
    // 2. Check if participant is the team leader
    const member = await this.teamsRepository.findMember(teamId, participant.id);
    ...
    if (member.role === 'leader') {
      throw new Error('Team leader cannot leave the team. Transfer leadership or disband the team.');
    }
