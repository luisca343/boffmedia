...
export class InvitesFacadeService {
  ...
  async createInvite(uuid: string, username: string): Promise<InviteCreationResult> {
    try {
  ...
  async registerWithInvite(inviteId: string, registrationData: RegistrationData): Promise<RegistrationResult> {
    try {
      // Validate registration data
      const validation = await this.registrationService.validateRegistrationData(registrationData);
      if (!validation.valid) {
        return {
          success: false,
