...
export class MineRepository {
  ...
  async findPlayerEnergy(uuid: string): Promise<{ energy: number } | null> {
    const result = await this.db.select({ energy: smartrotomUsers.energy })
  ...
  async findPlayerLastCharge(uuid: string): Promise<Date | null> {
    const result = await this.db.select({ lastCharge: smartrotomUsers.lastCharge })
      .from(smartrotomUsers)
      .where(eq(smartrotomUsers.uuid, uuid))
      .limit(1);
  ...
  async updatePlayerEnergy(uuid: string, energy: number, lastCharge?: Date): Promise<void> {
    const updateData: Partial<SmartRotomUser> = { energy };
    if (lastCharge) {
      updateData.lastCharge = lastCharge;
    }

    await this.db.update(smartrotomUsers)
  ...
  async createGameSession(uuid: string): Promise<{ insertId: number }> {
    const result = await this.db.insert(mineGames)
  ...
  async findMaxRewardValue(): Promise<number> {
    const result = await this.db.select({ maxValue: max(mineRewards.value) })
      .from(mineRewards);
    
    return result[0]?.maxValue || 0;
  }

  async createGameRewards(gameId: number, rewards: { rewardId: number; value: number; claimed?: number }[]): Promise<void> {
    if (rewards.length === 0) return;

    await this.db.insert(mineGamesDetail)
  ...
  async findTopPlayers(limit: number = 10): Promise<RankingEntry[]> {
    const result = await this.db.select({
      ...
      totalValue: sql<number>`COALESCE(SUM(${mineGamesDetail.value}), 0)`.as('totalValue'),
      gamesPlayed: sql<number>`COUNT(DISTINCT ${mineGames.id})`.as('gamesPlayed')
    })
    .from(smartrotomUsers)
  ...
  async findPlayerRanking(uuid: string): Promise<{ rank: number; totalValue: number } | null> {
  ...
  async createInventoryEntries(entries: InventoryEntry[]): Promise<void> {
    ...
    await this.db.insert(smartRotomInventory)
  ...
  async findUnclaimedItems(uuid: string): Promise<{ id: number; itemId: string; type: string }[]> {
    return this.db.select({
  ...
  async getPlayerStats(uuid: string): Promise<{
    ...
    const result = await this.db.select({
      totalGames: sql<number>`COUNT(DISTINCT ${mineGames.id})`.as('totalGames'),
      totalValue: sql<number>`COALESCE(SUM(${mineGamesDetail.value}), 0)`.as('totalValue'),
