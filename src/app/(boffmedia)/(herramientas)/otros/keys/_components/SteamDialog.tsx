...
export default function SteamDialog({
  ...
  return (
    <Dialog open={isModalVisible} onOpenChange={setIsModalVisible}>
      <DialogContent className="sm:max-w-[700px] xl:max-w-[1200px] bg-surface-900 text-surface-100 border border-surface-700">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary-300 to-primary-600 flex items-center gap-2">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-3 bg-surface-800">
            <TabsTrigger
              value="info"
              className="text-surface-300 data-[state=active]:text-primary-400 data-[state=active]:bg-surface-700"
            >
          ...
          <TabsContent value="info" className="mt-4">
            <ScrollArea className="sm:h-[300px] xl:h-[600px] rounded-md border border-surface-700 p-4">
              <h3 className="text-lg font-semibold mb-2 text-primary-400">
                Descripción
              </h3>
              <div 
                dangerouslySetInnerHTML={{ __html: selectedGame.description }}
                className="text-surface-300 w-[616px]"
              />
              <h3 className="text-lg font-semibold mb-2 mt-4 text-primary-400 flex items-center gap-2">
                ...
                {selectedGame.genres ? (
                  selectedGame.genres.map((genre, index) => (
                    <Badge
                      key={index}
                      variant="secondary"
                      className="bg-surface-700 text-primary-400"
                    >
                      {genre}
                    </Badge>
                  ))
                ) : (
                  <span className="text-surface-300">No hay géneros</span>
                )}
              </div>
            </ScrollArea>
          </TabsContent>
            ...
            <div className="space-y-4">
              {selectedGame.normalPrice !== selectedGame.currentPrice && (
                <div className="flex justify-between items-center p-4 bg-surface-800 rounded-lg">
                  <span className="text-surface-300">Precio Normal:</span>
              ...
              <div className="flex justify-between items-center p-4 bg-surface-800 rounded-lg">
                <span className="text-surface-300">Precio Actual:</span>
                    ...
                    return (
                      <motion.div
                        ...
                        className={`relative cursor-pointer ${
                          index === selectedMediaIndex ? "ring-2 ring-primary-500" : ""
                        }`}
        ...
        <DialogClose asChild>
          <Button
            variant="secondary"
            className="mt-4 w-full bg-primary-600 text-white hover:bg-primary-700 transition-colors duration-200"
          >
            Cerrar
          </Button>
        </DialogClose>
