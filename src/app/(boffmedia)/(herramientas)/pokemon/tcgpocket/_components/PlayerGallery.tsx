    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mb-4 space-y-4 sm:space-y-0">
        ...
        <div className="flex items-center space-x-4">
          <Select value={selectedEvent} onValueChange={setSelectedEvent}>
            <SelectTrigger className="w-[200px] bg-surface-3 text-white border-border-dark">
              <SelectValue placeholder="Seleccionar evento" />
            </SelectTrigger>
            <SelectContent className="bg-surface-3 text-white border-border-dark">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="bg-surface-3 text-white border-border-dark max-w-4xl">
              <DialogHeader>
                <DialogTitle className="text-2xl font-bold text-primary">Probabilidades de Nuevas Cartas por Pack</DialogTitle>
      ...
      {loading ? (
        <div className="flex items-center justify-center min-h-[50vh]">
          <Loader2 className="w-8 h-8 text-primary-light0 animate-spin" />
        </div>
      ) : allCards.length === 0 ? (
        <p className="text-center text-text-secondary">No se encontraron cartas.</p>
      ) : (
              ...
              return (
                ...
                >
                  <div className="group block">
                    <div className={`relative bg-surface-3/50 backdrop-blur-sm rounded-xl p-3 transition-all duration-300 hover:bg-surface-3/50 hover:scale-105 hover:-translate-y-1 shadow-lg hover:shadow-xl ${isMissing ? 'grayscale' : ''}`}>
                      ...
                      <div className="mt-2">
                        ...
                        <p className="text-xs text-text-tertiary text-center">{trans(card.expansion)}</p>
                        {editable && (
      ...
      {editable && Object.keys(changes).length > 0 && (
        ...
        >
          <Button onClick={onSaveChanges} className="bg-primary-light hover:bg-primary-hover" disabled={loading}>
