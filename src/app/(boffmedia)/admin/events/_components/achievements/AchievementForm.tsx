...
const achievementSchema = z.object({
  ...
  criteria: z.string().min(10, "Los criterios deben tener al menos 10 caracteres"),
})
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4 pb-6">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Criterios de Desbloqueo</FormLabel>
              <FormControl>
                <Textarea
                  placeholder="Describe cómo se desbloquea este logro"
                  className="bg-surface-700 border-surface-600 text-surface-50 min-h-[100px]"
                  {...field}
                />
              </FormControl>
              <FormDescription>
                Explica detalladamente qué deben hacer los usuarios para obtener este logro.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
