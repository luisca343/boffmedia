...
export default function AdminPage() {
    ...
    useEffect(() => {
        const eventSource = new EventSource(`${process.env.NEXT_PUBLIC_API}/herramientas/ptcgp/fetch-status`)
                ...
                {status && (
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            ...
                            <Badge
                                variant={status.status === 'success' ? 'default' : status.status === 'error' ? 'destructive' : 'default'}
                            >
                                {status.status}
                            </Badge>
                        </div>
