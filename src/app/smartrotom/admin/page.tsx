...
export default function PanelControlAdmin() {
  ...
  return (
    <div className="w-full min-h-screen bg-black text-green-400 font-mono p-4 overflow-hidden terminal-container relative">
      <pre className="text-xs sm:text-sm md:text-base text-center text-green-500 mb-4 overflow-x-auto">
        {`
         ______ _                _           _         
        |  ____(_)              | |         | |        
      ...
      <div className="border border-green-700 rounded-md mb-6 p-4">
        <div className="bg-green-900/30 px-4 py-1 border-b border-green-700 flex justify-between items-center">
          ...
          <div className="text-xs text-green-400">root@ficus-labs:~</div>
          <div className="text-xs text-green-400">{ipAddress}</div>
        </div>
        
        <div className="p-4 bg-black/90 text-sm terminal-content" style={{ fontFamily: 'monospace' }}>
          {!loaded ? (
            <>
              <p className="text-green-500">$ iniciando_sistema</p>
              <p className="text-green-300">Cargando módulos del núcleo... <span className="text-yellow-400">OK</span></p>
              <p className="text-green-300">Verificando integridad... <span className="text-yellow-400">OK</span></p>
              <p className="text-green-300">Estableciendo conexión segura... <span className="text-yellow-400">OK</span></p>
              <p className="text-green-300">Autenticando privilegios de administrador... <span className="text-yellow-400">OK</span></p>
              <p className="text-green-500 animate-pulse">Cargando interfaz de administración...</p>
            </>
          ) : (
            <h1 className="text-2xl font-bold mb-4 text-center text-green-500 uppercase tracking-widest glitch">
          ...
          {typing && loaded && (
            <div className="typing-effect">
              <span className="text-green-400">$:</span> <span className="text-green-200">acceso_concedido</span>
      ...
      {loaded && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {appsAdmin.map((app) => (
              <Link href={app.enlace} key={app.nombre} className={`block transition-opacity duration-500 ${visibleApps.includes(app.id) ? 'opacity-100' : 'opacity-0'}`}>
                <Card className="hover:shadow-neon bg-black border-green-500 border h-full relative transition-all duration-300 hover:scale-[1.01]">
                  <div className="absolute top-0 left-0 w-full h-full bg-grid-pattern opacity-10"></div>
                  <div className="absolute top-0 right-0 p-1 text-xs text-green-400">#{app.id}</div>
                  <CardHeader className="flex flex-row items-center space-x-4 px-3 py-3">
                    <div className="bg-green-900/40 rounded-sm p-2 text-green-400">
                      {app.icono}
                    </div>
                    <CardTitle className="text-green-400 font-bold text-base">
                      <span className="text-green-600 mr-2">&gt;</span>{app.nombre}
                    </CardTitle>
                  </CardHeader>
          ...
          {/* System Stats */}
          <div className="mt-8 border border-green-800/60 rounded bg-black/60 p-3">
            <div className="text-xs flex flex-col sm:flex-row justify-between mb-2">
              <p className="text-green-500/80"><span className="text-green-600">[SYS]</span> Acceso autorizado: {timestamp}</p>
              <p className="text-green-500/80 flex items-center">
                <span className="text-green-600">[SEC]</span> Nivel de seguridad: 
            ...
            <div className="grid grid-cols-3 gap-4 text-xs">
              <div>
                <div className="flex justify-between">
                  ...
                  <span className={`${stats.cpu > 80 ? 'text-red-400' : 'text-green-400'}`}>{stats.cpu}%</span>
                </div>
                <div className="w-full bg-green-900/30 h-1 rounded">
                  <div className={`h-1 rounded ${stats.cpu > 80 ? 'bg-red-500' : 'bg-green-500'}`} style={{width: `${stats.cpu}%`}}></div>
                </div>
              </div>
              <div>
                <div className="flex justify-between">
                  <span className="text-green-600">MEM:</span>
                  <span className={`${stats.mem > 80 ? 'text-red-400' : 'text-green-400'}`}>{stats.mem}%</span>
                </div>
                <div className="w-full bg-green-900/30 h-1 rounded">
                  <div className={`h-1 rounded ${stats.mem > 80 ? 'bg-red-500' : 'bg-green-500'}`} style={{width: `${stats.mem}%`}}></div>
                </div>
              </div>
              <div>
                <div className="flex justify-between">
                ...
                <div className="w-full bg-green-900/30 h-1 rounded">
                  <div className="h-1 bg-green-500 rounded" style={{width: `${Math.min(stats.net/10, 100)}%`}}></div>
                </div>
              </div>
            </div>
          </div>
