      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 max-w-4xl w-full z-10">
        {juegos.map((juego) => (
          <InternalLink
            key={juego.nombre}
            href={juego.href}
            className={`bg-surface-800 bg-opacity-80 border-4 border-${juego.color}-500 rounded-lg p-6 flex flex-col items-center justify-center space-y-4 hover:bg-surface-700 hover:bg-opacity-80 transition duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-${juego.color}-400 focus:ring-opacity-50`}
          >
            {juego.icono ? (
