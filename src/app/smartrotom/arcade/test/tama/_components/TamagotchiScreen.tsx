...
export function TamagotchiScreen({
  ...
  return (
    <div className={`w-64 h-64 rounded-lg overflow-hidden mb-8 relative shadow-[inset_0_2px_10px_rgba(0,0,0,0.3)] border-2 border-border-dark z-20 transition-colors duration-300 ${state.isLightOn ? 'bg-green-200' : 'bg-surface-3'}`}>
      ...
      {isOn && isReset ? (
        <>
          {isHatched ? (
            <>
              {/* Top Menu Items */}
              <div className={`absolute top-0 left-0 right-0 flex justify-around py-2 z-30 border-b border-green-400/50 ${state.isLightOn ? 'bg-green-300/80' : 'bg-surface-3/80'}`}>
                {MENU_ITEMS.slice(0, 4).map((item, index) => (
                  <div
                    key={index}
                    className={`p-1 rounded-full ${selectedMenuIndex === index ? (state.isLightOn ? 'bg-green-500' : 'bg-foreground0') : ''}`}
                  >
                    {React.createElement(item.icon, {
                      size: 20,
                      className: `${state.isLightOn ? 'text-green-800' : 'text-text-secondary'} ${selectedMenuIndex === index ? 'text-white' : ''}`,
                    })}
                  </div>
                ))}
              </div>
              ...
              {activeSubMenu === 'HEALTH_METER' ? (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className={`text-sm ${state.isLightOn ? 'text-green-800' : 'text-text-secondary'}`}>
                    {renderHealthMeter()}
                  </div>
                </div>
              ) : (
              ...
              {/* Bottom Menu Items */}
              <div className={`absolute bottom-0 left-0 right-0 flex justify-around py-2 z-30 border-t border-green-400/50 ${state.isLightOn ? 'bg-green-300/80' : 'bg-surface-3/80'}`}>
                {MENU_ITEMS.slice(4).map((item, index) => (
                  <div
                    key={index + 4}
                    className={`p-1 rounded-full ${selectedMenuIndex === index + 4 ? (state.isLightOn ? 'bg-green-500' : 'bg-foreground0') : ''}`}
                  >
                    {React.createElement(item.icon, {
                      size: 20,
                      className: `${state.isLightOn ? 'text-green-800' : 'text-text-secondary'} ${selectedMenuIndex === index + 4 ? 'text-white' : ''}`,
                    })}
                  </div>
                ))}
              </div>
              ...
              {activeSubMenu && activeSubMenu !== 'HEALTH_METER' && (
                <div className="absolute inset-x-0 top-12 flex justify-center items-start z-40">
                  <div className={`px-4 py-2 rounded-b-lg shadow-md border-x border-b ${state.isLightOn ? 'bg-green-300/90 border-green-400' : 'bg-surface-3/90 border-border-dark'}`}>
                    <div className="flex space-x-4">
                      {MENU_ITEMS[selectedMenuIndex].subMenu?.map((item, index) => (
                        <div
                          key={index}
                          className={`p-2 rounded ${subMenuIndex === index 
                            ? (state.isLightOn ? 'bg-green-500 text-white' : 'bg-foreground0 text-white') 
                            : (state.isLightOn ? 'bg-green-200 text-green-800' : 'bg-surface-5 text-text-secondary')}`}
                        >
                          {item}
                        </div>
                      ))}
            ...
            <div className="flex items-center justify-center h-full">
              <div className={`text-2xl font-bold animate-pulse ${state.isLightOn ? 'text-green-800' : 'text-text-secondary'}`}>Incubando...</div>
            </div>
          )}
        </>
      ) : isOn && !isReset ? (
        <div className="flex items-center justify-center h-full">
          <div className={`text-2xl font-bold ${state.isLightOn ? 'text-green-800' : 'text-text-secondary'}`}>Presiona cualquier botón para iniciar</div>
        </div>
      ) : (
        <div className="flex items-center justify-center h-full">
          <div className={`text-2xl font-bold ${state.isLightOn ? 'text-green-800' : 'text-text-secondary'}`}>Quita la pestaña para comenzar</div>
        </div>
      )}
    </div>
  )
