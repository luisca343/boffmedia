...
export default async function Ranking() {
  ...
  if(!ranking) return <></>
  return (
    <MenuWrapper className="w-full h-screen overflow-hidden bg-main-900 text-white pt-4  flex flex-col items-center">
      <div className="bg-black bg-opacity-70 p-6 rounded-lg w-3/4 max-w-3xl ">
        <h2 className="text-2xl font-bold mb-4">RANKING</h2>
        <table className="w-full">
          <thead>
            <tr className="border-b border-main-600">
          ...
          <tbody>
            ...
            {ranking.map((user: any, i: number) => (
              <tr key={i} className="border-b border-main-600">
