...
export default function Reclamar(){
    ...
    <MenuWrapper>
            ...
            Object.keys(unclaimed).map((key, i) => {
                return (
                    <div key={i} className="relative inline-block">
                        ...
                        <span className="absolute bottom-0 right-0 bg-primary-500 text-white text-sm rounded-full h-6 w-6 flex items-center justify-center">
                            {unclaimed[key]}
                        </span>
                    </div>
                )
        ...
        <button onClick={() => claimReward()} className="bg-primary-500 text-white rounded-md p-2 mt-4">Reclamar</button>
        </div>
    </MenuWrapper>
    )
}