...
export default async function Drops() {
  ...
  if(!drops) return <></>
  return (
    <MenuWrapper className="w-full min-h-full overflow-hidden bg-surface-2 text-white pt-4   flex flex-col items-center">
        ...
        <div className="space-y-4 overflow-auto">
          {Object.keys(drops).map((type: string) => (
            <Collapsible key={type}>
              <CollapsibleTrigger className="mx-auto text-2xl p-4 border-b  shadow flex flex-col items-center m-2 w-[80%] text-text-secondary">
                ...
                <div className="flex flex-wrap justify-center mx-auto w-4/5">
                  {drops[type].items.map((drop: Drop) => (
                    ...
                    >
                      ...
                      />
                      <p className="text-text-secondary text-center text-lg ">{drop.name}</p>
                      <p className="text-text-secondary text-center text-md ">
