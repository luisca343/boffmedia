...
export function BarraEnergia() {
    ...
    return (
        <div className='flex flex-col w-full ml-auto mt-auto text-text-primary text-shadow-border1 tex-lg  xl:text-xl '>
        {energy >= maxEnergy ? 
            <></> : 
            <div className='mx-auto '>Siguiente recarga en: {getHour()}</div>
        }
        <motion.div className=" ml-auto px-1 w-full flex  rounded-lg border-2 border-border-dark bg-surface-2 flex-wrap items-start justify-start">
            <AnimatePresence>
                {[...Array(maxEnergy)].map((_, i) => {
                    ...
                    return <motion.div
                        key={i}
                        style={{ width: `${100 / 10 }%` }}
                        className={` rounded-lg h-12 xl:h-20 border-y-4 border-x-2 border-border-dark text-center flex items-center justify-center`}
