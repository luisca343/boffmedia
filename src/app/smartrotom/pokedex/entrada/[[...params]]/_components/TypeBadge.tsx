...
export default function TypeBadge({type}: {type: string}){
    ...
    return <div className={` m-1 h-8 w-32 font-bold text-xl 2xl:text-base min-w-28 flex flex-row items-center  text-text-primary bg-${type.toLowerCase()} pl-2 pr-2 rounded text-shadow-border2 2xl:text-shadow-border1`} 
...
export function TypeBadgeSmall({type}: {type: string}){
    ...
    return <div className={` m-1 h-6 font-bold text-xs w-16 flex flex-row items-center  text-text-primary bg-${type.toLowerCase().trim()} pl-1 pr-1 rounded text-shadow-border1`} 
        style={{backgroundColor: typeColors?.backgroundColor, color: 'white'}}>
