...
export function TypeBadgeSmall({type}: {type: string}){
    ...
    return <div className={` m-1 h-6 font-bold text-xs min-w-16 flex flex-row items-center  text-white bg-${type.toLowerCase().trim()} pl-1 pr-1 rounded text-shadow-border1`} 
        style={{backgroundColor: typeColors?.backgroundColor, color: 'white'}}>
