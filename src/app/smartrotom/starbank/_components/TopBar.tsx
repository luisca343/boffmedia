...
export default function TopBar({
  ...
  return (
    <header className="bg-secondary-200 shadow-sm border-b border-secondary-300">
      <div className="h-20 mx-auto py-4 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        ...
        <div className="flex items-center relative" ref={dropdownRef}>
          {activeAccount && (
            <div 
              className="flex items-center gap-3 cursor-pointer bg-secondary-100 hover:bg-secondary-300 transition-colors duration-200 px-4 py-2 rounded-full"
              onClick={() => setShowDropdown(!showDropdown)}
            >
              <div className="text-right hidden sm:block">
                <p className="text-sm font-medium text-secondary-950">{session?.user?.name || "Usuario"}</p>
                <p className="text-xs text-secondary-800">{activeAccount.name}</p>
              </div>
              <AccountImage width={40} height={40} type={activeAccount.type} name={activeAccount.name}/>
              <ChevronDownIcon className="h-4 w-4 text-secondary-800" />
            </div>
          )}
          ...
          {showDropdown && (
            <div className="absolute top-full right-0 mt-2 w-64 bg-white rounded-md shadow-lg z-50 py-2">
              ...
              <div className="px-4 py-2 border-b border-surface-200">
                <p className="text-sm font-medium text-secondary-950">{session?.user?.name || "Usuario"}</p>
                <p className="text-xs text-secondary-500">Cambiar cuenta</p>
              </div>
                    ...
                    {accounts.map((account: StarBankAccount) => (
                      <div
                        ...
                        className={`flex items-center px-4 py-2 hover:bg-secondary-50 cursor-pointer ${
                          activeAccount!.id === account.id ? 'bg-secondary-50' : ''
                        }`}
                      ...
                      >
                        ...
                        <div className="flex-grow">
                          <p className="text-sm font-medium text-secondary-900">{account.name}</p>
                          <p className="text-xs text-secondary-600">{formatMoney(account.balance)}</p>
                        </div>
