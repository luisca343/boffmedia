...
import { ASPECT_RATIO, TARGET_WIDTH, SCALE_WIDTH, getImageSize, getOffset } from "../_utils/viewUtils";
...
export const BattleCanvas = forwardRef(({ battle, pov, messageBar }: { battle: Battle, pov: 0 | 1, messageBar: string[] }, ref) => {
    const pokemonRefs = useRef<{ [key: string]: PokemonRefType | null }>({});
    ...
    useImperativeHandle(ref, () => ({
        ...
        animateMove(attackerIdent: PokemonIdent, moveName: string, defenderIdent: PokemonIdent) {
            ...
            /*
            if(attackerRef) attackerRef.bounce();
            if(defenderRef) defenderRef.bounce();
            */

            // Move the attacker to the defender
            if(attackerRef && defenderRef) {
                const attackerPosition = getOffset(battle, attacker, 1);
                const defenderPosition = getOffset(battle, defender, 1);


                setTimeout(() => {
                    attackerRef.moveTo(defenderPosition.left, defenderPosition.top);
                }, 200);
            }
        }
    }));
        ...
        <div  id="game" className="flex overflow-hidden relative" style={{
            ...
            >          
                ...
                <PokemonElement
                    ...
                    ref={el => pokemonRefs.current[position] = el}
                    side={battle.p1}
                    position={position}
                    />
                ))}
