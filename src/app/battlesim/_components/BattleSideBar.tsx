...
import NpcSkin from "@/components/smartrotom/MinecraftSkin";
  ...
  export function PlayerDataBar({ battle, side, pov } : { battle: Battle, side: 'p1' | 'p2', pov: 'p1' | 'p2' }) {
    ...
    if(!avatarId) return <></>
    return (
      <div className={` ${side === 'p1' ? 'content-end' : 'content-start'} bg-surface-2 h-full bg-opacity-30 z-10`}>
        <div className="text-center font-bold text-text-primary">{player.name.charAt(0).toUpperCase() + player.name.slice(1)}</div>
        <div className="h-fit">
          {
            uuid !== null ? (
