...
export function Game({battleName = 'medalla_doku'}: {battleName?: string}) {
  ...
  useEffect(() => {
    if(!isAnimationPlaying && actionQueue.length > 0){
      ...
      if(action.startsWith('AUDIO:') || action.startsWith('AUDIO_FAINT:')) {
        ...
        const currActionQueue = actionQueue.slice(2);
        
        setActionQueue(currActionQueue);
        setTimeout(() => {
  ...
  }, [isAnimationPlaying, actionQueue, battle]);
  ...
  async function performAction(localBattle: Battle, line: string) {
    ...
    if(args[0] === 'faint'){
      ...
      if(pokemon){
        timeout.end = -1;
        await scene.playBattleAnim('faint', positionIdent.split(':')[0] as PokemonIdent, positionIdent.split(':')[0] as PokemonIdent, async() => {
          setBattle(localBattle);
  ...
  useEffect(() => {
    if (battleLog) {
      if(isPlaying) readTurn(copyBattle(battle), battleLog, 0).then(() => {
  ...
  useEffect(() => {
    if(isPlaying && actionQueue.length === 0){
      readTurn(copyBattle(battle), battleLog as string, battle.turn + 1)
    }
  }, [battle]);
