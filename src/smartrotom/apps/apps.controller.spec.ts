...
describe('AppsController', () => {
  ...
  describe('create', () => {
    it('should create an app', async () => {
      ...
      expect(await controller.create(app)).toEqual(app);
    });
  });
  ...
  describe('findAll', () => {
    it('should return an array of apps', async () => {
      ...
      expect(await controller.findAll()).toEqual(apps);
    });
  });
  ...
  describe('update', () => {
    it('should update an app', async () => {
      const updateAppDto = { id: 1, name: 'Updated App', description: 'Updated Description' };
      const result = { id: 1, ...updateAppDto, raw: [], generatedMaps: []};
      jest.spyOn(repo, 'update').mockImplementation(() => Promise.resolve(result));

      expect(await controller.update(1, updateAppDto)).toBe(result);
