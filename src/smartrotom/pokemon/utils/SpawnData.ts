...
export class SpawnData {
    ...
    async loadSpawnData() {
        ...
        await Promise.all(folders.map(async folder => {
            const defaultDirDef = path.join(__dirname, '../../../../', 'public/smartrotom/packs/default_datapack/data/pixelmon/spawning');
            const publicDir = path.join(__dirname, '../../../../', 'public/smartrotom/packs/datapack/data/pixelmon/spawning');

            const defaultDir = await fsPromises.readdir(path.join(defaultDirDef, folder));
            ...
            for (const file of fullDir) {
                ...
                if(fs.existsSync(fileName)) {
                    data = JSON.parse(await fsPromises.readFile(fileName, 'utf8')) as SpawnInfos
                    terasCounter++;
                } else {
                ...
                //if(folder === 'standard'){
                    await data.spawnInfos.forEach((spawnInfo: SpawnInfo) => {
                        ...
                        */
                        ...
                        if(palette){
                            if(!this.spawnByPalette[palette]) this.spawnByPalette[palette] = []
        ...
        console.log(`Cargados ${terasCounter} spawns de Teras y ${defaultCounter} spawns por defecto en ${Date.now() - startingTime}ms`);
    }
}