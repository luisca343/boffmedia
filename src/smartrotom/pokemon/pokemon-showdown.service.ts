...
export class PokemonShowdownService {
    ...
    private convertPixelmonAbilityToShowdown(abilityName: string): string {
        ...
        let showdownName = abilityName.replace(/([A-Z])/g, ' $1').trim();
        
        // Capitalize each word
        return showdownName.split(' ')
    ...
    async getTerasPokemonShowdownData(): Promise<ShowdownPokemonData> {
        ...
        const pokemonWithForms = new Map<string, { 
            basePokemon: any, 
            forms: any[],
            allFormNames: string[],
            defaultForm: string
        ...
        for (const [pokemonName, pokemonData] of pokemonWithForms.entries()) {
            ...
            let defaultFormObj = forms.find(form => form.name === defaultForm) || forms[0];
            ...
            const alternateFormNames = formNames
                .filter(name => name !== defaultForm && !['mega', 'gmax', 'terasmega', 'terasgmax'].includes(name));
            ...
            if (alternateFormNames.length > 0) {
                ...
                const baseShowdownId = pokemon.name.toLowerCase().replace(/[^a-z0-9]/g, '');
                
                // Process default form abilities
                const defaultFormAbilities = defaultFormObj.abilities || forms[0].abilities;
                const abilities: { [key: string]: string } = {};
                
                if (defaultFormAbilities) {
                    defaultFormAbilities.abilities.forEach((ability, index) => {
                        const showdownAbilityName = this.convertPixelmonAbilityToShowdown(ability);
                        abilities[index.toString()] = showdownAbilityName;
                    });
                    
                    if (defaultFormAbilities.hiddenAbilities && defaultFormAbilities.hiddenAbilities.length > 0) {
                ...
                const standardizedDefaultForm = defaultForm !== 'base' ? 
                    this.standardizeFormName(defaultForm) : undefined;
                ...
                const showdownPokemon = {
                    ...
                    types: defaultFormObj.types ? 
                        defaultFormObj.types.map(type => type.charAt(0).toUpperCase() + type.slice(1).toLowerCase()) : 
                        [],
                    baseStats: {
                        hp: statsSource?.hp || 0,
                ...
                if (defaultFormObj.gender) {
                    ...
                    else if (defaultFormObj.gender === 'female') genderRatio = {"M": 0, "F": 1};
                    else if (defaultFormObj.gender === 'none') genderRatio = {"M": 0, "F": 0};
                ...
                if (defaultFormObj.preEvolutions && defaultFormObj.preEvolutions.length > 0) {
                    showdownPokemon['prevo'] = defaultFormObj.preEvolutions[0];
                }
                
                if (defaultFormObj.evolutions && defaultFormObj.evolutions.length > 0) {
                    showdownPokemon['evos'] = defaultFormObj.evolutions.map(evo => evo.to);
                }
                ...
                if (defaultFormObj.abilities) {
                    ...
                    if (defaultFormObj.abilities.hiddenAbilities && defaultFormObj.abilities.hiddenAbilities.length > 0) {
                        abilities['H'] = this.convertPixelmonAbilityToShowdown(
                ...
                if (defaultFormObj.dimensions?.height) {
                    terasPokemonData[baseShowdownId].heightm = defaultFormObj.dimensions.height / 10;
                }
                
                if (defaultFormObj.preEvolutions && defaultFormObj.preEvolutions.length > 0) {
                    terasPokemonData[baseShowdownId].prevo = defaultFormObj.preEvolutions[0];
                }
                
                if (defaultFormObj.evolutions && defaultFormObj.evolutions.length > 0) {
                    terasPokemonData[baseShowdownId].evos = defaultFormObj.evolutions.map(evo => evo.to);
                }
            }
            ...
            for (const form of pokemon.forms) {
                ...
                const isSpecialForm = ['mega', 'gmax', 'terasmega', 'terasgmax'].includes(form.name);
                ...
                if (abilitiesSource) {
                    ...
                    if (abilitiesSource.hiddenAbilities && abilitiesSource.hiddenAbilities.length > 0) {
                ...
                const showdownPokemon: any = {
                    ...
                    types: form.types ? 
                        form.types.map(type => type.charAt(0).toUpperCase() + type.slice(1).toLowerCase()) : 
                        defaultFormObj.types.map(type => type.charAt(0).toUpperCase() + type.slice(1).toLowerCase()),
                    baseStats: {
                        hp: statsSource?.hp || 0,
                        atk: statsSource?.attack || 0,
                ...
                if (form.dimensions?.height || defaultFormObj.dimensions?.height) {
                    showdownPokemon.heightm = (form.dimensions?.height || defaultFormObj.dimensions?.height) / 10;
                }
                ...
                } else if (defaultFormObj.preEvolutions && defaultFormObj.preEvolutions.length > 0) {
                    showdownPokemon.prevo = defaultFormObj.preEvolutions[0];
                }
