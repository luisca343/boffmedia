  ...
  private async loadOrGenerateManifest(): Promise<void> {
    try {
      if (fs.existsSync(this.MANIFEST_PATH)) {
        ...
        const manifestData = await fsPromises.readFile(this.MANIFEST_PATH, 'utf8');
        this.manifest = JSON.parse(manifestData);
        this.logger.log(`Loaded ${this.manifest.count.total} sprites from manifest`);
      } else {
...
async generateManifest(): Promise<void> {
  ...
  if (!allSpecies || allSpecies.length === 0) {
    this.logger.error('No Pokemon species found! Make sure Pokemon data is loaded first.');
    return;
  }
  ...
  for (const pokemon of allSpecies) {
    ...
    if (!pokemon.forms || pokemon.forms.length === 0) {
      this.logger.warn(`Pokemon ${pokemon.name} (${pokemonId}) has no forms, skipping`);
      continue;
    }
      ...
      if (form.genderProperties) {
        for (const genderProp of Object.values(form.genderProperties)) {
          ...
          for (const palette of genderProp.palettes) {
            ...
            await this.addSpriteToManifest(pokemonId, formName, paletteName, palette);
          }
        }
      }
      
      // Ensure we always have a "none" palette entry
      await this.addSpriteToManifest(pokemonId, formName, 'none', 
        form.genderProperties?.[0]?.palettes?.[0] || null);
    }
    
    // If no base form exists, use the first form as base
    if (!hasBaseForm && firstFormName) {
      this.logger.debug(`Pokemon ${pokemon.name} (${pokemonId}) has no base form, using ${firstFormName} as base`);
      ...
      for (const key of Object.keys(this.manifest.sprites)) {
        if (key.startsWith(`${pokemonId}:${firstFormName}:`)) {
          ...
          if (!this.manifest.sprites[baseKey]) {
            this.manifest.sprites[baseKey] = { ...this.manifest.sprites[key] };
  ...
  this.logger.log(`Generated sprite manifest with ${this.manifest.count.total} sprites (${this.manifest.count.default} default, ${this.manifest.count.custom} custom) in ${duration}ms`);
}
  ...
  private async addSpriteToManifest(
    ...
    try {
      ...
      const defaultResourcePath = path.join(this.DEFAULT_RESOURCEPACK_PATH, resourcePath);
      const customResourcePath = path.join(this.CUSTOM_RESOURCEPACK_PATH, resourcePath);
      ...
      if (fs.existsSync(customResourcePath)) {
        ...
        finalPath = path.join('/smartrotom/packs/default_resourcepack', resourcePath);
        isDefault = true;
        this.manifest.count.default++;
      } else {
        // Fallback to missingno if sprite not found
        finalPath = '/smartrotom/packs/default_resourcepack/assets/pixelmon/textures/pokemon/000_missingno/all/base/none/sprite.png';
        isDefault = true;
        this.manifest.count.default++;
      }
   ...
   */
  getSprite(pokemonId: number = 0, formName: string = 'base', paletteName: string = 'none'): string {
