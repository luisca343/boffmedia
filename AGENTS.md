...
This repo ships a custom MCP server `boff-agent` that owns context resolution,
guardrails, verification, BookStack docs, GitLab, and metrics. **Every AI agent
working in this repo (Claude, Copilot, OpenCode, any other) MUST use it for ALL
code-modifying tasks.** It is not optional. Full canonical rules:
[`.agents/boff-workflow.md`](./.agents/boff-workflow.md).

### Decide first

| Use `plan_goal` when... | Use `begin_task` when... |
|---|---|
| Spans 2+ NestJS modules | Single concern, single module |
| Spans 2+ apps (api + web) | Estimated < 10 files changed |
| Will change > 10 files | Already-approved task from a plan |
| Combines schema + service + controller + UI | Single layer, single design system |
| User says "feature", "migrate", "rebuild", "audit", "epic" | User says "fix", "tweak", "add X to Y" |

`plan_goal` decomposes into a BookStack chapter of `status:draft` task pages. STOP
after drafting. The user reviews and flips `status:draft` → `status:pending` before
any `begin_task` runs.

`begin_task` requires `taskType` (`feature` / `bugfix` / `refactor` / `ui-ux`).
`ui-ux` triggers the Playwright step.

### Rules

- Call `plan_goal` or `begin_task` BEFORE any Edit/Write on a code file.
...
Pure read-only questions ("explain X", "where is Y"), single-line typo fixes,
conversational replies, edits inside `packages/agent/**` itself, edits to
instruction / config files (`*.md`, `.mcp.json`, `opencode.json`). When in doubt,
USE the workflow.

If `boff-agent` MCP tools are not registered in your session, STOP and tell the
