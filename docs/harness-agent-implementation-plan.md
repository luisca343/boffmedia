...
| Phase | Deliverable | Usable checkpoint |
|---|---|---|
| 1 | Package scaffolding + Docker Compose env | `docker.sh up` works |
| 2 | Shell scripts layer | All scripts run and exit correctly |
| 3 | Tool layer (`runner.ts`, shared types) | Tool functions callable in isolation |
| 4 | MCP server (stdio) + `run_verification` + `check_guardrails` | Opencode can lint and check guardrails |
| 5 | `git_operation` + `reset_environment` + `seed_database` | Opencode can manage environment and git |
| 6 | `run_playwright` with trace + screenshot capture | Full verification pipeline via MCP |
| 7 | Agent memory + `save_run` + `get_run_history` | Opencode can persist and query run history |
| 8 | `resolve_context` | Relevant file injection per task |
| 9 | MCP server (HTTP/SSE) | Remote and persistent connections |
| 10 | Opencode config + end-to-end workflow test | Agent fully connected, usable in daily work |
| V2 | LLM adapters + orchestrator + queue + CLI | See `boff-agent-v2-autonomous.md` |

> **Tip**: Phase 10 is a complete, production-ready agent for interactive use with Opencode. V2 is purely additive — build it when you need to run tasks unattended, not before.
